...
Extensions may create a mount under `agent/extensions/`. For example, `eve add eve/self-modification` scaffolds an explicit self-modification extension mount. Connections write their initial definition under `agent/connections/` and install `@vercel/connect` when required. Instrumentation integrations write one destination file under `agent/instrumentation/`, so you can install and configure multiple exporters independently. Configure generated files and required environment variables before running your agent.
