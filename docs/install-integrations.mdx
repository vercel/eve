...
Web Chat installs a project-level Next.js application and cannot currently be added to a top-level `agents/` workspace. For that topology, create a root Next.js application and configure [`withEve({ agents })`](./guides/frontend/nextjs) instead. See [Next.js app with several agents](./project-structures#nextjs-app-with-several-agents) for the recommended layout. eve rejects `eve add channel/web` before writing files when the selected agent belongs to such a workspace.
