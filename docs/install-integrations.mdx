...
Web Chat installs a shared Next.js application at `apps/web` for either a standalone agent or a selected member of an agent workspace. In a workspace, setup asks whether Next.js should host the agents or whether Vercel should run the frontend and agents as peer services; peer services are recommended and used by non-interactive setup. Existing authored Next.js or Vercel configuration is preserved, and eve stops with manual composition guidance instead of overwriting it. See [Project Structure](./concepts/project-structure) for browser-app placement and [Next.js](./guides/frontend/nextjs) for manual integration.
...
Extensions may create a mount under `agent/extensions/`. For example, `eve add eve/self-modification` creates an authored self-modification mount. Connections write their initial definition under `agent/connections/` and install `@vercel/connect` when required. Instrumentation integrations write one destination file under `agent/instrumentation/`, so you can install and configure multiple exporters independently. Configure generated files and required environment variables before running your agent.
