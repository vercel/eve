...
Web Chat installs a shared Next.js application at `apps/web` for either a standalone agent or a selected member of an agent workspace. It configures the frontend and agents as peer Vercel services, so `pnpm dev:services` starts the complete graph locally. Existing authored Next.js or Vercel configuration is preserved, and eve stops with manual composition guidance instead of overwriting it. See [Project Structure](./concepts/project-structure) for browser-app placement and [Next.js](./guides/frontend/nextjs) for manual Next.js-owned integration.
