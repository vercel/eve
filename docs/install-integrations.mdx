...
Web Chat installs a project-level Next.js application and cannot currently be added to a top-level `agents/` workspace. For that layout, create a root Next.js application and wrap its config with [`withEve()`](./guides/frontend/nextjs); eve discovers the workspace agents automatically. eve rejects `eve add channel/web` before writing files when the selected agent belongs to such a workspace.
