...
## Set up an integration without prompts

`eve add <item> --non-interactive` never prompts. Instead of asking, it prints a structured NDJSON event describing what it still needs and exits with a code you can branch on:
...
Every non-interactive run ends on exactly one terminal event — `completed`, `blocked`, `failed`, or `cancelled` — including a `--skip-install` continuation. That event, not a repeat of the command, is how you confirm the outcome.

On exit code `2`, read the final event and run its `next.command`. For a non-secret question, the command includes `--answer key=<JSON value>`; replace `<JSON value>` with the answer you collected. The continuation already carries `--skip-install` once the registry files are installed, so the retry only resumes setup. Resupply any answers you provided earlier — each invocation starts from the recorded state, not from a live session.

Every `--answer` value is JSON, so strings need JSON quotes. When a question has a nested `editable.key`, you may answer both keys in one invocation:
...
Add `--yes` to accept recommended values and reduce round trips; explicit `--answer` values still take precedence. Never pass a secret in `--answer` — supply it through the environment variable or secret store the integration documents.

Two events need handling beyond a plain retry:

- An `external_action` event with `blocking: true` means the command is still running while it waits for a person to finish something out of band, such as an OAuth grant. Surface its URL and code, keep the process alive, and wait for the matching `external_action_resolved` event or a terminal event. Do not start a continuation.
- A completed event with `deploymentRequired: true` means the change only takes effect once deployed. Run the `next` command it reports.

A setup that authenticates through Vercel may report `eve link` as a prerequisite. Run it, then retry the continuation. See [Deployment](./guides/deployment/vercel.mdx) for the non-interactive `eve link` and `eve deploy` forms.
