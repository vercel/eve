...
Web Chat installs a project-level Next.js application and cannot currently be added to a top-level `agents/` workspace. For that topology, create a root Next.js application and configure [`withEve({ agents })`](./guides/frontend/nextjs) instead. See [Add a Web App](./guides/frontend/add-a-web-app) for the recommended arrangement. eve rejects `eve add channel/web` before writing files when the selected agent belongs to such a workspace.
