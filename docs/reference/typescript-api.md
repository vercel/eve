...
`getDeploymentSource` from `eve` returns the server-only GitHub repository, commit identity, and application root captured by a production build, or `null` when the build could not establish them. Complete `EVE_SOURCE_REPOSITORY`, `EVE_SOURCE_REVISION`, and `EVE_SOURCE_ROOT` build metadata takes precedence over Vercel Git metadata.
