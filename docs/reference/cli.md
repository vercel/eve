...
| `eve registry <command>`      | Manage sources and list, search, or view registry catalog items                                                                                       |
...
Commands for installing and discovering [shadcn registry](https://ui.shadcn.com/docs/registry) items. Bare item slugs use eve's official registry; URLs and configured registry addresses pass through to shadcn unchanged.

```bash
eve add agent-browser --yes
...
`eve registry add`, `remove`, and `sources` manage configured sources. `eve registry list` and `search` aggregate the official catalog and all configured sources by default, or browse one supplied URL or namespace. The wrapped shadcn CLI still requires its project configuration, including `components.json`.
