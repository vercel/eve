...
Both scaffold modes install dependencies. A fresh project initializes Git; an existing project keeps its repository and scripts. Human invocations run `eve dev` through the detected project package manager — fresh projects inherit a parent workspace manager when one is present, otherwise they use the manager that launched `eve init`. Coding-agent invocations print a handoff instead, and the agent can run that command with `--no-ui` for headless verification.
