...
| Target                                                                     | What happens                                                                                                                                                             |
| -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `eve init my-agent`                                                        | Creates an agent project in `my-agent/`, including when that directory already exists but is empty                                                                       |
...
If dependency installation fails for a new project, eve removes the incomplete target. A preexisting empty target is preserved but returned to an empty state. Resolve the package-manager error, then rerun `eve init`. If you initialized in a nonempty directory or added eve to an existing package, eve preserves the generated files and prints the package-manager command to retry instead. In a workspace, review shared configuration and lockfile changes after a failed initialization.
