...
A delegated action is an `invoke_agent` caller span in the parent trace (`INTERNAL` for a local subagent and `CLIENT` for a remote agent). A workflow tool remains an ordinary `agent.action` / `execute_tool` pair under its authored name; every `agent()` call inside it gets a separate caller span parented to that action. The child session opens its own trace, whose `agent.session` root links back to its caller; once the child confirms its trace identity, that caller also carries `agent.child.trace.id`. Remote agents propagate this context over `traceparent`.
