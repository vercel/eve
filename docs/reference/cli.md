...
When no authored `agent/instrumentation.ts` exists, local dev also records traces under `.eve/traces/`. That store is swept when a session finishes and once at startup: eve keeps traces belonging to open sessions, the twenty newest traces, and anything that received a span in the last seven days, then evicts oldest-first above a 512 MB budget. Tune it with `EVE_TRACES_MAX_AGE_MS`, `EVE_TRACES_MAX_TOTAL_BYTES`, and `EVE_TRACES_RETAIN_COUNT` in `.env.local`, or set `EVE_TRACES=off` to disable trace capture and sweeping entirely. See [instrumentation.ts](../guides/instrumentation#local-trace-retention).
