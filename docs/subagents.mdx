...
Subagent model calls automatically retry transient provider failures, including overload errors delivered after a stream starts. A retry repeats only the current uncommitted model call, so completed earlier steps, tool results, and sandbox work remain available to the child.
