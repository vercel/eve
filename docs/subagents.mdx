...
A subagent is a child agent that one agent delegates a focused subtask to. Split work into one to run it in parallel, to give the child a narrower set of tools, or to give a specialist its own identity. There are two kinds, the root-only built-in `agent` tool (a copy of the root agent) and declared subagents (specialists with their own directory).

## The built-in `agent` tool

The root agent gets an `agent` tool by default. The model calls it to delegate a subtask to a fresh copy of the root agent:
...
The copy shares the root's sandbox and tools, and a child's file writes are immediately visible to the root. That is what makes parallel calls natural: emit multiple `agent` calls in one response to fan out a small, fixed set of independent subtasks. eve runs that batch concurrently and returns every result before the root continues. Give children non-overlapping write scopes when they work in the shared sandbox. The copy inherits auth and connections, but starts with fresh conversation history and fresh state.

The built-in tool is deliberately one level deep. A recursively called copy does not receive `agent`, and declared subagents never receive the built-in tool. If a stale or forced recursive call reaches execution, eve rejects it instead of starting another child session.
...
Declared subagent delegation is not blocked by accumulated session depth. A declared subagent may call the nested subagents authored in its own directory, and that filesystem graph provides the boundary. The recursive built-in uses the stricter root-only rule above. `limits.maxSubagentDepth` remains accepted for existing configurations, but the root-only boundary supersedes it and declared chains do not use it.

`Workflow` is root-only. Delegated subagent sessions can still call their own visible declared or remote subagent tools, but they do not receive the `Workflow` orchestration wrapper or the recursive built-in. The number of subagent calls one Workflow program may dispatch is capped separately by `limits.maxSubagents` (default 100); see [Dynamic workflows](./guides/dynamic-workflows).
