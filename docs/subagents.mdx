...
eve lowers every subagent (built-in copy, declared, or [remote](./guides/remote-agents)) into a model-visible tool with the same default `{ message, outputSchema? }` shape. The parent packs `message` with everything the child needs, since the child never sees the parent's history. Set `outputSchema` to run the child in task mode, returning structured output as the tool result.

### Custom input schema

A declared or remote subagent can replace the default tool shape with its own `inputSchema`, so the parent model calls it with structured input instead of one free-form message. Pair it with `formatInput` to control how that input renders into the prompt the child receives:

```ts title="agent/subagents/issues/agent.ts"
import { defineAgent } from "eve";
...
export default defineAgent({
  ...
  formatInput: (input) =>
    `Create an issue titled "${input.title}" with priority ${input.priority ?? "medium"}.`,
});
```

`inputSchema` accepts a Standard Schema or a JSON Schema object and is lowered to JSON Schema at compile time. `formatInput` must be synchronous; eve still wraps its return value in the standard subagent preamble. Without `formatInput`, eve uses the input's `message` string when present, or the JSON-serialized input otherwise. Declaring `inputSchema` removes the per-call `outputSchema` escape hatch — the definition-level `outputSchema` still applies. Both fields are ignored on the root agent and on the built-in `agent` tool, which keeps the default shape.
...
A declared subagent's tool name is the bare path-derived name, with no prefix. `agent/subagents/researcher/` registers as the tool `researcher`. Unlike connection tools (`<connection>__<tool>`), it carries no namespace, so the model, approvals, logs, and evals all reference it by that name. Its default input schema is:
...
A definition-level `inputSchema` replaces this shape entirely (see [Custom input schema](#custom-input-schema)).
