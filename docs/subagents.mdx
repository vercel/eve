...
Declared subagent delegation is not blocked by accumulated session depth. A declared subagent may call the nested subagents authored in its own directory, and that filesystem graph provides the boundary. The recursive built-in uses the stricter root-only rule above. The former `limits.maxSubagentDepth` option has been removed because recursive delegation is always exactly one root-to-copy level.
