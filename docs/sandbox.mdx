...
Opt into `glob` or `grep` when the model needs dedicated file discovery or content search; see [Built-in tools](./concepts/default-harness#add-framework-provided-tools) for the definitions to export.
