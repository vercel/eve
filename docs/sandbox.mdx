...
export default defineSandbox({
  backend: vercel({
    ...
    authProxyBaseUrl: "https://eve-tunnel.example.com",
    networkPolicy: {
      allow: {
        "api.github.com": [
          {
...
On-request resolution uses Vercel's `forwardURL` firewall capability. The authenticated proxy writes a demand marker into the originating sandbox and returns HTTP 428. eve terminates the blocked awaited command, resolves only the demanded rules, then replays the command. Missing interactive credentials park and resume through eve's normal callback lifecycle. Replay can repeat effects performed before the blocked request, so commands should be safe to retry. Detached or otherwise un-awaited processes do not support demand-triggered consent.

Both modes require Vercel Pro or Enterprise because firewall transformations are not available on Hobby. On-request mode additionally uses a beta Vercel capability and requires a public HTTPS eve endpoint. Vercel deployments derive that endpoint automatically; local development must set `authProxyBaseUrl` to a public HTTPS tunnel or deployment. eve also uses this origin for interactive authorization callbacks, avoiding protected immutable deployment URLs when an explicit public origin is configured.
