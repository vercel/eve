...
These tools run with `/workspace` as the working directory. The model-facing file tools accept both absolute paths and paths beginning with `$HOME/`; eve resolves the latter inside the sandbox before reading, writing, or searching. Authored runtime functions can get a live sandbox handle with `ctx.getSandbox()`. A `defineWorkflowTool` must opt in with `sandbox: true`, and only its `"use step"` functions may call the accessor; calling it from the `"use workflow"` executor throws.
...
`ctx.getSandbox()` takes no arguments and is async. For ordinary authored callbacks, it works
inside eve-managed runtime execution. For workflow tools, it works only inside `"use step"`
functions after the tool opts in with `sandbox: true`. See
[Use the sandbox from steps](./tools/workflows#use-the-sandbox-from-steps).
...
Workflow-tool steps do not support `delete()`. Delete the sandbox from another authored runtime
callback that owns the session sandbox.
