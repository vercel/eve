...
The default image includes Node, npm, pnpm, Git, curl, and ripgrep. It does not include Python or other language runtimes. Build an OCI image from the eve image when your local sandbox needs additional system packages:

```dockerfile title="Dockerfile"
FROM ghcr.io/vercel/eve:your_eve_version
...
Pass the resulting image to `docker()` or `microsandbox()`:
...
export default defineSandbox({
  backend: docker({ image: "ghcr.io/acme/my-eve-python:1.0.0" }),
});
```

To use the same image when either local OCI backend is available, configure both through `defaultBackend()`:
...
export default defineSandbox({
  backend: defaultBackend({
    docker: { image: "ghcr.io/acme/my-eve-python:1.0.0" },
    microsandbox: { image: "ghcr.io/acme/my-eve-python:1.0.0" },
  }),
});
```

`vercel()` always uses eve's version-matched Vercel Sandbox image, so its `image` cannot be overridden. Install additional runtimes in `bootstrap` when you deploy on Vercel, or supply a Vercel Sandbox `source` snapshot, Git revision, or tarball as the template base layer.
