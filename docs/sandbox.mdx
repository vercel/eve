...
Opt into `glob` or `grep` when the model needs dedicated file discovery or content search; see [Built-in tools](./concepts/built-in-tools#add-framework-provided-tools) for the definitions to export.
