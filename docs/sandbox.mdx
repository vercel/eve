...
With the Vercel backend, `vercel()` can resolve non-interactive authorization strategies for every runtime step and pass the results to a function-form policy:
...
export default defineSandbox({
  backend: vercel({
    credentials: {
      service: {
        async getToken() {
          return { token: process.env.EXAMPLE_API_TOKEN! };
        },
      },
    },
...
The strategy uses the same `getToken` contract as connection authorization and may set `principalType: "user"` for per-user credentials. Interactive authorization is not supported here because sandbox attachment cannot pause for consent.

Eve resolves the credentials before authored step code runs, applies the resulting policy, and replaces every brokered token with an empty value when the step ends or fails. If resolution fails, that credential is also represented by an empty token while the authored egress restrictions remain active. A failed cleanup fails the step rather than being ignored. Brokered credentials are unavailable during template bootstrap.

This managed form belongs specifically to `vercel()`; a generic sandbox backend is not required to implement it. Avoid also setting network policy through `onSession` or `setNetworkPolicy` when using managed brokering, since a second writer can replace the brokered policy.

You can also construct transforms directly on Vercel Sandbox or microsandbox:
...
For mid-turn brokering, call `setNetworkPolicy` with the same shape. The [Vercel Sandbox docs](https://vercel.com/docs/sandbox) cover the firewall mechanism itself.
