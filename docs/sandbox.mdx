...
export default defineTool({
  description: "Run a Node analysis script and return its output.",
  inputSchema: z.object({ script: z.string() }),
  async execute({ script }, ctx) {
    const sandbox = await ctx.getSandbox();
    await sandbox.writeTextFile({ path: "analysis/run.mjs", content: script });
    const result = await sandbox.run({ command: "node analysis/run.mjs" });
...
const server = await sandbox.spawn({
  command:
    "node -e \"require('node:http').createServer((_, response) => response.end('ok')).listen(8000)\"",
});
...
`docker()` drives the Docker CLI directly. By default, it uses the `ghcr.io/vercel/eve` image tag matching the installed eve version. eve creates `/workspace` and verifies Bash during framework setup, before authored bootstrap code runs. Templates are committed as local Docker images and reused across sessions when the sandbox source, seed files, `revalidationKey`, and Docker backend options still match. Sessions run as long-lived containers whose filesystems persist `/workspace` changes across turns for the same durable session. `eve dev` prunes stale template images in the background.

### microsandbox

`microsandbox()` runs each sandbox in a lightweight local VM with snapshot-backed templates, a `vercel-sandbox` user, and a firewall capable of domain-level network policies and credential brokering. It is the closest local match to hosted Vercel Sandbox. By default, it uses the `ghcr.io/vercel/eve` image tag matching the installed eve version. Supported hosts are macOS on Apple Silicon, or Linux (glibc) with KVM. The `microsandbox` npm package and its VM runtime are not bundled with eve, so `eve dev` installs both automatically when missing (disable with `setup: { autoInstall: false }`); production processes fail with actionable install errors instead.

### Add runtimes with a custom image

The default image includes Node, npm, pnpm, Git, curl, and ripgrep. It does not include Python or other language runtimes. Derive an image from eve's version-matched base image when a Vercel Sandbox needs additional system packages:

```dockerfile title="Dockerfile"
FROM vcr.vercel.com/vercel/eve/base:your_eve_version
...
Build and push the image to a [Vercel Container Registry](https://vercel.com/docs/container-registry) repository. From the linked Vercel project, use the Vercel CLI; it authenticates Docker and adds the team and project segments to the resulting image reference:

```sh
vercel vcr build docker . eve-python:your_eve_version --push
```

Pass the resulting VCR image to `vercel()`. Its reference includes your team slug, Vercel project name, repository name, and tag:
...
export default defineSandbox({
  backend: vercel({
    image: "vcr.vercel.com/your-team/your-project/eve-python:your_eve_version",
  }),
});
```

To customize a local Docker or microsandbox backend instead, derive from `ghcr.io/vercel/eve:your_eve_version` and pass the resulting image to that backend.
