...
Sandbox definitions use two distinct forms. Pass an object to give an agent or subagent an independent sandbox. The callback form exists only for sharing: return `parent.sandbox` to select the dispatching agent's exact live sandbox:
