...
Use `toolCall.toModelOutput`, keyed by operation name, to project a smaller
model-facing result for selected operations. It follows the same contract as
[MCP result projection](./mcp#control-result-size): `action.result`, channel events,
and hooks retain the full `{ status, statusText, body }` response. Operations without
a matching entry keep the default serialization. Non-2xx responses and thrown errors
