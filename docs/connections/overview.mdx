...
`credentialOwner` is eve's field for raw connection auth. Existing raw `principalType` definitions still work, but are deprecated. `principalType` remains the correct option for `connect()` from `@vercel/connect/eve`, where it belongs to the external Vercel Connect API, and for route-auth session identities.
