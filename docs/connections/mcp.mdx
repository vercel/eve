...
By default, eve sends the remote result to the model. Use
`toolCall.toModelOutput`, keyed by remote tool name, to project a smaller result
for tools that return more data than the model needs:

```ts title="agent/connections/traces.ts"
import { defineMcpClientConnection } from "eve/connections";
import { z } from "zod";
...
export default defineMcpClientConnection({
  url: "https://traces.example.com/mcp",
  description: "Search traces.",
  tools: { allow: ["get_trace"] },
  auth: { getToken: async () => ({ token: process.env.TRACES_API_KEY! }) },
  toolCall: {
    toModelOutput: {
      get_trace(result) {
        ...
        return {
          type: "json",
          value: { contentParts: parsed.content.length },
        };
      },
    },
  },
...
Each callback receives the remote tool result. Validate its shape before extracting
fields, and preserve pagination or other metadata needed to interpret a partial
result. Return the same `text`, `json`, or `content` shape as an authored tool's
[`toModelOutput`](/docs/tools#shape-what-the-model-sees-with-tomodeloutput).

Only the model-facing representation changes. `action.result`, channel events, and
hooks retain the full remote result, and tools without a matching entry keep the
default serialization. MCP results marked `isError: true` and thrown remote errors
bypass the projection.

When you control the MCP server, prefer pagination or a purpose-built search or
summary tool. A projection reduces predictable, well-structured output; it is not a
general size limit for unexpected responses.
