...
eve stores the mapping in `package.json#registries`. The `{name}` placeholder becomes the item name, so `@acme/analytics` resolves to `https://registry.acme.com/r/analytics.json`.
...
Install a known item URL directly when you do not need a namespace:
