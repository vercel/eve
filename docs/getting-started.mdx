...
## When to use multiple root agents

Start with one root agent. Use a [declared subagent](./subagents) when one agent needs to delegate specialized work with a different prompt, tool surface, or sandbox.

Use a multi-agent project only when agents need separate public endpoints, channels, schedules, deployments, or ownership boundaries. See [Multi-Agent Projects](./multi-agent-projects) to create a workspace or add another root agent.
