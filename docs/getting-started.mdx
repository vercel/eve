...
The [terminal UI](/docs/guides/dev-tui) opens after interactive initialization. To return later, enter the project and run:
...
This starts an interactive session where you can send messages to your agent. Edit `agent/instructions.md` to change its behavior and `agent/agent.ts` to configure its model. eve reloads your changes as you work. You can also ask the agent to update its own instructions, tools, or skills; see [Self-Modification](./guides/self-modification).
