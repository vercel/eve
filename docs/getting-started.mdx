...
New pnpm workspace files set `minimumReleaseAge: 0` so freshly published dependencies remain usable when pnpm checks the lockfile during later installs and dev commands. Existing workspace policies are preserved. For a project created by an older eve release that fails with `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION`, add `minimumReleaseAge: 0` to its `pnpm-workspace.yaml`, then rerun `pnpm dev`.
