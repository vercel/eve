...
The quick start uses `eve init` for a guided scaffold; `eve init .` does the same inside an existing project, adding the `eve`, `ai`, and `zod` dependencies and a compatible Node engine for you.
