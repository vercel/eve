...
eve builds an agent by walking the filesystem under `agent/`. Each directory is an authored slot, and the slot a file lands in determines how eve loads it. For choosing a layout that includes a Next.js app or several root agents, see [Project structures](./project-structures).
