...
A pending question from a task, such as a `ctx.ask()` question from a workflow tool like `ask_question` or a question a subagent asks, can be answered with plain text. When it is the only pending question, a message that matches an option answers it, and so does any message when the question allows free text. Otherwise eve does not guess which question the text addresses: the message follows the normal `turnPolicy`, questions this session's workflow tools created with `dismissible: true` resolve as `dismissed`, and other questions stay open. Only a message from a person answers or dismisses a question this way, never one a delegating caller sends. In an interactive root session, a steering message that answers nothing also moves a call whose question stays open to the background; the question remains answerable. Use structured responses to target requests unambiguously.
