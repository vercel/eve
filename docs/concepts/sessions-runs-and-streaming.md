...
Agent calls and workflow tool calls run as tasks. A delegated subagent publishes progress on its own child-session stream. When the child starts, the parent emits `task.started` with the `taskId`, the `callId` of the delegating call, the agent `name`, and a `child` with the child `sessionId` and the `streamPath` that a client follows with `session.streamSubagent()`. `kind` is `agent`, and `mode` is `foreground` because the calling turn waits for the result.

Every [workflow tool](/docs/tools/workflows) call is also a task. When its run starts, the parent emits `task.started` with `kind: "workflow"`, the tool `name`, and no `child`, because a workflow run has no session stream to follow. Its progress stays on `action.partial` and its result on `action.result`. Consumers that only follow delegation should filter `task.*` events by `kind` and `name`.

A workflow tool defined with [`detach: true`](/docs/tools/workflows#return-a-receipt-with-detach) starts a task with `mode: "background"`. Its `action.result` arrives at once with the receipt `{ status: "working", taskId }` as `output`, and its `task.settled` arrives when the run finishes, often after the turn that started it.

A waited agent or workflow tool call that [detaches](/docs/tools/workflows#detach-a-waited-call) emits `task.detached` with its `taskId`, `callId`, and `reason`: `"steer"` when a steering message moved it to the background, or `"timeout"` when the tool's `detach: { timeout }` timer fired. Its `action.result` then carries the same receipt, and its `task.settled` arrives when the task finishes. `task.detached` belongs to the turn that made the call, so a `MessageResponse` for the steering message does not include it; read the session stream to observe it. eve then delivers the result to the model as session input, which appears on the stream as `message.received` with `data.kind: "task.result"` and `data.taskIds`, usually at the start of a result turn. That event is not a user message: the default client reducer, the dev TUI, and the built-in channels do not render it as one, and custom renderers should skip it too.

Every call that starts a task emits exactly one `task.settled` with its first outcome: `completed` with `output`, `failed` with `error: { code, message }`, or `cancelled` when the parent cancels the call. `usage` is present when the child reported its token spend. A call that fails before its child starts emits `task.settled` without `task.started`; for a workflow tool call whose run could not start, or a remote agent call that could not start, `error.code` is `START_FAILED`. A call still working at its time limit (for an agent, 2 hours of active time unless its definition sets `timeout`) settles `failed` with `error.code` `TIMED_OUT`, and a call whose child session ends before it replies settles `failed` with `AGENT_SESSION_ENDED`. A call rejected before a task exists, such as one naming an unknown `agentId`, emits neither, and its failure appears only on `action.result`. For an agent, `taskId` is the agent ID, so continuing the agent emits another `task.started` and `task.settled` pair with the same `taskId` and a new `callId`. Settlement does not mean that the reusable child session has ended.

When the parent proxies a child's `input.requested`, `authorization.required`, or `authorization.completed` event onto its own stream, the event carries that child's `taskId`. An `input.requested` for a workflow tool call's question or approval carries the call's `taskId` too.
...
A pending `ctx.ask()` question from a workflow tool, such as `ask_question`, can be answered with plain text. When it is the only pending question, a message that matches an option answers it, and so does any message when the question allows free text. Otherwise eve does not guess which question the text addresses: the message follows the normal `turnPolicy`, questions created with `dismissible: true` resolve as `dismissed`, and other questions stay open. In an interactive root session, a steering message that answers nothing also moves a call whose question stays open to the background; the question remains answerable. Questions from subagents always need a structured response. Use structured responses to target requests unambiguously.
