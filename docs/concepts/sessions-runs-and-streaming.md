...
A workflow tool defined with [`detach: true`](/docs/tools/workflows#return-a-receipt-with-detach) starts a task with `mode: "background"`. Its `action.result` arrives at once with the receipt `{ status: "working", taskId }` as `output`, and its `task.settled` arrives when the run finishes, often after the turn that started it. eve then delivers the result to the model as session input, which appears on the stream as `message.received` with `data.kind: "task.result"` and `data.taskIds`, usually at the start of a result turn. That event is not a user message: the default client reducer, the dev TUI, and the built-in channels do not render it as one, and custom renderers should skip it too.
