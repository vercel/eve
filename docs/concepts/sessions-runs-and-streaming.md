...
`reasoning.appended`, `message.appended`, and `action.input.appended` stream incremental output as it arrives. Each append stores only its new text in `reasoningDelta`, `messageDelta`, or `inputTextDelta`. Accumulate the deltas in stream order when you need the text so far. eve writes the first delta of each stream immediately, then coalesces adjacent deltas for the same event type, stream coordinates, and tool `callId` into about one append every 100 ms, sooner when its queue fills. Other events flush pending deltas first. The resulting text and event ordering stay the same.
