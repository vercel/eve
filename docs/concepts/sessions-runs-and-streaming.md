...
When a model returns no text or tool calls, eve retries the model call once with a temporary recovery instruction. That instruction asks the model to continue the current request, use fresh reads when requested, reuse suitable results, and avoid repeating completed writes. It does not change tool permissions or approval requirements and is not saved in session history.
