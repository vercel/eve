...
A turn doesn't end while tasks it started are working. When the model ends its text early, that `message.completed` reports `finishReason: "tool-calls"`, and the turn continues once a result arrives.
