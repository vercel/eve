...
A delegated subagent publishes progress on its own child-session stream, which a client attaches to through the `childSessionId` on the parent's `subagent.called`. The parent re-emits a narrow slice of that stream so a channel bound to the parent session stays useful without attaching: the authorization and human-input events it has to respond to, plus the `actions.requested` and `action.result` pairs for subagent and remote-agent calls the child dispatched onward. A child's own tool calls stay on the child stream, and a forwarded `actions.requested` carries only the dispatch entries. Forwarded events keep the child's `turnId`, which is how a consumer tells them apart from the parent's own.
