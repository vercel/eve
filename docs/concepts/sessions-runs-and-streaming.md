...
| `subagent.admitted`       | A background subagent was admitted and returned its working task receipt.                                                                          |
| `subagent.completed`      | A subagent invocation returned a successful result; its child session can remain reusable.                                                         |
...
A delegated subagent publishes progress on its own child-session stream. The parent emits `subagent.called` with a `childSessionId`, which a client uses to attach. `subagent.admitted` carries a working task receipt after admission; terminal outcomes arrive as task-triggered `message.received` notifications with `data.kind: "execution.background_task"`. The default frontend reducer retains these events in `events` but does not project their runtime-authored text as participant messages.
...
`subagent.completed` marks a successful blocking invocation result. It does not end the child session: persistent children can accept another invocation. Background task completion, failure, and cancellation remain separate task notifications. Stream version 26 separates admission from completion; the client normalizes receipt-bearing `subagent.completed` events from older streams into `subagent.admitted`.
