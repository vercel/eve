...
| `message.received`        | Input reached a turn; carries flattened text plus structured text/file parts. `kind: "task.result"` marks task results from eve. |
...
When a request explicitly makes delivery conditional, such as a scheduled check that reports only changes, and there is nothing new to report, the agent can finish with exactly `<eve-empty-delivery/>`. eve emits `message.completed` with `message: null` for that intentional silence. The marker must be the entire response, apart from surrounding whitespace; the HTML-escaped form `&lt;eve-empty-delivery/&gt;` is also accepted. A response that quotes the marker in prose or code is delivered normally.

Agent calls and workflow tool calls run as [tasks](./tasks). `task.started` announces each generation of a task with its `taskId`, `generation`, the `callId` of the call that started it, its `name`, `kind` (`agent` or `workflow`), and `resumable`. `mode` is `attached` when the calling turn or workflow body awaits the result and `detached` when the call returned a receipt, which is the default for agent calls and workflow tool calls. A delegated subagent publishes progress on its own child-session stream: its `task.started` carries a `child` with the child `sessionId` and the `streamPath` that a client follows with `session.streamSubagent()`. A workflow tool call has no `child`; its progress stays on `action.partial` and its result on `action.result`. Consumers that only follow delegation should filter `task.*` events by `kind` and `name`.

A detached call's `action.result` carries the receipt `{ status: "working", taskId }`. Each generation emits exactly one `task.settled` with its first outcome: `completed` with `output`, `failed` with `error: { code, message }`, or `cancelled`, and each task emits one `task.ended` when it stops taking input. eve delivers a result that no `task_wait` received to the model as session input, which appears on the stream as `message.received` with `data.kind: "task.result"` and `data.taskIds`, inside the turn that started the task. That event is not a user message: the default client reducer, the dev TUI, and the built-in channels do not render it as one, and custom renderers should skip it too.
...
A pending question from a task, such as a `ctx.ask()` question from a workflow tool like `ask_question` or a question a subagent asks, can be answered with plain text. When it is the only pending question and the session waits on no approval or session-limit prompt of its own, a message that matches an option answers it, and so does any message when the question allows free text. Otherwise eve does not guess which question the text addresses: the message follows the normal `turnPolicy`, questions this session's workflow tools created with `dismissible: true` resolve as `dismissed`, and other questions stay open. Only a message from a person answers or dismisses a question this way, never one a delegating caller sends. A steering message that answers nothing cancels an attached call whose question stays open; a detached task's question remains answerable. Use structured responses to target requests unambiguously.
...
`"accepted"` means the live session durably queued the request; cancellation completes asynchronously. Confirm turn cancellation on the stream as `turn.cancelled` followed by `session.waiting`. The session then accepts the next message normally. Each cancelled child reports its own boundary on its child-session stream. A live but already-parked session returns `"accepted"`; turn cancellation is a no-op there, and working tasks are still cancelled: every one, or only those the named turn started when the request passes `turnId`. The exception is a parked session that still owes the result of its last turn, such as a subagent session parked on an approval: cancelling it cancels every task it started, and its caller receives the call as cancelled; the subagent stays available for new work. `"no_active_turn"` means the session or channel address is unknown or terminal. Both statuses are success, so clients can fire and forget. See the [eve channel](../channels/eve) for the full route contract.
