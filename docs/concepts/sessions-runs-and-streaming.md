...
Every call that starts a task emits exactly one `task.settled` with its first outcome: `completed` with `output`, `failed` with `error: { code, message }`, or `cancelled` when the parent cancels the call. `usage` is present when the child reported its token spend. A call that fails before its child starts emits `task.settled` without `task.started`; for a workflow tool call whose run could not start, `error.code` is `START_FAILED`. An agent call still working after 2 hours of active time settles `failed` with `error.code` `TIMED_OUT`. A call rejected before a task exists, such as one naming an unknown `agentId`, emits neither, and its failure appears only on `action.result`. For an agent, `taskId` is the agent ID, so continuing the agent emits another `task.started` and `task.settled` pair with the same `taskId` and a new `callId`. Settlement does not mean that the reusable child session has ended.
