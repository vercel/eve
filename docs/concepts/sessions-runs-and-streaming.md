...
`reasoning.appended`, `message.appended`, and `action.input.appended` stream incremental output as it arrives. Each append stores only its new text in `reasoningDelta`, `messageDelta`, or `inputTextDelta`. Apply `startsBlock: true` by resetting the accumulator before adding that delta; apply `false` by appending to the existing block. This avoids repeating the cumulative block in every durable event while letting consumers replace partial output when a model retry starts a new block. The reset operation is idempotent: normalized v24 tool-input streams may repeat it for the empty input-start event and first content delta. When the durable stream writer is busy, eve may coalesce adjacent continuation deltas for the same text block or tool call. Text remains in source order; `startsBlock: true`, a different event type, tool `callId`, or stream coordinate forms an ordering barrier.
