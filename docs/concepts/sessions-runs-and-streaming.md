...
The client validates the `x-eve-stream-version` header on every connection and supports both v24 cumulative appends and v25 offset appends. It normalizes v24 events to the current offset contract before exposing them, so a reconnect can cross deployments without changing the reducer input. A current server also normalizes v24 events when replaying a session written by an earlier deployment. A missing or unsupported version, or an append whose fields do not match its declared version, fails instead of being interpreted as a current event.
