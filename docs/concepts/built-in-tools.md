...
- **`bash`** accepts a returned process id through the same tool: use `poll` to read current output, `wait` to wait for another bounded foreground interval, or `kill` to stop it. Every result reports `wallTimeSeconds`, and one eve runtime tracks at most 64 commands per sandbox (completed command state is reclaimed first). Vercel commands can be reattached after the app runtime relocates; other backends report an unavailable process id when their runtime-local handle is lost. An unavailable process id is reported rather than replayed.
