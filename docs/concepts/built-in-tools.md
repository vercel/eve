...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| `bash`              | Run a shell command. After a bounded foreground wait (30 seconds at most), a command still running continues in the background and returns a process id.                                                            | Sandbox       |
...
- **`bash`** caps every foreground interval at 30 seconds. `yieldTimeMs` can request a shorter interval; near a Vercel Function deadline, eve yields earlier to leave time for the tool step to commit. A yielded command returns a process id. On Vercel, a durable completion monitor reconnects to the detached Sandbox command and queues a new session message with its exit code and bounded stdout/stderr, so the model does not need to poll. A parked session starts a turn for that message; an active turn receives it after the current turn settles. The monitor serializes completion against `kill`: an accepted kill returns synchronously and suppresses any completion message not already delivered. `poll`, `wait`, and `kill` remain available through the same tool. Other backends need reconnectable command support before they can provide the same asynchronous delivery guarantee. Every result reports `wallTimeSeconds`, and one eve runtime tracks at most 64 commands per sandbox (completed command state is reclaimed first).
