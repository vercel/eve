...
Steering waits for the current batch to commit, then adds the accepted message
to the active turn before the next model step. Increasing the batch size also
increases the interval between steering boundaries.
...
When a session is waiting, a delivery through its ID or any claimed channel address wakes it and starts the next turn. Message sends default to `turnPolicy: "steer"`: while a turn is active, the session owner buffers the message and applies it after the current workflow step commits. The model call and tool work complete safely, and steering continues the same turn with the same turn ID. Before emitting the turn's completion events, the owner checks for steering accepted at that boundary.

This check also runs when the model produces a final answer. Pending steering
continues the open turn; otherwise, a separate durable step completes it without
another model call. Input accepted after this final check starts the next turn.
Text already streamed remains visible, and provider-managed tools within a
single model response do not create additional steering boundaries.
