...
The world package backs workflow state, queues, hooks, and streams. `world` accepts a bare package name, not a package subpath, filesystem path, or URL. Keep secrets and deployment-specific options in runtime environment variables read by that package, not in `agent.ts`. Custom worlds must declare and implement the runtime protocol expected by eve's vendored `@workflow/*` packages (currently the `5.0.0-beta` line); eve verifies the installed package during compilation and rejects an incompatible line before loading its code. See [Self-host eve](../guides/deployment/self-hosting#persist-workflow-state), [agent.ts](../agent-config#workflow-world), and [Workflow Worlds](https://workflow-sdk.dev/worlds).
