...
The first execution step reads the latest settled session snapshot once, after
claiming the turn inbox. Intermediate state travels through Workflow step inputs
and results. At turn end, eve finalizes lifecycle effects, appends one full session
snapshot, and then releases the inbox. The shared snapshot stream is not read or
written between execution steps. Live response events use a separate event stream.

Event writes enqueue locally and drain eagerly in order while model preparation
and execution continue. Opening storage and writing preamble events do not block
the model request. The step registers its final drain with `waitUntil` and waits
for persistence before completing, so later steps and stream closure cannot
overtake earlier output. Lifecycle handlers that prepare context still run before
the model call. A failed drain aborts active model work and fails the step.
...
Snapshot publication is a separate step after finalization. If its write is retried,
it reuses the finalized state without rerunning lifecycle handlers. A retry can append
an identical snapshot; the latest record remains the settled state. Intermediate
execution uses normal Workflow retries rather than additional snapshot markers.
