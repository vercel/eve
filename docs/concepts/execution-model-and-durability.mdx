...
When upgrading eve, restart sessions pinned to an older inbox wire format before
sending them new commands. eve sends the current format without negotiating with
the session's deployment. Redeploys that keep the same inbox format can continue
existing sessions; current consumers still decode older persisted payloads during
replay.
