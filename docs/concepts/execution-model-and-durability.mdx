...
Sessions with a channel continuation address stay on their current deployment.
Moving those sessions safely requires an atomic handoff primitive that keeps
every address continuously owned. ID-only sessions can hand off because their
stable session address does not create a replacement session when temporarily
unowned.
