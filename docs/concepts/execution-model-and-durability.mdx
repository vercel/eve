...
Turn-step results record changes to conversation history and serialized session
context. During replay, the workflow applies those changes to its current state
without rerunning completed model or tool calls. Appended messages do not repeat
earlier history in each result; edits, deletions, and context changes are retained.
Cyclic or shared objects are replaced together within their containing state
subtree. Values with custom serialization retain a full replacement so their
serialization semantics are preserved.

Step inputs still store the full state. Delta results reduce repeated output
storage, but total storage can still grow quadratically with a continuously
growing conversation. Initial creation and deployment handoffs retain full
checkpoints.
