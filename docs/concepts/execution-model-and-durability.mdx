...
The long-lived session driver remains on its original deployment. New message handlers use a compatible delivery envelope so that driver can keep forwarding messages and input responses to subsequent turns without restarting the conversation. New session controls can still require newer driver behavior: for example, `cancel({ tasks: true })` is rejected before delivery when the existing driver predates session-owned task cancellation. Ordinary messages and turn cancellation continue to work for that session.
