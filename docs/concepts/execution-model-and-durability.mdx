...
Handoff requires a readable checkpoint with embedded session state. eve does not
migrate stream-only checkpoints from the former execution model; start a new
session when upgrading from that model. Transparent rollback to a deployment
that uses the former model is also unsupported; retire and restart affected
sessions before rolling back. Authorization callback URLs without an attempt ID
are unsupported; restart authorization to obtain a current URL.
...
Deployment handoff currently has a brief interval between releasing and
reclaiming the addresses. ID-addressed requests retry missing hooks within a
bounded window. An alias-only request cannot distinguish that interval from an
unowned address and may start a competing candidate. Continuous alias ownership
requires an atomic handoff primitive from Workflow.
