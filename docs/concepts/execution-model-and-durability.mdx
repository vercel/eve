...
in default execution; background workflow tools consume yields without publishing progress. See [background execution](../tools#background-execution) and
...
A handoff moves each of the session's addresses to the successor atomically,
so a delivery that arrives mid-handoff always reaches a live owner. A channel
therefore never starts a replacement session for an address that is
mid-handoff. Messages the previous owner accepted just before the move are
forwarded to the successor in the order it accepted them. A deployment running
eve 0.66.3 or earlier cannot take addresses over, so after a rollback to such a
deployment, sessions stay on their current deployment.

If the session's current owner was started by a handoff from eve 0.66.2 or
earlier, or by a legacy-session import, its next handoff releases the
addresses briefly instead. That owner leaves a short-lived marker for each
address, so a delivery that lands in the interval retries until the successor
claims the address instead of being reported as an inactive session.
