...
Child sessions can still call their own declared or remote subagents, but they receive neither the root-only `workflow` tool nor the built-in `agent`. On the root, adding the [dynamic workflow tool](./tools/dynamic-workflows) makes visible subagents available to the model-authored program while keeping the same subagents available for direct background calls. Calls inside `workflow` await final child results instead of returning task receipts.
