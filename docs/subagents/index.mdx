...
A subagent can finish a model turn while its own background tasks are still running.
That turn yields; its interim reply does not complete the caller's delegated task. If the child session expires, resets, or closes before replying, the delegated task fails and reports the usage accumulated since its previous settlement.
