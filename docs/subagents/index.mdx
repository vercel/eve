...
`subagent.completed` reports what the call returned. With `data.backgroundTask`, it is a working receipt and the child is still running. Without that marker, a successful blocking call has returned its actual result. Neither event ends the reusable child session. Background task outcomes arrive through task notifications.
