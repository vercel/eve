...
A subagent can finish a model turn while background tasks started in that turn are still running.
That turn yields; its interim reply does not complete the caller's delegated task. Background work started by an earlier turn does not hold the reply, and a turn that ends with an error always answers the caller. If the child session expires, resets, or closes before replying, the delegated task fails and reports the usage accumulated since its previous settlement.
After the nested tasks finish, the subagent resumes and returns its final result to
the caller. This applies to root-agent copies, declared local subagents, and remote
subagents. The subagent session remains available for later calls.
...
terminal results await delivery. With `taskDeliveryPolicy: "cohort"`, eve holds
completion, failure, and cancellation notifications
until every task in that cohort has completed, failed, or been cancelled, then
delivers all outcomes together in one parent turn. Partial settlements
do not invoke the parent model. Work started after the cohort settles forms a new
cohort.

New channel sessions default to `"auto"`; schedules default to `"cohort"`.
Pass `taskDeliveryPolicy` to `send(...)` to select or update the session policy. See
[Background task results](/docs/channels/overview#background-task-results).

With `"auto"`, a terminal outcome can start a parent turn while other tasks are
still running. The parent reports independently useful results and stays silent
when a useful answer depends on unfinished tasks. Ready results may share a turn.
The reporting context includes the whole cohort and every available output,
including previously withheld results, for a later combined report.

The setting applies to background tools and built-in, declared, and remote
subagents owned by the session. Internal subagent sessions use `"cohort"` for
background work they start.
...
`task_cancel` is available to sessions that own background tasks, including a child that starts nested background work. Cancelling a delegated agent task also requests cancellation of its child's nested background tasks, even after that child's model turn has yielded. The child does not start another model turn for notifications from that cancelled work. This applies to self, local, and remote delegation. The calling model receives the cancellation receipt in the `task_cancel` result; a later report of that same outcome does not start another model turn.
...
### Let the parent choose the model

Use `choice` from `eve/models` to let the caller pick a model when it starts a child. The first entry is the default, and each description tells the caller when to pick it:
...
export default defineAgent({
  description: "Investigate ambiguous questions before the parent agent responds.",
  model: choice({
    "anthropic/claude-sonnet-5": "Routine lookups where speed matters.",
    "anthropic/claude-opus-5.5": "Hard questions that need careful reasoning.",
  }),
});
```

The subagent's tool then accepts an optional `model` field limited to those slugs, with the descriptions in its schema. `ctx.agent("researcher", { message, model })` accepts the same slugs, and `ctx.agents.researcher.models` lists them. The pick behaves like a `session.started` [dynamic model](./agent-config#choose-the-model-dynamically) selection: it applies to the new child for its whole lifetime, and compaction follows the chosen model's context window.

`choice` also takes a list. A list entry is a slug or `{ model, description?, modelOptions? }`, where `model` may be an AI SDK `LanguageModel`; the caller then sends its Gateway slug. `modelOptions` applies to that choice only, so AI Gateway fallback models still work inside a choice:
...
model: choice([
  ...
  {
    model: "anthropic/claude-opus-5.5",
    modelOptions: { providerOptions: { gateway: { models: ["openai/gpt-5.5"] } } },
  },
]),
```

- Omitting `model` uses the first entry.
- An unlisted slug, or `model` sent to a subagent without `choice`, is rejected as a tool error.
- `model` with the `agentId` of an existing child is rejected; the child keeps its model.
- Context windows come from the Gateway catalog, so `choice` does not accept a sibling `modelContextWindowTokens` or `modelOptions`. Slugs must be unique.
- `choice` applies only to declared local subagents. A root agent has no caller, so the build rejects `choice` there. Dynamic subagent configs must return one static model.
...
```ts
{
  ...
  agentId?: string;      // continue or steer an existing child
  model?: string;        // only for subagents whose `model` is an array; new children only
  outputSchema?: object; // require structured output for this turn
}
```
...
Each delegated subagent spins up its own child session and stream. The parent stream carries the control-plane events `subagent.called` and `subagent.completed`, plus interactive `input.requested`, `authorization.required`, and `authorization.completed` events proxied from descendants so the root channel can prompt the user. To follow the child's other progress, pass the `subagent.called` event to [`session.streamSubagent()`](./guides/client/streaming#follow-a-subagent) on a client session attached to the parent. It reads `subagent.called.data.childStreamPath`: the child's own session stream for a local child, or a parent-origin proxy for a remote child, which the parent authenticates to the remote deployment.
