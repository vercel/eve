...
An [agent workspace](/docs/guides/deployment/vercel#multi-agent-workspaces) can expose other workspace members as remote subagents. Add a file under the calling member's `agent/subagents/` directory and select members by workspace-relative path:

```ts title="agents/foreman/agent/subagents/tasks.ts"
import { defineWorkspaceSubagents, vercelWorkspaceTarget } from "eve";

export default defineWorkspaceSubagents({
  include: ["agents/tasks/*"],
  exclude: ["agents/tasks/experimental"],
  resolveTarget: vercelWorkspaceTarget(),
});
```

`include` and `exclude` use glob patterns. When `include` is omitted, eve includes every other workspace member; `exclude` wins when both match. Every authored pattern must match a workspace member, and a member never selects itself. The generated tools use member names and root-agent descriptions. When a member has no description, eve generates a name-based fallback.

`vercelWorkspaceTarget()` routes requests through the member routes generated by an eve Vercel workspace deployment and authenticates them with the ambient Vercel OIDC token. The receiving member still controls access through its channel authentication policy. Workspace-aware local routing is not yet available: `eve dev` can start an agent that uses this helper, but calling one of its workspace subagents locally returns the ordinary unreachable-agent failure.

For a self-hosted workspace, provide a resolver that reads routing and credentials at runtime:
...
export default defineWorkspaceSubagents({
  ...
  resolveTarget: (member) => ({
    url: process.env[`EVE_${member.name.toUpperCase()}_URL`]!,
    auth: bearer(() => process.env.INTERNAL_AGENT_TOKEN!),
  }),
});
```

The resolver receives `{ name, path, description? }`. It may return static values or function-valued URLs and headers, so secrets do not need to be serialized into compiler artifacts. Set `forwardPrincipal: true` only when the receiving members trust this caller to forward user identity, as described in [Remote agents](./guides/remote-agents#forward-the-callers-identity).

Multiple workspace-subagent files are allowed. Their selected members share the ordinary subagent tool namespace, so duplicate member selections and collisions with authored tools fail rather than choosing one definition.
