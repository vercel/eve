...
For each model-visible agent lifecycle event, eve appends an independent framework-authored `<agent>` message to the conversation. A `created` message introduces its stable `id`; `working` and `input_required` identify an agent that cannot accept a continuation; `available` identifies a resumable agent and may carry its latest status; and `unavailable` removes it from the resumable set. For each `id`, the latest message is authoritative:

```xml
<agent status="created" name="researcher" id="ag_researcher:abcdef123456"/>
<agent status="available" name="researcher" id="ag_researcher:abcdef123456">initial findings</agent>
```

The messages are injected by eve, not written by the user, and never require a reply. Between context compactions, messages remain append-only to preserve the existing prompt-cache prefix; unchanged agents are not repeated. If compaction removes an agent's announcement, eve restores its current status before the next model call.
