...
If a delegated child starts its own background task, an acknowledgement from that child remains visible on the child stream but does not complete the parent's delegated invocation. The invocation stays pending until the child receives the nested task's terminal notification and answers with the available result. Failures, cancellation, and session termination still end the invocation without waiting for a successful nested result.
