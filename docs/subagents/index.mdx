...
Child sessions can still call their own declared or remote subagents, but they receive neither `code_mode` nor the built-in `agent`. When the root enables [code mode](/docs/tools/code-mode), its JavaScript program can await subagent results, sequence dependent calls, or use `Promise.all` for concurrent calls. The program's budget of 100 subagent calls counts every invocation, including retries and continuations.
