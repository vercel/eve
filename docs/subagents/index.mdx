...
Tasks launched in the same parent turn form a cohort. eve holds their successful
completion notifications until every task in that cohort has completed, failed,
or been cancelled, then delivers the successful results together in one parent
turn. Partial completions do not invoke the parent model. No configuration or
debounce timer is required.

The cohort stays together across intervening user turns. User messages, input
requests, authorization events, failures, and cancellation are handled without
waiting for the cohort. Different cohorts complete independently. To follow work
in progress, subscribe to the child session streams.
