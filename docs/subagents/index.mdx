...
Set `model` to an array of AI Gateway model ids to let the caller pick one when it starts a child. The first entry is the default:
...
The subagent's tool then accepts an optional `model` field limited to those ids, and `ctx.agent("researcher", { message, model })` accepts the same field. The choice behaves like a `session.started` [dynamic model](./agent-config#choose-the-model-dynamically) selection: it applies to the new child for its whole lifetime, and compaction follows the chosen model's context window.

- Omitting `model` uses the first entry.
- An unlisted id, or `model` sent to a subagent without choices, is rejected as a tool error.
- `model` with the `agentId` of an existing child is rejected; the child keeps its model.
- Every entry must be a Gateway model id with catalog metadata. An array does not accept sibling `modelContextWindowTokens` or `modelOptions`.
- The array applies only to declared local subagents. A root agent uses its first entry, and dynamic subagent configs must return one static model.
