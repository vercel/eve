...
Child sessions can still call their own declared or remote subagents, but they receive neither the root-only `workflow` tool nor the built-in `agent`. On the root, enabling [dynamic workflows](./tools/dynamic-workflows) adds visible subagents to the model-authored program while keeping the same subagents available for direct background calls. Calls inside `workflow` await final child results instead of returning task receipts.
