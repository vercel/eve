...
Code mode does not absorb subagent calls. Child sessions can still call their own declared or remote subagents directly; they do not receive the root-only built-in `agent`.
