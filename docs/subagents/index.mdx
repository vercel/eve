...
Harness-backed agents can delegate to the built-in `agent` tool and static or active dynamic local and remote subagents. Their delegation calls use the same background-task receipts and completion delivery as model-backed agents. `task_cancel` is not yet available to harness-backed agents; it remains available to model-backed agents.

When a harness-backed root and its children share a Vercel sandbox, expose a separate port for each session that needs to run at the same time. eve reserves the first available exposed port for each session and uses the same port when that session resumes. Port reservations remain until the sandbox ends, so size the port list for every harness-backed session that will use the sandbox during its lifetime. If the ports run out, the next session fails with an error asking for another exposed port.
