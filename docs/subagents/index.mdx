...
Use the peer's directory name under `agents/`. eve requires an exact workspace-member match, so add another declared subagent file for each additional peer. Run `eve dev` from the workspace root to start every member and enable the default local workspace transport. Running `eve dev` from one member directory starts only that agent; use an explicit transport if it must call a separately running peer.
