...
The messages are injected by eve, not written by the user, and never require a reply. They remain append-only so each model request extends the provider's existing prompt-cache prefix; unchanged agents are not repeated.
