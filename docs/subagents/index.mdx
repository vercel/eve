...
You can also export `disableTool()` from `agent/tools/agent.ts`. Re-exporting the framework definition at that path takes priority over `tool: false`; arbitrary replacements are not supported. Both forms control model visibility only. A workflow in a top-level root session can still invoke the root copy with `ctx.agent("agent", ...)` and read its description from `ctx.agents.agent`.
