...
Each delegated agent call has a time limit of 2 hours of active time. Time the child spends waiting on a question or approval that reaches the root channel does not count. A call still working at the limit fails: the parent's tool call gets an error with code `TIMED_OUT`, `task.settled` reports `failed` with the same error, and eve cancels the child's current turn. The child stays available for another call. If a local child has not stopped 30 seconds after eve cancels it, whether for a timeout or a parent cancel, eve terminates the child's session, and a later call with its `agentId` fails with `UNKNOWN_AGENT`.
