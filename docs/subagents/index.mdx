...
The copy uses the root's instructions, connections, auth, and sandbox. It receives the same tools except for the root-only `agent`, and starts with fresh conversation history and fresh state. Its file writes are immediately visible to the root. To run independent tasks in parallel, emit multiple `agent` calls in one response; eve runs the batch concurrently and returns every result before the root continues. Give parallel children non-overlapping write scopes.
...
Code mode keeps ordinary foreground subagent calls direct. When both `experimental.codeMode` and `experimental.tasks` are enabled, generated programs may stage up to eight local or remote subagent background tasks and receive `{ taskId, status: "working" }` receipts. Eve dispatches the staged launches only after the program succeeds; failed, cancelled, or timed-out programs launch none of them. The program cannot await final child results. Results arrive in later turns, and active tasks remain manageable through the direct task tools. Child sessions can still call their own declared or remote subagents directly; they do not receive the root-only built-in `agent`.
