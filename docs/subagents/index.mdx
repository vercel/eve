...
`vercelWorkspaceTarget()` routes requests through the member routes generated by an eve Vercel workspace deployment and authenticates them with the ambient Vercel OIDC token. The receiving member still controls access through its channel authentication policy.
