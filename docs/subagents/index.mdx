...
The copy uses the root's instructions, connections, auth, and sandbox. It receives the same tools except for the root-only `agent` and `Workflow`, and starts with fresh conversation history and fresh state. Its file writes are immediately visible to the root. Without `experimental.tasks`, eve returns every child result before the root continues. With tasks enabled, each built-in `agent` call returns a task receipt and the child continues in the background. Give parallel children non-overlapping write scopes.
...
`description` is required. The parent reads it to decide whether to delegate, so the compiler rejects any subagent whose `agent.ts` leaves it out. `background` defaults to `false`, which waits for the child result. Set `background: true` to return a durable task receipt immediately and let the child continue in the background. Background subagents require `experimental.tasks: true` on the root agent.

Execution is selected independently for every declared local or remote subagent. The built-in root `agent` tool keeps its existing root-wide behavior: it runs in the background whenever the root enables tasks.

Task-enabled declared subagents also receive the framework `task_update` and `task_cancel` tools. A task-owned child uses `task_update` to report progress to its parent; a child that starts nested background work uses `task_cancel` to stop tasks in its own session.
...
The model-authored `Workflow` tool currently exposes only blocking subagents. Background subagents run through the model tool loop and are not available inside a `Workflow` program.
...
Cancelling a parent turn requests cancellation of every active blocking child it started. An admitted background task survives cancellation of the turn that started it; use `task_cancel` to stop the task. Parent-session finalization cancels remaining live tasks.
