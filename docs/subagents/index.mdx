...
Every declared local or remote subagent call waits for the child's answer, which becomes the tool result. Human input requests from the child surface on the parent session while the call waits.
