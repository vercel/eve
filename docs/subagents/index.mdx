...
eve automatically combines adjacent successful completions already queued from
tasks launched in the same parent turn. Every result is retained, and the parent
responds to the batch in one turn. No configuration is required.
...
The copy uses the root's instructions, connections, auth, and sandbox. It receives the same tools except for the root-only `agent` and `code_mode`, and starts with fresh conversation history and fresh state. Its file writes are immediately visible to the root. The built-in `agent` runs in the background when called directly and needs no configuration: each call returns `{ status: "working", taskId, agentId }`, then task notifications wake the parent with updates or the final result. Give parallel children non-overlapping write scopes.
...
`description` is required. The parent reads it to decide whether to delegate, so the compiler rejects any subagent whose `agent.ts` leaves it out. When called directly, every declared local or remote subagent runs as a durable background task: the call returns `{ status: "working", taskId, agentId }` immediately, then task notifications wake the parent with updates, completion, failure, or cancellation. Human input requests surface separately on the parent session.
...
Child sessions can still call their own declared or remote subagents, but they receive neither `code_mode` nor the built-in `agent`. When the root enables [`experimental.codeMode`](/docs/agent-config#other-defineagent-fields), its JavaScript program can await subagent results, sequence dependent calls, or use `Promise.all` for concurrent calls. The program's budget of 100 subagent calls counts every invocation, including retries and continuations.
