...
The copy uses the root's instructions, connections, auth, and sandbox. It receives the same tools except for the root-only `agent` and `Workflow`, and starts with fresh conversation history and fresh state. Its file writes are immediately visible to the root. To run independent work in parallel, emit multiple `agent` calls in one response. Without `experimental.tasks`, eve returns every child result before the root continues. With tasks enabled, each call returns a task receipt and the children continue in the background. Give parallel children non-overlapping write scopes.
...
A declared subagent lives under `agent/subagents/<id>/` and uses `defineLocalSubagent`. Declare one when the child needs a clearly different prompt, role, tool surface, or execution policy.
...
`description` is required. The parent reads it to decide whether to delegate, so the compiler rejects any subagent whose `agent.ts` leaves it out. `background` defaults to `false`: the parent waits for the child and receives its result. Set `background: true` to return a task receipt immediately and let the child continue as a durable background task.

Use `defineRemoteSubagent` for a remote declaration with the same execution switch:

```ts title="agent/subagents/reviewer.ts"
import { defineRemoteSubagent } from "eve";

export default defineRemoteSubagent({
  background: true,
  description: "Review a proposal on the remote deployment.",
  url: () => process.env.REVIEWER_URL!,
});
```

Background subagents require `experimental.tasks: true` on the root agent:
...
Execution is selected independently for every declared local or remote subagent, so one parent can wait for a focused reviewer while starting a long-running researcher in the background. The built-in root `agent` tool keeps its existing behavior: it runs in the background whenever the root enables tasks.

Existing `defineAgent` and `defineRemoteAgent` declarations under `agent/subagents/` remain supported during migration. They keep the previous root-wide behavior: task-enabled roots run them in the background, and other roots wait for their result. New declarations should use the subagent-specific helpers and choose `background` explicitly.
...
`defineDynamic` from that subagent's `agent.ts`. Return a `defineLocalSubagent`
...
`defineDynamic`, rather than the `defineLocalSubagent` returned by an event handler:
...
`Workflow` currently exposes only blocking subagents. Background subagents run through eve's background-tool executor in the model tool loop and are not available inside a Workflow program.
...
Cancelling a parent turn also requests cancellation of every active blocking child it started, recursively through nested and remote subagents. Each affected session emits its own `turn.cancelled` → `session.waiting` boundary; the cancelled parent does not synthesize tool results or emit `subagent.completed`. A child that already completed or parked has no active turn and is a benign no-op. An admitted background task survives cancellation of the turn that started it; use `task_cancel` to stop that task. Parent-session finalization cancels remaining live tasks.
