...
To steer a running background child, call the same subagent tool with its `agentId` and the updated `message`. The child finishes its current Workflow step, then applies the update before its next model call in the same turn. The receipt retains the same `agentId` and `taskId`; steering does not cancel the task, replace its completion callback, or undo tool side effects that have already occurred.
