...
```ts
{
  ...
  agentId?: string;      // with persistent sessions or background tasks: continue an existing child
  outputSchema?: object; // require structured output for this invocation
}
```

The `agentId` field exists when the agent opts into [agent messaging](#agent-messaging) or [background tasks](/docs/tools/background-tasks); without either opt-in the schema is `{ message, outputSchema? }`.

The copy uses the root's instructions, connections, auth, and sandbox. It receives the same authored tools except for the root-only `agent` and `Workflow`, and starts with fresh conversation history and fresh state. Its file writes are immediately visible to the root. Under `experimental.tasks`, framework task controls also differ: the owning root receives `task_cancel`, while its task-owned copy receives `task_update`. To run independent tasks in parallel, emit multiple `agent` calls in one response; without `experimental.tasks`, eve returns every child result before the root continues. With [background tasks](/docs/tools/background-tasks), each call returns a working task receipt instead. Give parallel children non-overlapping write scopes.
...
eve lowers every subagent visible to the current agent (the root built-in copy, declared, or [remote](./guides/remote-agents)) into a model-visible tool with the same `{ message, outputSchema? }` shape — plus `agentId` when the agent opts into [agent messaging](#agent-messaging) or background tasks. The parent packs `message` with everything the child needs, since the child never sees the parent's history. `outputSchema` requires structured output for that invocation; it does not select execution mode. The value becomes the blocking tool result or, under `experimental.tasks`, later task completion data.
...
Each delegated subagent spins up its own child session and stream. The parent stream carries the control-plane events `subagent.called` and `subagent.completed`, plus interactive `input.requested`, `authorization.required`, and `authorization.completed` events proxied from descendants so the root channel can prompt the user. `subagent.completed` means the delegation tool call settled: in blocking mode that coincides with child output, while a `backgroundTask.status: "working"` payload records task admission before the child finishes. To follow the child's progress or terminal boundary, read `subagent.called.data.childSessionId` and subscribe at `GET /eve/v1/session/:childSessionId/stream`.

Without background tasks, cancelling a parent turn also requests cancellation of every active child it started, recursively through nested and remote subagents. Each affected session emits its own `turn.cancelled` → `session.waiting` boundary; the cancelled parent does not synthesize tool results or emit `subagent.completed`. A child that already completed or parked has no active turn and is a benign no-op. Under `experimental.tasks`, admitted tasks survive cancellation of the initiating turn and must complete, fail, or be cancelled through their own lifecycle.
...
Without this opt-in or `experimental.tasks`, delegated children run as one-shot tasks: they answer once and terminate, and the subagent tool schema carries no `agentId` field. Background task mode also uses persistent child sessions and exposes `agentId`, but reports each invocation through the task lifecycle described in [Background tasks](/docs/tools/background-tasks).
...
- [Workflow tool](./concepts/built-in-tools#workflow-tool): have the model orchestrate its subagents programmatically (fan-out, map-reduce).
