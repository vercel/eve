...
Each subagent call blocks the calling turn until the child replies, and the child's final result is
the tool result. If the child session expires, resets, or closes before replying, the call fails
and reports the usage accumulated since its previous settlement. This applies to root-agent copies,
declared local subagents, and remote subagents. The subagent session remains available for later
calls.
...
A declared local or remote subagent call waits for the child's reply and returns it as the tool result. Human input requests from the child surface on the parent session while the call waits. Cancelling the calling turn also cancels the child's current turn.
...
eve lowers every tool-enabled subagent visible to the current agent (the root built-in copy, declared, or [remote](./guides/remote-agents)) into a model-visible tool with the same `{ message, agentId? }` shape. The parent packs `message` with everything the child needs, since the child never sees the parent's history. To require structured output from a child, open a session with it from an authored workflow tool with [`ctx.agent(name)`](./tools/workflows#delegate-work-ctxagent) and pass `outputSchema` to `send()`; the schema applies to that turn, and the session accepts follow-up messages until the workflow run finishes.
...
Every invocation emits `subagent.called` after dispatch and `subagent.completed` after the parent records successful completion. Completion carries the actual output and identifies the same invocation by `callId`; the child session can remain available for reuse. Failure and cancellation do not emit `subagent.completed`; the failed tool result carries the error.

A session an authored workflow tool opens with `ctx.agent` is announced with `agent.started` instead, which carries the workflow tool's `callId`, the agent `name`, the child's `sessionId`, and a `streamPath` that `session.streamSubagent()` also accepts. Its questions and sign-ins reach the root session the same way.
...
A call that names a child that is still starting or running returns `AGENT_BUSY`. Delivery failures are reported to the caller; eve does not start a replacement child session to hide them.
