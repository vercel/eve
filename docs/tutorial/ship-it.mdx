...
Your auth lives in one module that turns a request into a user. Create `agent/lib/auth.ts` and wire your real provider (a cookie session, Better Auth, Auth.js, Clerk, etc.) in here. The stub below returns a fixed user so the page compiles and runs end to end:
