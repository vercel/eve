...
Now point the channel at it. Replace the contents of `agent/channels/eve.ts`, which Step 6 left with a dev-only `devTeam` entry and `placeholderAuth()`. List your app auth first, ahead of the catch-all helpers, so any entry that doesn't recognize the caller falls through to the next one:
...
const appAuth: AuthFn<Request> = async (request) => {
  ...
  if (!user) return null;
  return {
    attributes: { team: user.team }, // the claim Step 6's playbook reads
    principalType: "user",
    principalId: user.id,
    authenticator: "app",
...
That `team` attribute is exactly what the dynamic playbook in [Step 6](./team-playbooks) reads from `ctx.session.auth`. Identity is set in this one place and flows out to every capability from there.
...
That's the full assistant, deployed and authed. It queries the sample data, runs analysis in a sandbox, charts the results, remembers your team's definitions, loads the right playbook per team, and asks before it spends.
