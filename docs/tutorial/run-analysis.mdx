...
Hydrate the warehouse schema into the sandbox so the model isn't guessing at table shapes. The folder sandbox layout puts anything under `agent/sandbox/workspace/` into `/workspace` during template preparation.
...
This tool shells out to `python` with matplotlib, which the sandbox base image does not preinstall. Install the runtime in an exported sandbox template's `prepare()` callback (or bake it into a custom image) so `python plot.py` resolves. See [Sandbox](../sandbox) for the build-time template lifecycle.
...
The local provider runs the sandbox on your laptop during `eve dev`; on Vercel it runs on Vercel Sandbox. Lifecycle, providers, and network policy are in [Sandbox](../sandbox).
