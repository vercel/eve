...
- A model credential. The scaffold's default model goes through the [Vercel AI Gateway](../getting-started), so you need `AI_GATEWAY_API_KEY` (or `VERCEL_OIDC_TOKEN` pulled via `vercel link`). A model configured outside AI Gateway needs its provider's setup. For `anthropic("claude-opus-4-8")`, that is `@ai-sdk/anthropic` and `ANTHROPIC_API_KEY`.
