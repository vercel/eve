...
The `key` is an application-defined reuse identity. eve combines it with the prepared image and managed-resource generation, so changing the Dockerfile or managed resources opens different native compute. Environment options are fixed for every attached session; `open()` accepts no options.

Each eve session receives its own logical sandbox identity, but commands and files operate on the reused native Sandbox. Session `stop()` and `delete()` calls do not stop or delete the native Sandbox. Calling `setNetworkPolicy()` changes the shared native network boundary for every attached session.
