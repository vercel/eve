...
`ExperimentalVercelDockerfile` builds `agent/sandbox/Dockerfile` as a Linux AMD64 OCI image, publishes a digest-pinned image, and stores managed workspace and skill inputs in immutable Drive snapshots. The built-in `VercelSandbox` remains snapshot-backed; use this separate environment only when the sandbox must start directly from an authored image.
...
This environment does not accept `prepare`. Put immutable software setup in the Dockerfile. The build artifact records only the digest-pinned image reference and content-addressed Drive identities; it never records credentials. Runtime strictly resolves those resources and fails with rebuild or redeploy guidance when they are unavailable.
