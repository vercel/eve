...
For session creation and message delivery, use the [drive API](./cases#the-drive-api).
`t.session()` creates a session without a first message; call `session.send()` or `session.start()`
on the returned handle to use it. `t.send()` creates a new session with its first message.
These helpers handle the session protocol, retries, and event collection for the eval.
