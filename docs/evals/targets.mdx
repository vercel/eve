...
Local targets send no auth: `eve eval` owns the dev server it boots. For a remote `--url`, eve first asks Vercel to resolve that exact HTTPS origin under the account linked in `.vercel/project.json`. Ambient Vercel credentials are sent only when that lookup succeeds; an arbitrary URL remains anonymous.
...
`EVE_EVAL_AUTH_TOKEN` is an explicit bearer override for targets whose auth is not Vercel OIDC.
