...
- `t.expectInputRequests(filter?)` asserts the previous turn parked and returns a non-empty tuple. Filters match tool name, action input, prompt, display, option ids, and exact count.
...
Use `expectOk()` only when the next operation depends on that intermediate turn succeeding. A final `t.completed()` already records a complete-run gate.

To intentionally omit an eval for the current target, call `t.skip(reason)`. Skipped evals are reported separately and do not affect the exit code.
