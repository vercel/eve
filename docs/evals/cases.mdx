...
export default defineEval({
  async test(t) {
    await t.send({ message: "What is the weather in Brooklyn?" });
    t.succeeded();
...
export default defineEval({
  async test(t) {
    const first = await t.send({ message: "My favorite word is marigold." });

    const second = await t.send({ message: "Thanks for remembering." });
...
- `t.send({ message, ... })` sends a turn and waits for it to settle. It accepts the same input object as `ClientSession.send()` and resolves to a turn carrying `.message` and `.expectOk()`.
