...
- **Scoped methods** read the whole run when called on `t`, one session when called on an `EveEvalSession`, or one response when called on an `EveEvalTurn`. See [Assertions](./assertions).
...
Use `await t.require(value, assertion)` for a gate that must pass before the script can safely continue. Use `t.skip(reason)` for an intentionally unsupported target capability.
