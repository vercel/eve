...
A background subagent that has only returned a working task receipt remains `pending`; use `t.calledSubagent(name, { status: "pending" })` to assert that delegation. The receipt settles dispatch, not the child's work. A successful blocking call with an actual result is `completed`, even when the child session stays available for another call. Check task notifications for a background task's final outcome.
