...
A subagent call stays `working` until its result is observed, and becomes `completed` when its successful result is observed through `subagent.completed`, even when the child session stays available for another call. A rejected dispatch is `failed`; an explicit cancelled child outcome is `cancelled`.
