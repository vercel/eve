...
A background subagent that has only returned a working task receipt remains `pending`; use `t.calledSubagent(name, { status: "pending" })` to assert that delegation. The receipt settles dispatch, not the child's work. Both blocking and background calls become `completed` when their actual successful result is observed through `subagent.completed`, even when the child session stays available for another call. This eval status describes the captured events: an unresolved delegation is `pending`, while the underlying admitted task is `working` or `input_required`. Check task notifications for failure and cancellation.
