...
A subagent call is a task call: it stays `working` until its `task.settled` event arrives, then takes that event's `status` (`completed`, `failed`, or `cancelled`) and, when completed, its `output`. A completed call leaves the child session available for later calls with the same `taskId`.

The runner derives these values from captured `task.started`, `task.settled`, and `agent.started` events. A proxied input request alone does not change the subagent call's status; assert that request with `requireInputRequest`.
