...
`t.check(value, assertion)` grades an explicit value against a builder from `eve/evals/expect`. The value can be a turn’s `.message`, parsed JSON, or any local you computed:

```ts
import { includes, equals, matches, satisfies, similarity } from "eve/evals/expect";

t.check(turn.message, includes(/sunny/i)); // substring or RegExp (gate)
...
t.check(turn.message, similarity("Sunny, 72F")); // fuzzy 0–1 Levenshtein (soft)
t.check(
  latencyMs,
...
`t.calledTool` and `t.calledSubagent` take matcher objects. Tools accept `{ input, output, status, count }`; subagents accept `{ callId, childSessionId, remoteUrl, output, status, count }`. Calls match `status: "completed"` by default. Tool lifecycle checks also accept `"pending"`, `"failed"`, and `"rejected"`. Subagent checks accept `"working"`, `"completed"`, `"failed"`, and `"cancelled"`. A numeric `count` requires an exact number of calls matching every supplied constraint. Use a predicate for ranges or other custom count requirements; it receives the observed number of matching calls.

A subagent call is recorded from its `task.started` event with `kind: "agent"` and stays `working` until its `task.settled` event arrives. A workflow tool call is also a task, but it is recorded only as a tool call. The status then matches `task.settled`: `completed` when the call returns a successful result, even when the child session stays available for another call, `failed` when it returns an error, and `cancelled` when the parent cancels the call, including by cancelling the parent turn. `output` holds the result, or the `{ code, message }` error of a failed call. A model call to an agent tool that fails before its child starts has no `task.started`; the runner records it from the call and its `task.settled`, so `t.calledSubagent` matches it with `status: "failed"` and the error as `output`. A `ctx.agent` call from a workflow body that fails before its child starts is not recorded as a subagent call.

The runner derives these values from captured events. A proxied input request does not change the subagent call's status; assert that request with `requireInputRequest`.
...
Beyond the raw `session.events` stream, the runner derives typed facts the assertions read: tool calls (name, input, output, lifecycle status), subagent calls, and HITL input requests. A turn that leaves the session open for a next message is the normal end state of a successful turn; parking on unanswered HITL input is tracked separately.
...
turn.eventOrder([
  { type: "task.started", data: { name: "researcher" }, count: 2 },
  { type: "task.settled", data: { status: "completed" }, count: 2 },
]);
```
...
t.check(turn.message, similarity("Sunny")).label("forecast wording").atLeast(0.8);
t.check(turn.message, includes("error")).soft(); // track without failing the build
```
