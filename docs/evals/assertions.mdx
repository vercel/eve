...
For subagents, `status: "completed"` (the default) requires a successful invocation result. Use `status: "admitted"` to assert a background task receipt; admission does not prove that the task finished. Check the task's terminal outcome separately. Neither status implies that the persistent child session has ended.
