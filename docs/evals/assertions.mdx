...
A subagent call stays `working` until its result is observed. It becomes `completed` when its successful result is observed through its tool result or `subagent.completed`, even when the child session stays available for another call. A rejected dispatch or failed call is `failed`; an explicit cancelled child outcome is `cancelled`.

The runner derives these values from captured events. A proxied input request alone does not change the subagent call's status; assert that request with `requireInputRequest`.
