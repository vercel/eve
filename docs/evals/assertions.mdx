...
A subagent call stays `working` until its result is observed. It becomes `completed` when the call returns a successful result, even when the child session stays available for another call, and `failed` when the call returns an error or its dispatch is rejected. An explicit cancelled child outcome is `cancelled`; a call interrupted by cancelling the parent turn stays `working`.
