...
Install a compatible `dd-trace` peer dependency (`^6.12.0`) and set both `DD_API_KEY` and `DD_APP_KEY`. By default, the project name comes from the first eval id, and eve generates the experiment name. Pass `projectName`, `experimentName`, `description`, `dataset`, `experimentConfig`, `metadata`, or `tags` when you need to override the external experiment. Without a `dataset.id`, Datadog creates a placeholder dataset for the experiment.

The reporter does not require runtime tracing. It reuses the result metadata produced for Braintrust, including the session id, status, verdict, tool and subagent calls, and failed assertions. When the eval runner observes runtime traces, the row also includes `eveTraceIds` and `eveTraceContexts`.
...
Datadog's external experiment API creates a new span identity in `submitSpan()`. The captured trace contexts remain metadata on the experiment row; the row is not the existing runtime span. A native association requires a Datadog API that accepts the runtime `traceId` and `spanId` when starting or attaching an experiment case.
