---
title: "Reporters"
description: "Ship eval results to Datadog or Braintrust experiments, or write JUnit XML. eve runs and scores everything itself."
---
...
`Datadog(...)` creates one external Datadog experiment for the run, submits one row per eval, and attaches each assertion as an evaluation metric:
...
Install `dd-trace` 6.12 or newer and set both `DD_API_KEY` and `DD_APP_KEY`. `projectName`, `experimentName`, `description`, `dataset`, `experimentConfig`, `metadata`, and `tags` configure the external experiment. Without an existing `dataset.id`, Datadog creates the placeholder dataset required by its external experiment API.

The reporter does not require runtime tracing. Every experiment row carries `eve.eval.run_id`, the generated case-execution id under `eve.eval.id`, the path-derived case name under `eve.eval.name`, and, when a session started, `eve.session.id`. If the target emits traces, eve also puts the run, case, and session identifiers on its runtime spans, propagates them through follow-up turns and subagents, and records the observed trace contexts using the same `eveTraceIds` and `eveTraceContexts` metadata convention as the Braintrust reporter.

For example, an agent can export its runtime spans through OpenTelemetry independently of the reporter:
...
Datadog's current external experiment API still creates a new span identity in `submitSpan()`. The shared eve identifiers and captured trace contexts make the experiment row and runtime trace queryable together, but the row is not yet the existing runtime span. A native Datadog association requires an API that accepts the runtime `traceId` and `spanId` when starting or attaching an experiment case.
...
`onRunStart` fires once before any eval runs and includes the generated `runId`; `onRunComplete` fires once with the aggregated summary and the same id. Within that run:

- `onEvalStart` fires when an eval is scheduled. It includes the eval definition, generated `caseId`, run id, target, and start time.
...
- `onEvalComplete` fires with the checks, scores, verdict, `caseId`, and run id. The runner also supplies `context`, including every distinct trace context collected across the eval's sessions. The same list is available as `result.result.traceContexts`.
