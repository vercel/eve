...
For an application with multiple tenants, follow the
[Multi-tenant memory](/docs/patterns/multi-tenant-memory) guide to scope memory
by authenticated tenant and caller.
