...
When a subagent requests input, eve emits the same `input.requested` event on its parent session, with the `taskId` of the call that asked. Answering through that parent session routes the response directly to the blocked child without invoking the parent model. This works the same for a local subagent and a [remote agent](/docs/guides/remote-agents#questions-approvals-and-sign-in): a remote child's request reaches the parent over the parent's callback URL, and eve forwards your answer to the remote deployment. A child inherits the parent session's capabilities, so it can ask only when the parent session can request input.
