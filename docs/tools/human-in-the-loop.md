...
For `ctx.ask()` questions from workflow tools and questions from subagents, a follow-up message from the principal of the turn that asked answers the question only when exactly one question is pending and the session has no approval or session-limit prompt of its own pending. The message must match an option, or the question must allow free text. Otherwise the message joins that turn as a steering message, and each pending question this session's workflow tools created with `dismissible: true` resolves as `dismissed`. A message from another principal, or one sent with `turnPolicy: "queue"`, waits until the turn ends and then starts a turn of its own; only then can its text answer or dismiss a question. Structured `inputResponses` reach their requests at once, whoever sends them.

A follow-up message that answers nothing never stops a detached task, such as a subagent waiting on an approval or a workflow tool without `attached: true` whose question is not dismissible. Their requests stay pending and answerable, and each task's result arrives later in the same turn once it finishes. An attached call waiting on a request that is not dismissible is cancelled instead. See [Steering during a turn](/docs/concepts/tasks#steering-during-a-turn).
...
When a subagent requests input, eve emits the same `input.requested` event on its parent session, with the `taskId` of the call that asked. Answering through that parent session routes the response directly to the blocked child without invoking the parent model. Once the request is resolved, its `input.resolved` appears on the parent session too. This works the same for a local subagent and a [remote agent](/docs/guides/remote-agents#questions-approvals-and-sign-in): a remote child's request reaches the parent over the parent's callback URL, and eve forwards your answer to the remote deployment. A child inherits the parent session's capabilities, so it can ask only when the parent session can request input. Anyone who can send to the parent session can answer; the child attributes the answer to that principal, the `responder` your response policy checks, and keeps acting as the principal that started it.
