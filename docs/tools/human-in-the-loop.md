...
When the decision depends on the input, pass your own policy instead of a helper. It receives the same session context as tool execution, plus `{ toolName, toolInput, approvedTools, callId }`, and returns an AI SDK 7 approval status synchronously or as a promise. `callId` identifies the tool call being evaluated — it matches the `callId` on the call's `actions.requested` and `action.result` stream events and the `ctx.callId` that `execute` receives if the call is approved, so approval-gated tools can key domain records to one identity across proposal, rejection, and execution. Use `ctx.session.auth.current` to guard by the caller of the current turn and `ctx.session.auth.initiator` to guard by the caller that created the session. Return `"user-approval"` to pause for a person or `"not-applicable"` to continue without a prompt. `toolInput` can be undefined, so guard the access. This policy denies cross-tenant calls, then requires approval only when an amount crosses a threshold:
