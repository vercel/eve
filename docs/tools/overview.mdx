...
- `ctx.abortSignal`: aborts when the active turn is cancelled. Pass it to fetches, SDK calls, and other cancellation-aware work the tool starts so cancelling the turn stops that work too. Sandbox sessions from `ctx.getSandbox()` are already bound to this signal — every `run`, read, and write is cancellable without passing it explicitly. Cancellation is best effort: a tool that ignores the signal runs to completion, but its result is discarded.
