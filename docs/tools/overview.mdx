...
export default defineTool({
  ...
  activity: {
    label: ({ project }) => `Build report for ${project}`,
    update: (partial) => partial.phase,
    result: (output) => `Report ready with ${output.report.sections.length} sections`,
  },
...
Use `activity.update(partial)` to project preliminary snapshots into
user-facing activity, and `activity.result(output)` to describe successful
settlement differently. Both callbacks receive the typed value yielded by
`execute`. Each non-empty update replaces the previous activity label; the
...
label. `activity.result` does not run for failed or rejected tool calls. The
