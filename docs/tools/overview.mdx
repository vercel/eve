...
cohort and, by default, instructs the model to acknowledge that the work started without waiting for results.
Use `taskDeliveryPolicy: "auto-silent"` or `"cohort-silent"` when an activity card
already shows the launch. See [Background task results](/docs/channels/overview#background-task-results).
