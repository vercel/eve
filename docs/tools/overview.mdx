...
returns a `working` receipt immediately while an external executor continues the task. Background
tools are a normal execution mode and need no root-agent flag.

Built-in, declared local, and remote subagents use this background task lifecycle automatically.
After delegating, an
