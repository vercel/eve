...
tool result before it continues. The one exception is a workflow tool defined with `detach: true`:
its call resolves at once with a receipt, and the workflow's result arrives later in its own message.
See [Return a receipt with `detach`](/docs/tools/workflows#return-a-receipt-with-detach).
...
message that arrives while a tool call waits follows the channel's turn policy: `steer` (the
default) is applied after the wait ends, in the same turn, and `queue` waits until the turn settles.
