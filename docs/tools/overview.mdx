...
The `TaskExec` argument and `task.postMessage` have been removed. Yield values for stream-only
progress, and return the terminal result. In a workflow body, keep values needed after a wait in local variables; workflow replay
...
| `defineWorkflowTool` with `execution: "background"` | Stream-only task progress; does not request a parent-agent turn.                        | Explicit return value becomes the task output; no return completes with `null`. The last yield is not a fallback. |
