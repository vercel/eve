...
When the model should call a named, typed action that is fulfilled by your UI instead of app-runtime code, use `defineClientTool` from `eve/tools`. Client-side tools expose the same model-facing `description` and `inputSchema`, but replace `execute` with `inputRequest` metadata. The call becomes an `input.requested` pause and resumes with a normal JSON tool result. See [Human-in-the-loop](./human-in-the-loop#client-side-tools) for an example.
