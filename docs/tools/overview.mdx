...
- `ctx.callId`: the id of the current tool call. It matches the `callId` on the call's `actions.requested` and `action.result` [stream events](/docs/concepts/sessions-runs-and-streaming) and on the approval policy's context, so a tool can derive record identities that stay stable across proposal, approval, and execution.
