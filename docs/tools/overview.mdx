...
This contract currently supports framework-owned task executors. A task-enabled root runs the
built-in `agent` in the background; declared local and remote subagents opt in with their own
`background` field. After delegating, an in-process executor reports
