...
Enable `experimental.tasks` to run authored tools or subagent calls as durable background work.
...
returns a `working` receipt immediately while an external executor continues the task. eve rejects
background tools at build time when the root agent has not enabled the experiment.
...
See [Background tasks](/docs/tools/background-tasks) for activation, lifecycle, cancellation, human input,
and the current authored-tool boundary.
