...
yield task.setState({ jobId, status: "running" }); // durable model-visible state; no wake
yield task.postMessage("The export needs review."); // one new parent turn; state unchanged
```

`setState` replaces the task's prior authored state. `postMessage` is the only yield that wakes the
agent; use [`ask`](/docs/tools/workflows#ask-a-human-ask) when the body needs an answer from the human.
