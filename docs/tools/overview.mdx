...
Client applications observe the same lifecycle through durable `task.updated` events on the
parent session stream. The default frontend reducer exposes those views as `data.tasks`, including
explicit `task_update` activity and terminal output or token usage when available.
