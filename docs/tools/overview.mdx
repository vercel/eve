...
`defineTool({ execution: "background" })` receives a third `task` argument and returns
`{ status: "working", taskId }` to the model. Add `"use workflow"` to let the body outlive the
initiating step; without it, the step waits for the body to finish. Return completes the durable task; throw fails it. An
async generator may additionally yield:

```ts
yield { progress: 0.5 }; // stream-only progress; does not wake the agent
yield task.postMessage("The export needs review."); // one new parent turn
```
...
`task.delegated()` has been removed. Move external work into a `"use workflow"` body and
return its result when it finishes. Rebuild extensions using the removed API after migrating.

`postMessage` is the only yield that wakes the parent agent. Keep values needed by the body
in local variables; workflow replay reconstructs them across durable waits.
Use [`ask`](/docs/tools/workflows#ask-a-human-ask) when the body needs an answer from the human.
