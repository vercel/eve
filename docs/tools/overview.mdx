...
the result in this mode; yield the final output. Workflow generators have different result
rules, described under [yield and return](#yield-and-return).
...
### Tool calls finish inside the turn

Every tool call resolves inside the turn that made it. A `defineTool` call runs inside the model
step, and the model sees its output as the tool result before it continues. A workflow tool call
starts a [task](/docs/concepts/tasks): by default it resolves at once with a receipt, the workflow
keeps working, and its result reaches the model later in the same turn, through `task_wait` or a
`task.result` message, because no turn ends while its tasks are working. A workflow tool defined
with [`attached: true`](/docs/tools/workflows#hold-the-turn-with-attached) holds the turn instead,
and its output is the tool result.
...
| `defineWorkflowTool` | Yes; the task keeps working while it suspends.    | A receipt at once, then the workflow output later in the same turn; with `attached: true`, the workflow output as the tool result. |

Several tool calls in one model step run concurrently, and the turn waits for all of them. A
message that arrives while a tool call runs follows the channel's turn policy. With `steer` (the
default), a message from the turn's principal is applied at the next step boundary, in the same
turn. It ends an attached workflow tool call and any `task_wait`, but never stops a detached task.
With `queue`, or from another principal, the message waits until the turn settles. See
...
meaning depends on the executor:

| Executor             | Ordinary `yield value`                                                                  | Final output                                                                                                |
| -------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `defineTool`         | Earlier values are `action.partial` snapshots; the final yield becomes the tool result. | Last yielded value; a generator `return` value is ignored.                                                  |
| `defineWorkflowTool` | Every yield is an `action.partial` snapshot.                                            | Explicit return value; falls back to the last yield if the return is `null` or `undefined`, then to `null`. |

Progress snapshots do not enter model history as intermediate tool results. The final output
selected by the rules above becomes the tool result.
