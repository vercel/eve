...
`defineTool({ execution: "background" })` receives a third `task` argument and immediately returns
`{ status: "working", taskId }` to the model. Return completes the durable task; throw fails it. An
