...
A tool that produces an image — a screenshot, a rendered chart — can hand the pixels to a vision-capable model by returning `{ type: "content", value }` from `toModelOutput`. Build the parts with the `toolOutputPart` helpers from `eve/tools`:
