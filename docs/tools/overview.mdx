...
- `ctx.callId`: the id of the current tool call — the same `callId` carried by the call's [stream events](/docs/concepts/sessions-runs-and-streaming) and approval context.
