...
Use `activity.label(input)` to describe a call in user-facing activity. The callback receives the validated tool input, so it can select the useful fields without exposing the full argument object:
...
export default defineTool({
  ...
  activity: {
    label: ({ project, environment }) => `Deploy ${project} to ${environment}`,
  },
  async execute(input) {
    return deploy(input);
  },
...
The label is presentation only. It does not replace the path-derived tool name, enter model input, change execution, or persist in the public action event. Built-in channel activity can show `Deploy storefront to production` instead of `deploy`. eve normalizes and bounds the returned string; if the callback throws or returns an empty label, activity falls back to the tool name.
