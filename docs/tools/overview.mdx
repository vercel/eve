...
Use `{ type: "content", value }` when the model needs to inspect a file returned by a tool. For example, a screenshot can be attached as an image alongside a short instruction:
...
Convert file data to a base64 string before returning it from `execute`. Eve checks the content-part structure and base64 syntax, but does not decode or inspect the file contents. File URLs, data URLs, provider file references, raw byte arrays, and sandbox references are not supported in `toModelOutput`.
