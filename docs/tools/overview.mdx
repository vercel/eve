...
Optional input fields can be omitted. eve disables implicit provider strict-mode normalization to preserve that distinction and validates tool inputs against your schema before execution. Include rules such as mutually exclusive arguments in the tool description as well as the schema, since custom validation rules may not appear in the model's JSON Schema.
...
Set `experimental.codeMode: true` in `agent.ts` to add the framework `code_mode` tool. The model
writes a JavaScript program that calls eligible tools and subagents, and eve runs it as a durable
workflow with one step per nested call. See [Code mode](/docs/tools/code-mode) for which tools a
program can call, how discovery and authorization work inside it, and its current limits.
