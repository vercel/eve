...
Every tool call resolves inside the turn that made it. The model sees the tool's output as the
tool result before it continues. A workflow tool defined with `detach: true` is the exception: its
call resolves at once with a receipt, and the workflow's result arrives later in its own message.
See [Return a receipt with `detach`](/docs/tools/workflows#return-a-receipt-with-detach). In an
interactive root session, a new message can also move a waiting workflow tool or subagent call to
the background, which resolves it with a receipt; see
[Detach a waited call](/docs/concepts/tasks#detach-a-waited-call). [Tasks](/docs/concepts/tasks)
describes which calls wait and how background results arrive.
...
message that arrives while a tool call waits follows the channel's turn policy. With `steer` (the
default), the message is applied after the wait ends, in the same turn. In an interactive root
session, waiting workflow tool and subagent calls move to the background instead, so the turn
answers the message right away. With `queue`, the message waits until the turn settles.
