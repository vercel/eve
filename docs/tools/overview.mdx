...
Add `agent/tools/workflow.ts` to expose the model-facing `workflow` tool on the root agent:

```ts title="agent/tools/workflow.ts"
export { default } from "eve/tools/workflow";
```

The tool runs one model-authored JavaScript program that can coordinate visible subagents, but it cannot call ordinary tools. Calls inside the program await final child results, while the same subagents remain available as direct background tools outside the program. See [Dynamic workflows](/docs/tools/dynamic-workflows) for custom budgets and execution semantics.

### Workflows as tools

Use `defineWorkflowTool` from `eve/tools` to author a named tool whose implementation runs as a durable Workflow SDK run. Its executor must start with `"use workflow"` and receives `ctx.agent` and `ctx.ask`. The authored tool can wait for a person, webhook, or timer without holding compute, then return its result to the model. This API is separate from the model-facing `workflow` tool exported by `eve/tools/workflow`. See [Workflows as tools](/docs/tools/workflows) for the authoring rules and examples.
