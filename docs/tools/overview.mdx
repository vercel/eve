...
subagents when `experimental.tasks` is enabled. The external executor wire is not yet a stable
authored-tool API; do not use background execution for application tools that need to report later
lifecycle updates.
