...
eve validates the model's input against `inputSchema` before `execute` runs:

- A Zod or Standard Schema validates with its own library, so tools work with whichever Zod version your app installs.
- A plain JSON Schema object reaches the model as written, without its `$schema` keyword. eve validates input against it with JSON Schema 2020-12 rules, which also cover draft-07 keywords, then fills omitted properties from their `default` values.
- eve treats `format` as an annotation and does not reject input for it.
- When eve cannot evaluate a JSON Schema locally, for example because of an unknown `type` name or a `$ref` it cannot resolve, the model still receives the schema and eve passes the input to `execute` unvalidated.

Optional input fields remain optional for authored tools and connection tools. eve explicitly disables provider strict mode for function tools so providers such as OpenAI Responses do not implicitly require every property. Local input validation still follows the tool's schema.

### Write descriptions for the model

The model receives every advertised tool's `description` and input schema on each step, and uses them to choose a tool and fill in its arguments. Write them as instructions for the model rather than as API reference:

- Say when to use the tool. When two tools overlap, name the one to use instead.
- For an input the model cannot invent, such as an ID, say where to get it: "Order ID from a `list_orders` result."
- State defaults and limits in the field description, such as "Defaults to 20, max 100."
- Keep examples accurate, because the model tends to copy them literally.
- Throw errors that say what went wrong and what to do next. The model receives the message as the tool error and can correct its next call.
- Add a tool only when the agent needs it. Each advertised tool is one more option on every step, so an optional input on an existing tool is often clearer to the model than a second, similar tool.

```ts title="agent/tools/cancel_order.ts"
import { defineTool } from "eve/tools";
import { z } from "zod";

export default defineTool({
  description:
    "Cancel an order that has not shipped. For a shipped order, use refund_order instead.",
  inputSchema: z.object({
    orderId: z.string().describe("Order ID from a list_orders or get_order result."),
    reason: z
      .string()
      .max(200)
      .optional()
      .describe("Reason shown to the customer, up to 200 characters. Defaults to none."),
  }),
  async execute({ orderId, reason }) {
    const order = await getOrder(orderId);
    if (order.status === "shipped") {
      throw new Error(`Order ${orderId} has shipped and cannot be cancelled. Use refund_order.`);
    }
...
### Hide a tool from delegated agents

Set `availableInSubagents: false` when a tool should be callable only in a top-level root session:

```ts title="agent/tools/coordinator.ts"
export default defineTool({
  availableInSubagents: false,
  description: "Coordinate work that only the top-level root agent should start.",
...
The option defaults to `true` and works with `defineTool` and `defineWorkflowTool`. A root copy does not receive a tool with this option, and a tool authored inside a declared subagent is unavailable because that agent runs in a delegated session. This controls tool availability only; it does not add or remove entries from a workflow's `ctx.agents` registry.
...
Background execution controls how a workflow tool delivers its result to the parent agent.
Set `execution: "background"` on `defineWorkflowTool`; ordinary `defineTool` calls always finish
inside the initiating step.
...
| `defineWorkflowTool` + `execution: "background"` | Yes; the workflow body can outlive the initiating step.  | Task receipt before the body finishes; one combined report after all tasks in its cohort are terminal. |

A background workflow tool's result is the receipt `{ status: "working", taskId }`; its eventual
output belongs to the task. Use this mode when the conversation should continue while the body
waits for a person, webhook, or timer.
...
The `TaskExec` argument and `task.postMessage` have been removed. Return the terminal result;
background yields are consumed without publishing progress or retaining a task-progress stream. In a workflow body, keep values needed after a wait in local variables; workflow replay
...
Channel sessions default to `taskDeliveryPolicy: "auto"`: ready results can invoke
the parent while other tasks continue. The parent reports independently useful
results and can stay silent until related work settles. Each reporting context
includes the whole cohort and available outputs, including previously withheld
results. Ready results may share a turn.
Schedules default to `"cohort"`: terminal outcomes wait for overlapping tasks
to settle, then arrive together. Pass `taskDeliveryPolicy` to `send(...)` to select
or update the session policy. See
[Background task results](/docs/channels/overview#background-task-results) and
[Schedules](/docs/schedules#background-task-results).
User questions, input requests, and explicit task messages do not wait for the
cohort under either policy. Failure and cancellation update lifecycle state
immediately; their automatic reports follow the selected policy.
See [Completion batching](/docs/subagents#completion-batching).
...
| `defineWorkflowTool` with `execution: "background"` | Consumed without publishing progress or requesting a parent-agent turn.                 | Explicit return value becomes the task output; no return completes with `null`. The last yield is not a fallback. |

For a background workflow tool, return completes the task and throw fails it. The automatic
cohort report is separate from the original tool call's receipt.
...
executor, the terminal cohort report provides input to a later parent-agent turn.
...
export default defineTool({
  ...
  approval: always(), // or auto() / once() / never() / a policy
  async execute(input) {
    return refund(input);
  },
});
...
Approval is one half of eve's [human-in-the-loop](./human-in-the-loop) model — the page covers the `auto`/`always`/`once`/`never` helpers, input-dependent policies, and how a gated call pauses and resumes durably.
