...
Convert file data to a base64 string before returning it from `execute`. Eve checks the content-part structure but does not validate the base64 encoding. File URLs, provider file references, raw byte arrays, and sandbox references are not supported in `toModelOutput`.
