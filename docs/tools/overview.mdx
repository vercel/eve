...
| `defineWorkflowTool` | Yes; the tool call waits while the workflow can suspend. | Workflow output after it finishes, or an earlier `ctx.reply()`. |
...
`execute(ctx)` must start with `"use workflow"`, reads the call's input from `await ctx.receive()`,
and can use `ctx.agent` and `ctx.ask`. The tool can
