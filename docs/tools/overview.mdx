...
`defineWorkflowTool({ execution: "background" })` receives a third `task` argument and returns
`{ status: "working", taskId }` to the model. Its durable body can outlive the initiating step.
With ordinary `defineTool({ execution: "background" })`, the initiating step waits for the executor to finish.
Return completes the durable task; throw fails it. An async generator may additionally yield:
...
executor receives `ctx.agent` and `ctx.ask`; no `"use workflow"` directive is needed. The tool can
