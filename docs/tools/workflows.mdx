...
- In the body, `ctx` has `session`, `history`, `callId`, `toolName`, `abortSignal`, `agent`, and `ask`.
  `history` is an immutable copy of the completed conversation prefix captured before this tool call, so it excludes the calling tool exchange and does not provide live session access. A waiting workflow can use `appendHistory({ operationId, messages })` to append validated, non-system messages to its owning session before returning. Retries reuse `operationId`; the same messages acknowledge the existing append, while different messages fail. Background workflow tools cannot append parent history; collect their results and let a waiting owner perform the append. `getToken` and `requireAuth` work inside a `"use step"` helper that receives `ctx` as a direct argument;
