...
Pass a declared target name or a registered `{ id: agentId }` handle. Workflow
calls wait for the final result; ordinary tools return a background receipt.
[Register destinations](/docs/subagents#register-destinations-at-runtime) in hooks
or ordinary tools before calling the workflow. Registry mutations are unavailable
