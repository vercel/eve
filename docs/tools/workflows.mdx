...
`"use workflow"` as the first statement of its executor. Each call starts a durable Workflow SDK run. Use one when a tool must wait for a person, webhook, or timer,
delegate work to subagents, or coordinate retryable steps over a long period. This authored API is distinct from the built-in, model-facing [`workflow` tool](./dynamic-workflows), which is enabled with `experimental.dynamicWorkflows`.
