...
on `ctx.ask` or an approval does not count toward the limit. A `ctx.ask` request that a
`Promise.race` moved past still waits, so the clock stays stopped until the request is answered or
the run ends. `timeout: false` is the same as leaving it unset.
