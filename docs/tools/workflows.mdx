...
delegate work to subagents, or coordinate retryable steps over a long period. This authored API is distinct from the model-facing [`workflow` tool](./dynamic-workflows), which you add through `agent/tools/workflow.ts`.
