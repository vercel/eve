...
If a workflow report is redelivered after a step retry, its background task processes it only
once. Separate yields remain separate reports, even when their contents are identical.
