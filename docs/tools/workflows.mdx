...
ordinary value for stream-only progress, `task.setState(...)` for durable task state, or
`task.postMessage(...)` to wake the parent agent with one message.
...
| task state               | not applicable                      | `yield task.setState(...)`                  |
