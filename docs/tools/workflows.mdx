...
#### Reuse the routing step

Use `auto()` from `eve/tools/agent-router` when you want the standard selection step but still need to choose the candidate map, model, or instructions:
...
#### Use the provided router tool

Use `agentRouter()` when you do not need to customize the candidate map. It routes across every described target in `ctx.agents`. To replace the model-facing `agent` tool, export it from the `agent` slot:
