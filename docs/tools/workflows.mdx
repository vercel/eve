...
export default workflow({
  agents: ["ticket-triage", "ticket-review"],
  maxSubagents: 20,
});
```

The tool description lists the allowed agents automatically. The model supplies an async function
body through the tool's `js` input. Its only host capability is
...
`agents` is a required trusted allowlist of `1` through `128` unique names. `maxSubagents` defaults
to `100` and must be an integer from `1` through `128`. The generated program cannot access the workflow context, session state,
...
an agent catalog. Configure every callable agent in the required trusted `agents` option; the tool
description is generated from that same allowlist.
