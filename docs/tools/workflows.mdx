...
the workflow holds no compute. A tool defined with `detach: true` returns a receipt at once and
delivers its result later in its own message; see
...
starts with `mode: "background"` on `task.started`. Without `detach: true`, the call waits.
...
A session holds at most 10 working background tasks. A `detach: true` call over that limit does not
start and fails with `TOO_MANY_BACKGROUND_TASKS`. Yields from a background run still publish
`action.partial` for the originating call and never wake the model. Cancelling a turn does not
cancel background tasks it started, and when the session ends, eve cancels every working task and
delivers nothing afterward.
...
result, a `detach: true` call delivers it as a failed task result, and `task.settled` reports
`failed` with the same error. eve then cancels the run, which unwinds as described in
[Cancel and clean up](#cancel-and-clean-up-ctxabortsignal). Time the run spends waiting on
`ctx.ask` or an approval does not count toward the limit. `timeout: false` is the same as leaving it
unset.
...
or a cancellation. A `detach: true` call resolves once with its receipt, and its task delivers the
result once.
...
`"foreground"`), and no `child`. The call's first outcome emits one `task.settled`, and a run that
fails to start settles with `error.code` `START_FAILED`. Progress
...
until the turn settles. A message with `turnPolicy: "steer"` (the default) is applied after the tool
call resolves, in the same turn, so the model sees it before continuing. Steering does not cancel
the run. Input responses never steer. Cancelling the turn cancels the waited run, which withdraws
its requests.
