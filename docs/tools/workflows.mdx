...
- `"use workflow"` as the first statement of `execute` makes it a durable body. eve hoists the
  method into a workflow function named after the tool. You can also write the body as a top-level
...
- `ctx` inside the body carries `session`, `callId`, `toolName`, `replyTo`, and `abortSignal`. The
...
- `ask` and `yield` reach the session over `ctx.replyTo`, the owner's hook. `ask` is a small helper
  over `createHook` and that hook; you can write your own or send other messages yourself.
