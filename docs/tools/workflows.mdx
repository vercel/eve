...
A workflow tool is a static tool defined with `defineWorkflowTool` from `eve/tools`. Each call
starts a durable Workflow run. Use one when a tool must wait for a person, webhook, or timer,
...
Workflow tools use the [Workflow SDK](https://workflow-sdk.dev): `"use step"`, `createHook`,
`createWebhook`, `sleep`, retries, and replay. eve provides `ctx.ask` for questions answered through
the session's channel and `ctx.agent` for durable subagent delegation. A waiting workflow's `yield`
is turn progress. A background workflow also receives `task`: yield an ordinary value for
stream-only progress or `task.postMessage(...)` to wake the parent agent with one message.
Values needed after a durable wait stay in local variables in the workflow body.
...
- Export `defineWorkflowTool({ ... })` as the default export. Its `execute` must be an async
  ...
  eve compiles it as a workflow automatically; no `"use workflow"` directive is needed.
...
Replace `defineTool` with `defineWorkflowTool`, remove the executor's `"use workflow"` directive,
