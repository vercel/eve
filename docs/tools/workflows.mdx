...
A workflow tool call resolves inside the turn that made it. The turn waits for the workflow's
result, and the model sees that result as the tool result. While the body waits durably, the
workflow holds no compute.
...
One call, one result. A workflow tool's call resolves once, with the return value, the error, or a
cancellation.

Every workflow tool call is a task. When its run starts, the session stream emits `task.started`
with `kind: "workflow"`, the tool `name`, and no `child`. The call's first outcome emits one
...
While a workflow tool runs, the turn waits for it. A message with `turnPolicy: "queue"` waits until
the turn settles. A message with `turnPolicy: "steer"` (the default) is applied after the tool call
resolves, in the same turn, so the model sees it before continuing. Steering does not cancel the
run. Input responses never steer. Cancelling the turn cancels the run, which withdraws its requests.
