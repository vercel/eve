...
cancelled. It is durable — it survives replay, and a step that
...
The caller does not wait for that cleanup. Cancelling the turn records the call as cancelled at
once and signals the run, which unwinds on its own. A run that cannot receive the signal yet, for
example because it has only just started, is stopped immediately. A run that has not ended 35
seconds after the signal, 5 seconds past its own grace period, is stopped outright.
