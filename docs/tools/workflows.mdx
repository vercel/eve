...
The call's `abortSignal`, from `ctx.receive()`, aborts when the call is cancelled: by
`session.cancel()` or by the end of the session. It is durable: it survives replay, and a step
that receives it observes the abort. Pass it into the steps that should stop, and clean up in
`try/finally`:
