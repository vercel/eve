...
- Export `defineWorkflowTool({ ... })` as the default export. Define exactly one entry point,
  `execute` or `task`, as an async function or async generator, written inline or referenced as a
  top-level `async function` in the same module or an imported application module. Defining
  neither, or both, throws at definition time. Start the entry point with `"use workflow"` as its
...
`ctx.agent`, `ctx.agents`, and `ctx.ask` are available only on `WorkflowToolContext` and
`WorkflowTaskContext` in the workflow body.
...
- **Waiting:** the model gets `task_wait`, which parks the turn until one of its tasks settles, a
  ...
  `task` tool. See [`task_wait` and `task_cancel`](/docs/concepts/built-in-tools#task_wait-and-task_cancel).
...
- **Cancellation:** `task_cancel`, `session.cancel()`, a failed turn, and the end of the session
  ...
  finished 30 seconds after the cancel is stopped.
...
use the tool module and the name `execute` or `task`; imported ones use their declaring module and
name.
