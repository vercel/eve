...
model can send more calls by its `taskId`. See [Tasks](/docs/tools/tasks) for choosing
between them and for what the model sees while tasks work.
...
See [Upgrade to Tasks](/docs/tools/tasks-upgrade) for `task` replacing `execution: "background"`
and the other changes in this release.
...
The model waits for results with `task_wait` and stops a task with `task_cancel`, and a turn can't
end while tasks it started are working. `task_cancel`, `session.cancel()`, a failed turn, and the
end of the session abort the call's `abortSignal`; a run that hasn't finished 30 seconds after the
cancel is stopped. See [Tasks](/docs/tools/tasks) for what the model sees, how held turns look on
the stream, steering, events, and limits.
...
A `serve` tool's calls are tasks, with the receipts and results
[described above](#run-calls-as-tasks-task) and the waiting, turn rule, callers, and limit
described in [Tasks](/docs/tools/tasks). What differs:
