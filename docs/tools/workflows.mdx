...
`"use step"`, `createHook`, `createWebhook`, `sleep`, `start`, retries, replay. eve adds `ask` — a
question the human on the session's channel answers — and treats each `yield` as a durable progress
report. Both come from `eve/workflow`.
...
export default defineTool({
  ...
  async execute({ service }, ctx) {
    ...
    if ((await answer).optionId !== "approve") {
...
- `createHook`, `createWebhook`, `sleep`, `getWritable`, `getWorkflowMetadata`, and `FatalError`
  come from `workflow`; `start`, `getRun`, and `resumeHook` from `workflow/api` and are callable from
  a body. eve resolves both specifiers to the Workflow SDK version it builds and runs, so your app
  does not install it. For the TypeScript types, list `eve/workflow-modules` in your tsconfig's
  `compilerOptions.types` (new projects already do); an installed `workflow` package takes
  precedence if you add one.
...
- `ctx` inside the body carries `session`, `callId`, `toolName`, `owner`, and `abortSignal`. The
  members that need the turn's process — `getSandbox`, `getSkill`, `getToken`, and `requireAuth` —
  are unavailable in a workflow body and throw when touched. Read credentials from the environment
  inside a step instead.
- `ask` and `yield` reach the session through `ctx.owner`: the three hooks of the turn or task that
  started the run — `report` for progress, `request` for a question carrying the token of the hook its
  answer resumes, `outcome` for the result. `ask` is a small helper over `createHook` and `resumeHook`;
...
const response = await answer; // { optionId?: string; text?: string }
```

`ask` publishes an `input.requested` event on the session — rendered the way channels render
`ask_question` and tool approvals — and returns the hook the answer resumes. Awaiting the hook
suspends the run until a response arrives.

Because `ask` returns the hook rather than a promise of the answer, it composes with the SDK's own
constructs. Race it against a deadline:

```ts
const answer = await ask(ctx, { prompt: `Deploy ${service}?`, options: APPROVE_OR_CANCEL });
const decided = await Promise.race([answer, sleep("4h")]);
if (decided === undefined) return { deployed: false, reason: "timed out" };
```

Or iterate it as a standing question — one card on the channel, answered repeatedly:

```ts
const next = await ask(ctx, { prompt: "Next command?", allowFreeform: true });
for await (const command of next) {
  if (command.text === "stop") break;
...
receives it observes the abort. Pass it into the awaits that should stop, and clean up in
`try/finally`:
...
A run that never observes the signal is force-cancelled after a short grace period, so cancellation
always terminates the run; observing the signal is how you clean up gracefully first.
...
async execute({ service }, ctx) {
  ...
  const plan = await planDeploy(service);
  const pending = await ask(ctx, { prompt: `Deploy ${service}?`, display: "confirmation", options: APPROVE_OR_CANCEL });
...
export default defineTool({
  ...
  async execute({ orderId, amount }, ctx) {
    ...
    const decision = await await ask(ctx, {
