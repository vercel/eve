...
from a background workflow. eve treats each `yield` as a durable progress report.
...
Background workflow tools can call a visible subagent and wait for its result:
