...
An idle session starts a result turn for the result, a running turn from the principal whose call
started the task receives it at its next tool step, and a result never appears in another
principal's turn. Until the result arrives, the task is listed in the `[Tasks]` note. See
...
`action.partial` for the originating call and never wake the model. Cancelling a turn does not
cancel the background tasks it started. The model stops a background task with
