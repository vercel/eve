...
- **Waiting:** the model gets `task_wait`, which parks the turn until one of its tasks settles, a
  ...
  are offered, with a short system prompt block that explains tasks, only when the agent has a
  `task` or `serve` tool. See [`task_wait` and `task_cancel`](/docs/concepts/built-in-tools#task_wait-and-task_cancel).
