...
### Run a program supplied at runtime

Call `runWorkflowProgram` inside a workflow tool when the tool's input contains a JavaScript
function body produced at runtime:

```ts title="agent/tools/run-program.ts"
import { defineWorkflowTool, runWorkflowProgram } from "eve/tools";
import { z } from "zod";

const agents = ["ticket-triage", "ticket-review"] as const;

export default defineWorkflowTool({
  description:
    "Run JavaScript with ctx.agent(name, input). Available agents: ticket-triage, ticket-review.",
  inputSchema: z.object({ js: z.string() }),
  async execute({ js }, ctx) {
    "use workflow";
    return runWorkflowProgram(js, ctx, { agents, maxSubagents: 20 });
  },
});
```

The supplied source is an async function body. Its only host capability is
`ctx.agent(name, { message, agentId?, outputSchema? })`. For example:
...
`agents` is a required trusted allowlist. Describing an agent in the tool description does not
authorize it. `maxSubagents` defaults to `100` and must be an integer from `1` through `128`.
The generated program cannot access the workflow context, session state, credentials, imports,
or ordinary tools. Agent authorization, questions, approvals, cancellation, and `agentId`
continuation use the same owner-managed behavior as direct `ctx.agent` calls.
...
The experimental uppercase `Workflow` framework tool and the `eve/tools/workflow` entry point have
been removed. Replace the sentinel in `agent/tools/workflow.ts`:
...
with an ordinary authored workflow tool:
...
The path still gives the authored tool the model-facing name `workflow`. Unlike the removed
framework tool, eve does not discover or advertise an agent catalog automatically. List the agents
in the description for the model and in the required trusted `agents` option for authorization.
Move the old `maxSubagents` value into the `runWorkflowProgram` options.
