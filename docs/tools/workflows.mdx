...
the session's channel, `ctx.agent` for durable subagent delegation, and opt-in sandbox access from
steps. Use `yield` to report progress and `await` on a workflow operation to wait durably. Values
needed after a durable wait stay in local variables in the workflow body.
...
- In the body, `ctx` has `session`, `callId`, `toolName`, `abortSignal`, `agent`, and `ask`.
  `getSandbox` is also present, but it works only inside a `"use step"` function when the
  definition sets `sandbox: true`. Calling it from the `"use workflow"` executor throws.
  `getSkill`, `getToken`, and `requireAuth` are not part of `WorkflowToolContext`. Read credentials
  from `process.env` in a step; session-scoped workflow authorization is not yet available.
...
## Use the sandbox from steps

Set `sandbox: true` on a workflow tool that needs the current agent's sandbox. Call
`ctx.getSandbox()` only inside a `"use step"` function:
...
export default defineWorkflowTool({
  description: "Write a marker, then inspect it in a later step.",
  inputSchema: z.object({ marker: z.string() }),
  sandbox: true,
  async execute({ marker }, ctx) {
    "use workflow";
    await writeMarker(ctx, marker);
    return await readMarker(ctx);
  },
});

async function writeMarker(ctx: WorkflowToolContext, marker: string) {
  "use step";
  const sandbox = await ctx.getSandbox();
  await sandbox.writeTextFile({ path: "marker.txt", content: marker });
}

async function readMarker(ctx: WorkflowToolContext) {
  "use step";
  const sandbox = await ctx.getSandbox();
  return await sandbox.run({ command: "cat /workspace/marker.txt" });
}
```

eve opens or reconnects the sandbox before admitting the workflow run. Each step then reconnects
to that same sandbox when it calls `ctx.getSandbox()`, so completed file writes and commands remain
visible to later steps. This behavior is the same for default and background workflow tools.

The returned handle is the normal live sandbox API, including file methods, `run`, `spawn`,
`setNetworkPolicy`, and `stop`. Keep the handle, spawned process handles, and their streams within
the step that created them. Consume each spawned process and call `wait()` or `kill()` before the
step returns. `delete()` is not supported from workflow-tool steps.

Sandbox changes are not transactional. A failed, retried, or cancelled workflow does not roll back
files or completed commands. Without `sandbox: true`, a step that calls `ctx.getSandbox()` throws;
eve does not provision a sandbox based on runtime use.
