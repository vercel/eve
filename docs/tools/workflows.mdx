...
the session's channel, `ctx.agents` for effective callable-agent descriptions (plus `models` for a subagent that offers a [model `choice`](../subagents#let-the-parent-choose-the-model)), and `ctx.agent` for durable agent delegation. Use `yield` to report
...
`ctx.agent(name, { message, agentId?, model?, outputSchema? })`. `model` picks one of a subagent's [model choices](../subagents#let-the-parent-choose-the-model) by slug for a new child. This can invoke subagents configured with `tool: false` or hidden by a same-named `disableTool()` file. Include those path-derived names in the workflow tool description or agent instructions when the model writes the program. For example:
