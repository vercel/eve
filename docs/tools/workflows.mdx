...
the background. For the same reason, while a `ctx.agent` call is working, a model call that passes
that child's `agentId` fails with `AGENT_BUSY` instead of sending it a message. An inline
...
2. Otherwise, each pending question created with `dismissible: true` resolves as `dismissed`. Its
   call keeps waiting for up to 10 seconds to return as usual. A call still working after that
   detaches with the calls in step 3, so the turn never waits on it to show the model the message.
...
one `task.result` message, once no member is still working. The wait is bounded by each working
member's time limit, or by the session's lifetime for a workflow tool without `timeout`. Agent calls
in the same turn detach with them; see
[Subagents](/docs/subagents#steering-while-a-call-waits). A result stays in its group even if the
model gives that agent new work before the group is delivered.

A call waiting on an approval, or on a question that is not dismissible, also detaches. Its request
stays pending, and it does not hold the rest of its group: the others are delivered once they
finish. A later answer resumes the run, which reports its result in its own message.
...
The timer only affects the turn that made the call: once that turn stops waiting, a timer that has
not fired no longer detaches anything. Outside interactive root turns, the call waits. Inside one,
a steering message can still detach the call before its timer fires. `timeout` can be at most
2,147,483,647 milliseconds (about 24.8 days), the longest delay `setTimeout` accepts.
...
- **Several results.** Results from the same principal that are ready together share one message
  and one result turn.
- **Turn waiting on a person.** While a turn waits on an approval or a question, no result turn
  starts, whoever the result belongs to. Results wait until the request is answered or the turn
  ends, then follow the rules above.
...
Cancel rights come with access to the session, not with the principal that started a task: any
caller that can reach the session, and the model in any principal's turn, can stop any of its
tasks.
