...
- In the body, `ctx` has `session`, `callId`, `toolName`, `abortSignal`, `agent`, `agents`, and `ask`.
  ...
  `session`, `callId`, `toolName`, `abortSignal`, `getSandbox`, `getToken`, and `requireAuth`. Read `ctx.agents`, call
  `ctx.agent()`, and call `ctx.ask()` in the workflow body; pass only the serializable values the step needs.
  `getSandbox` opens the session sandbox on demand and runs only inside steps. `getSkill` remains unavailable.
...
### Access the session sandbox from a step
...
export default defineWorkflowTool({
  description: "Read a report from the session sandbox.",
  inputSchema: z.object({ path: z.string() }),
  async execute({ path }, ctx) {
    "use workflow";
    return await readReport(ctx, path);
  },
});

async function readReport(ctx: WorkflowStepToolContext, path: string) {
  "use step";
...
Pass `ctx` directly as a step argument. Keep sandbox handles, process handles, and streams inside
the step; return serializable results. Consume and wait for or kill spawned processes before the
step returns. `ctx.getSandbox()` throws in the workflow body.
Steps cannot call `sandbox.stop()` or `sandbox.delete()` because the owning session controls the
sandbox lifecycle. Static tools under `agent/tools/` support this option; dynamic and extension
workflow tools remain unsupported.
