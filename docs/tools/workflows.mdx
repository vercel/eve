...
- **Turn end:** a turn can't end while tasks it started are working. When the model ends such a
  ...
  turn. A `final_output` call made while tasks work returns an error naming them. The held text
  step reports `finishReason: "tool-calls"` on `message.completed`, so channels don't post it as
  the reply.
