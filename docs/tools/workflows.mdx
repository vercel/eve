...
questions answered through the session's channel and `agent` for durable subagent delegation. A
waiting workflow's `yield` is turn progress. A background workflow also receives `task`: yield an
ordinary value for stream-only progress or `task.postMessage(...)` to wake the parent agent
with one message. Values needed after a durable wait stay in local variables in the workflow body.
...
- In the body, `ctx` has `session`, `callId`, `toolName`, and `abortSignal`. `getSandbox`,
  `getSkill`, `getToken`, and `requireAuth` throw there. Read credentials from `process.env` in a
  step; session-scoped workflow authorization is not yet available.
...
`agent` and `ask` from `eve/workflow` require the `ctx` supplied to an eve workflow tool body.
Channel handlers, schedule handlers, ordinary tools, and `"use step"` functions cannot call them.
Adding `"use workflow"` to a channel or schedule handler does not make it a workflow tool.

eve reports statically identifiable misuse at build time with the source location and the helper
name. Shared helpers may call `agent` or `ask` when invoked from a workflow tool body with that
body's `ctx`; indirect calls that the build cannot check still fail at runtime if the context is
missing. To start a session from a channel or schedule, use the
[channel operations](/docs/channels/custom#channel-operations-and-session-handles) or
[schedule handler](/docs/schedules#handler-form-run) APIs.
...
| progress (`yield`)       | `action.partial` on the turn        | stream-only; does not wake the agent        |
| parent message           | not applicable                      | `yield task.postMessage(...)`               |
...
A workflow body may be an async generator. Each ordinary `yield` reports progress; the return
value is the result. If the body returns nothing, a waiting tool uses its last yield and a
background tool completes with `null`.
...
background tool ordinary yields are stream-only; `yield task.postMessage(message)` wakes the
owning agent. Returning ends the run with the result; throwing ends it with the failure.
