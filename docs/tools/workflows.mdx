...
[Return a receipt with `detach`](#return-a-receipt-with-detach). Each call runs as a
[task](/docs/concepts/tasks), the model eve uses for both workflow tool calls and agent calls.
...
result, and the run's result reaches the model later in a `task.result` message, like a
[`detach: true` result](#return-a-receipt-with-detach). An interactive root session is a root
session, not a subagent's session, in conversation mode. A turn that a schedule started never
detaches, so a scheduled run posts one final reply. See
[Which calls wait](/docs/concepts/tasks#which-calls-wait) for every call site.

### Detach on a new message

When a steering message (`turnPolicy: "steer"`, the default) arrives while an interactive root turn
waits on calls, it first answers a pending question that it matches, then resolves questions created
with `dismissible: true` as `dismissed`. Every call still waiting then detaches, including a call
waiting on an approval, whose request stays pending. eve emits `task.detached` with
`reason: "steer"` for each call, and the turn continues in one model step with the receipts followed
by the new message. The model reads a receipt such as:
...
The calls that one message detaches, including [agent calls](/docs/subagents#steering-while-a-call-waits)
in the same turn, deliver their results together once none of them is still working. A message with
`turnPolicy: "queue"` never detaches anything. eve's [`sleep` tool](/docs/concepts/built-in-tools#sleep)
is the exception in every session: a steering message ends a waited `sleep` early instead of
detaching it. See [Detach a waited call](/docs/concepts/tasks#detach-a-waited-call) for the full
order, detach groups, and how other sessions apply the message.
...
starts with `mode: "background"` on `task.started`. `detach: true` applies in every session, and a
subagent's session or a task-mode run finishes only after the result is delivered. Without
`detach: true`, the call waits until it returns or [detaches](#detach-a-waited-call).

When the run finishes, eve delivers its outcome to the model in a user-role message of kind
...
The turn that started the task receives the result at its next tool step, or when the model tries
to end the turn: a turn cannot end while tasks it started are working, so eve holds it until they
settle. No result starts a turn of its own, and a result never appears in another principal's turn. Until the result arrives, the task is listed in the `[Tasks]` note. See
[How results arrive](/docs/concepts/tasks#how-results-arrive) for every delivery rule.

A session holds at most 10 working background tasks, and a `detach: true` call over that limit does
not start: it fails with `TOO_MANY_BACKGROUND_TASKS`. Yields from a background run still publish
`action.partial` for the originating call and never wake the model. Cancelling a turn also
cancels the background tasks that are working. The model stops a background task with
[`task_cancel`](/docs/concepts/built-in-tools#task_cancel), and application code with
`session.cancel({ taskId })`; the run then unwinds as described in
[Cancel and clean up](#cancel-and-clean-up-ctxabortsignal). See
[Cancel tasks](/docs/concepts/tasks#cancel-tasks).
...
Every workflow tool call is a [task](/docs/concepts/tasks). When its run starts, the session stream
emits `task.started` with `kind: "workflow"`, the tool `name`, `mode` (`"background"` for
`detach: true`, otherwise `"foreground"`), and no `child`. A waited call that detaches emits
`task.detached`, and its `action.result` carries the receipt. The call's first outcome emits one
`task.settled`, and a run that fails to start settles with `error.code` `START_FAILED`. Progress
stays on `action.partial` and the result on `action.result`. Agent calls emit the same events with
`kind: "agent"`, so filter `task.*` events by `kind` and `name`. See
...
interactive root session; see [Detach on a new message](#detach-on-a-new-message). Elsewhere, the
message is applied after the tool call resolves, in the same turn, so the model sees it before
continuing. Steering never cancels the run, except for eve's `sleep`, which ends early. Input
responses never steer. Cancelling the turn cancels the waited run, which withdraws its requests.
