...
Pass `outputSchema` on a client turn when the caller wants structured data instead of only assistant text. When the model supplies the structured result, Eve emits it as `result.completed`.
...
## Require structured settlement

Set `outputSchemaRequired: true` when prose is not a valid terminal result. Eve still permits ordinary tool calls and iteration. If the model then ends in prose, Eve makes one final call with `final_output` selected. A second miss emits `REQUIRED_OUTPUT_SCHEMA_NOT_FULFILLED` and parks the conversation turn instead of silently treating prose as the structured result.

`outputSchemaRequired` is run-scoped, must accompany `outputSchema` when a session is created, and is cleared with that schema after success or failure.
