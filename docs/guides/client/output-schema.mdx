...
Pass `outputSchema` in the same input object as the message, headers, signal, context, attachments, or HITL responses:
...
const followUpResponse = await session.send({ message: "Now answer normally." });
