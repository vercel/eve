...
Pass `info({ signal: AbortSignal.timeout(5000) })` to cancel the inspection request after five seconds. This cancels the HTTP request, not a custom credential callback that is still resolving.

The development terminal limits startup inspection to two seconds for local agents and five seconds for remote agents. If inspection times out, local startup continues without the optional configuration details; remote connection checks report inspection as unavailable. Sending a message still requires the agent's normal authorization checks.
