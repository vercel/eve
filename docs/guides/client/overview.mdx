...
`client.sessions` stores only the session ID and stream cursor. Sending through a handle for an unknown or terminal ID fails instead of creating a replacement. Reset leaves the handle pinned to the retired ID.

A client can own many independent fixed sessions at once:

```ts
const { session: alice, response: aliceResponse } =
  await client.sessions.create("Summarize account A.");
const { session: bob, response: bobResponse } =
  await client.sessions.create("Summarize account B.");
