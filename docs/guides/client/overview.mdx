...
Use `info()` to inspect an agent. The client requires the complete agent-info version 6 response before returning it:
...
Pass `info({ signal })` to cancel the HTTP request. The signal is checked only after custom header and credential callbacks resolve; it does not limit how long those callbacks or the complete `info()` call can take.
...
Create and park an ID-addressed conversation before its first turn when you can start session work
...
The create promise resolves when the server accepts the durable workflow. It does not open the
event stream or wait for initialization; an early `session.send()` uses the
same bounded startup retries as other follow-ups.
...
`session.cancel()` cancels the active turn, its attached calls, and every working [task](../../concepts/tasks#cancel-tasks), whichever turn started it, then ends the turn. Idle agents and resumable tasks stay available. Pass the observed `turnId` to cancel only while that turn is still active; once no turn is active, it cancels only the tasks that turn started. There is no option to stop a single task from the client; the agent does that with `task_cancel`. Cancellation is asynchronous: a stopped task emits `task.settled` with `status: "cancelled"` and never delivers a result.
