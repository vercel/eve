...
await session.cancel({ taskId: "remind-q4x1ze" });
...
`session.cancel()` cancels the active turn and the tool calls it waits on. Pass `taskId`, from a receipt or a `task.started` event, to stop one [background task](../../tools/workflows#return-a-receipt-with-detach) and leave the turn running, or `tasks: true` to cancel the active turn and every working task. `taskId` cannot be combined with `tasks` or `turnId`. Cancellation is asynchronous: a stopped task emits `task.settled` with `status: "cancelled"` and never delivers a result.
