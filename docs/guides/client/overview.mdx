...
`client.sessions` stores only the session ID and stream cursor. Reset leaves the handle pinned to the retired ID rather than silently creating or following a replacement.
