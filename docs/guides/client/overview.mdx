...
await session.cancel({ taskId: "task_123" });
await session.compact();
await session.clear();
...
Background tasks owned by the session publish `task.updated` events on the same durable stream. The event carries a client-safe task view with private executor routing removed. It reports `working`, `input_required`, `completed`, `failed`, or `cancelled`, plus the current framework notification and terminal output or token usage when available. `session.cancel({ taskId })` targets that owned task; omit `taskId` to cancel the active parent turn instead.
