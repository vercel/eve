...
```ts
const { session, response } = await client.sessions.create("Create a launch checklist.");
...
const { session: fresh, response } = await client.sessions.create("Begin again.");
```
...
```ts
const { session: research, response: researchResponse } =
  await client.sessions.create("Research competitors.");
const { session: support, response: supportResponse } =
  await client.sessions.create("Draft a support reply.");
