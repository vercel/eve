...
For each `send()` or `respond()`, the server returns the durable tail captured
before it accepts the new input. An attached handle advances its local cursor to
that position, preventing an interrupted or stale saved cursor from making the
new response replay and settle on an older turn. Read missed history explicitly
with `snapshot()` or `stream()` before sending new input.
