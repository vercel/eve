...
`message.appended`, `reasoning.appended`, and `action.input.appended` are incremental delta events. Each carries its new text and a `startsBlock` boolean. Apply `startsBlock: true` by resetting the accumulator before adding that delta; apply `false` by appending to the existing block. The reset operation is idempotent, so consumers do not need to create a new generation each time they see it. eve may combine adjacent continuation deltas for the same text block or tool call while a durable stream write is in flight, but preserves their text and event ordering. A block start, different event type, tool `callId`, or stream coordinate is a barrier. The completed text forms, `message.completed` and `reasoning.completed`, are the compatibility path for clients that don't render deltas. A streamed tool input is complete when the matching validated call arrives in `actions.requested`.
...
A continuation is useful only when the consumer has already accumulated that block. After reconnecting without local state, replay from the block start or wait for its completed event instead of appending a continuation to an empty value. Apply the same rule to `reasoningDelta` and `inputTextDelta`. `startsBlock` identifies block boundaries; it does not validate character positions inside a block. Supported clients rely on the durable event cursor for ordered, gap-free delivery.

The eve client validates the stream version on every connection. It accepts v21–v24 cumulative message and reasoning append events and v24 offset-based tool-input appends, then exposes them through the current `startsBlock`-based `MessageStreamEvent` contract. This also applies when an automatic reconnect reaches a newer deployment. A missing or unsupported `x-eve-stream-version` header fails the stream instead of treating unknown JSON as the current event type.
