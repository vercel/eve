...
Between turns, `session.compact()` queues context compaction without sending model input. An accepted request reports the session id; confirm completion by reading `compaction.requested` and `compaction.completed` from the durable stream. A never-started session returns `no_active_session` as a successful no-op.
