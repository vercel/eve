...
A continuation is useful only when the consumer has already accumulated that block. After reconnecting without local state, replay from the block start or wait for its completed event instead of appending a continuation to an empty value. Apply the same rule to `reasoningDelta` and `inputTextDelta`.
