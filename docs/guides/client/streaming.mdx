...
The same stream request captures the server's current tail and closes after the first current-turn boundary at or beyond it. If the cursor is already current, the stream closes empty. A clean, fully consumed replay atomically commits its delivered events, current continuation, and absolute cursor to the public session state. A body error, malformed NDJSON, abort, or intentional early consumer return leaves public session state unchanged, so retry starts from the original cursor. `throughCurrentTail` cannot be combined with a negative tail-relative index.
