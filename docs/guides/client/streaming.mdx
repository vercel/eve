...
A delegated subagent reports its progress on its own child session. The parent stream announces each started child with a `task.started` event. Pass that event to `streamSubagent()` on the parent's session handle to follow the child:
