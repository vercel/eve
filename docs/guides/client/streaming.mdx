...
Between turns, `session.compact()` queues context compaction without sending model input. An accepted request reports the session id; consume the durable stream through the following `session.waiting` boundary before sending the next turn, checking for `compaction.requested` and `compaction.completed` along the way. A never-started session returns `no_active_session` as a successful no-op.
