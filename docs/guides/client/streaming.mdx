...
A `message.received` event with `data.kind: "task.result"` is not a user message. eve emits it when
it delivers [background task results](../../concepts/tasks#how-results-arrive) to the model, and
`data.taskIds` names the delivered tasks. The default message reducer skips it and never uses it to
confirm an optimistic submission. A custom renderer should skip it too, and show the assistant reply
that follows. To check that a custom consumer handles every task event, replay the
[recorded task streams](../../concepts/tasks#test-a-consumer-against-recorded-streams) that ship
with eve.
...
A delegated subagent reports its progress on its own child session. The parent stream announces each started child with a `task.started` event whose `kind` is `agent`. Workflow tool calls also emit `task.started`, with `kind: "workflow"` and no child session, so select agent tasks by `kind` and `name`. Pass the event to `streamSubagent()` on the parent's session handle to follow the child:
...
if (started?.type === "task.started") {
  for await (const event of session.streamSubagent(started)) {
    console.log(started.data.name, event.type);
    if (isCurrentTurnBoundaryEvent(event)) break;
  }
}
```

`streamSubagent()` reads the event's `child.streamPath` with the parent session's host and credentials, so the same client works for local and [remote](../remote-agents) children:

- A local child's path is its own session stream route on the same deployment. The channel's usual session authorization applies.
- A remote child's path is a proxy route on the parent deployment. The parent checks that the child belongs to this session, then relays the remote child's stream with the remote agent's authored `auth` and `headers`. The client never needs the remote URL or its credentials.

The child has its own cursor, starting at `0`, and reading it never advances the parent's `streamIndex`. Pass `startIndex` to resume, `follow: false` for a bounded read, and `signal` to detach. A child parks after answering and can be continued later, so stop at a turn boundary with `isCurrentTurnBoundaryEvent` when you only need the current turn. The matching `task.settled` event on the parent stream reports the call's outcome.

If a steering message moves a waiting call to the background, the parent stream emits `task.detached` with the call's `taskId`, `callId`, and `reason` (`"steer"`, or `"timeout"` for a workflow tool's `detach: { timeout }` timer). The call's `action.result` then carries a receipt, `{ status: "working", taskId }`, while the child keeps working, so keep following its stream until `task.settled`. `task.detached` is stamped with the delivery that made the call, so read it from `session.stream()` rather than from the steering message's `MessageResponse`. See [Detach a waited call](../../concepts/tasks#detach-a-waited-call). Passing a `task.started` event that has no `child` throws, and so does passing a remote child's event from another parent session, whose proxy route names that session.

## Abort a request

Pass an `AbortSignal` to cancel the POST or stream. Aborting is local transport cancellation: turns are resumable across disconnects, so detaching never stops server-side work. Use `session.cancel()` to stop the active turn and the tool calls it waits on, `session.cancel({ taskId })` to stop one background task, or `session.cancel({ tasks: true })` to stop both the turn and every working task. Cancellation is asynchronous; watch the stream for `turn.cancelled` followed by `session.waiting`, and for `task.settled` with `status: "cancelled"` for each stopped task.
