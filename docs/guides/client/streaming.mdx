...
The server also closes a live stream response on purpose after ten seconds without an event, so a long tool call produces several short connections rather than one long one. The client treats that clean close as a cue to reconnect right away, without backoff, and it does not count toward the empty-stream limit. Only connections that end abnormally, such as a dropped socket or a read that stalls for twenty seconds, back off before reconnecting.
