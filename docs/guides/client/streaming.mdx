...
Once `send()` is accepted, `response.cancel()` requests cooperative cancellation of that exact turn. Start consuming the response first; cancellation waits for the stream to identify the turn, guards the request with its ID, and never targets a later turn. The result status is `accepted` when the HTTP route schedules background delivery. Continue reading to observe cancellation. A response that settles before a turn starts returns `no_active_turn`:
