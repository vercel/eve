...
The eve client validates the stream version on every connection. It accepts v24 cumulative append events and v25 offset append events, then exposes both as the current offset-based `MessageStreamEvent` contract. This also applies when an automatic reconnect reaches a newer deployment. A missing or unsupported `x-eve-stream-version` header fails the stream instead of treating unknown JSON as the current event type.
