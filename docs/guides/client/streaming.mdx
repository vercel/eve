...
it delivers [background task results](../../tools/workflows#return-a-receipt-with-detach) to the
model, and `data.taskIds` names the delivered tasks. The default message reducer skips it and never
uses it to confirm an optimistic submission. A custom renderer should skip it too, and show the
assistant reply that follows.
...
If a steering message moves a waiting call to the background, the parent stream emits `task.detached` with the call's `taskId`, `callId`, and `reason` (`"steer"`, or `"timeout"` for a workflow tool's `detach: { timeout }` timer). The call's `action.result` then carries a receipt, `{ status: "working", taskId }`, while the child keeps working, so keep following its stream until `task.settled`. `task.detached` is stamped with the delivery that made the call, so read it from `session.stream()` rather than from the steering message's `MessageResponse`. See [Detach a waited call](../../tools/workflows#detach-a-waited-call). Passing a `task.started` event that has no `child` throws, and so does passing a remote child's event from another parent session, whose proxy route names that session.
