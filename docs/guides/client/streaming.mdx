...
it delivers [task results](../../concepts/tasks#how-results-arrive) that no `task_wait` received to the model, and
...
The child has its own cursor, starting at `0`, and reading it never advances the parent's `streamIndex`. Pass `startIndex` to resume, `follow: false` for a bounded read, and `signal` to stop reading. A child parks after answering and can be continued later with a send, so stop at a turn boundary with `isCurrentTurnBoundaryEvent` when you only need the current turn. The matching `task.settled` event on the parent stream reports the outcome of that generation, and `task.ended` reports that the agent stopped taking input.

An agent call returns a receipt, `{ status: "working", taskId }`, on its `action.result` while the child keeps working, and the receipt and `task.started` can arrive in either order, so keep following the child's stream until `task.settled`. A send to the same agent with its `taskId` that starts its next turn emits another `task.started` with the same `child` and the next `generation`. Passing a `task.started` event that has no `child` throws, and so does passing a remote child's event from another parent session, whose proxy route names that session.

## Abort a request

Pass an `AbortSignal` to cancel the POST or stream. Aborting is local transport cancellation: turns are resumable across disconnects, so detaching never stops server-side work. Use `session.cancel()` to stop the active turn, its attached calls, and every working task. Cancellation is asynchronous; watch the stream for `turn.cancelled` followed by `session.waiting`, and for `task.settled` with `status: "cancelled"` for each stopped task.
