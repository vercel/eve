...
for await (const event of response) {
  ...
  const next = appendStreamTextDelta(message, event.data.messageOffset, event.data.messageDelta);
  if (next !== undefined) message = next;
}
```

Offset `0` starts or restarts a text block. A later delta must begin exactly at the accumulated string's length; the helper returns `undefined` for a gap or overlap so a consumer does not project corrupt text. Apply the same helper to `reasoningDelta` and `reasoningOffset`, or `inputTextDelta` and `inputTextOffset`.

The eve client validates the stream version on every connection. It accepts v21–v24 cumulative append events and v25 offset append events, then exposes both as the current offset-based `MessageStreamEvent` contract. This also applies when an automatic reconnect reaches a newer deployment. A missing or unsupported `x-eve-stream-version` header fails the stream instead of treating unknown JSON as the current event type.
