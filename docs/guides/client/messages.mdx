---
title: "Messages"
description: "Send text, durable context, client context, attachments, and HITL responses with eve/client."
---
...
Pass `context` to `create()` for the first turn or in the options to `send()` and `respond()`. Each string becomes a user-role message immediately before the delivery and remains in durable session history:

```ts
const { session, response } = await client.sessions.create({
  message: "Summarize this project.",
  context: ["The workspace is Acme.", "The active project is Atlas."],
});

await response.result();
```

Later turns see this context through normal conversation history. Compaction can summarize it, and `clear()` removes it. Sending the same context again appends another copy; eve does not replace or deduplicate durable context.

## Add ephemeral client context

Use `clientContext` for volatile page state that should not remain in conversation history:

```ts
await session.send("What should I do on this screen?", {
  clientContext: {
    route: "/billing",
    plan: "pro",
    seatsUsed: 4,
...
`send(message, options)` and `respond(inputResponses, options)` are separate operations. Put `context`, `clientContext`, `outputSchema`, headers, or stream options in the second argument to either method.
