...
`/model`, `/add`, `/deploy`, `/link`, `/info`, and `/traces` are available when `eve dev` runs locally. They are unavailable when the UI connects to a server with `--url`.
...
A supported Vercel Connect connection installed through self-modification can be added before the project is linked. When a root-agent request explicitly targets that unavailable connection, the UI opens project linking or Vercel login as needed. After recovery, the UI retries the original request automatically; connector provisioning and service authorization continue on first use. Connection searches inside subagents remain ordinary tool results and do not open root-level setup.
