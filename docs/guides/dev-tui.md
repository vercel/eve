...
While a turn is running, `Enter` queues a follow-up message. Press `Esc` or `Ctrl+C` with queued messages to send the oldest message immediately as steering. Before assistant output begins, the runtime interrupts pending model generation and continues the same turn with your correction. Executing tools finish safely. After output begins, the runtime applies steering at the next workflow boundary and preserves streamed text. With no queued messages, `Esc` or `Ctrl+C` requests cancellation. If a direct cancellation requested with `/cancel` or `Ctrl+C` does not settle, press `Ctrl+C` to stop waiting. The UI then returns to the prompt and asks you to press `Ctrl+C` again to exit. At an idle prompt, press `Ctrl+C` twice to exit.
