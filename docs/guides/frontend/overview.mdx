...
When the root agent delegates, its stream emits `subagent.called` with the child's `childSessionId`, then `subagent.completed` when the delegation call settles. In blocking mode that coincides with child output. When `subagent.completed.data.backgroundTask.status` is `"working"`, the event records background-task admission and the child is still running. Detailed child progress and its terminal boundary live on the child session's stream instead of being flattened into the root `data.messages`. Use the lower-level [TypeScript client](../client/overview#sessions) to attach to that ID when your UI needs live subagent activity. See [What the parent sees](../../subagents#what-the-parent-sees) for the complete contract.
