...
| `pendingSubmissions` | Browser-local messages submitted while another turn is active.              |
...
After a successful delivery, `send()` resolves as soon as eve accepts it. It
does not wait for the resulting turn to finish. Transport failures surface
through `error` and `onError`. Use `status`, `onFinish`, or the session events
when later work depends on turn completion.

### Queue or steer during an active turn

The hook guards against accidental double submission. While `status` is
`"submitted"` or `"streaming"`, pass an explicit policy with each message:
...
await agent.send("Stop and respond to this instead.", {
  turnPolicy: "steer",
});
```

`"queue"` lets the current turn finish before eve processes the message.
`"steer"` durably buffers the message, then cooperatively cancels the active
turn and starts a replacement. Output already streamed and completed side
effects are not rolled back. Calling `send()` while busy without a
`turnPolicy` rejects.

Render `pendingSubmissions` when the composer remains available during a turn:

```tsx
{
  agent.pendingSubmissions.map((submission) => (
    <p key={submission.id}>
      {submission.message} — {submission.status}
    </p>
  ));
}
```

Each item reports its requested `turnPolicy` and a `status` of `"submitting"`,
`"queued"`, `"steering"`, or `"failed"`. The list is a projection for the
browser that submitted those messages, not an authoritative server-side queue.
eve may combine adjacent accepted messages into one later turn.

`cancel()` can be called as soon as `status` is `"submitted"`; the hook waits
for the shared session stream to identify the active turn when necessary, sends
one guarded cancellation request, and stays attached. The promise resolves
when eve accepts the request or reports that no turn is active, and rejects if
the request fails. The turn then settles on the same stream as `turn.cancelled`
followed by `session.waiting`, so the session is safe to continue. Accepted
queued messages remain buffered and run after the cancelled turn.
...
- `onSessionChange(session)`: fires when a session is created and when a turn boundary publishes its latest cursor. Persist it to resume across reloads.
...
Messages submitted during an active turn remain in `pendingSubmissions` until
eve emits `message.received`; they are not added to the optimistic transcript
as separate messages because the runtime may combine adjacent deliveries.
...
`pendingSubmissions` is not restored from `initialEvents` or `initialSession`.
If a queued or steering message must remain visible across reloads, persist that
application-level UI state before calling `send()` and reconcile it when the
session emits `message.received`.
