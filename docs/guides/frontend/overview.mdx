...
Prewarming defaults to `false`: the first send creates the session. Enable it when your
application's auth, headers, and any chat-record binding are ready at mount. Applications
that prepare those in the submit handler should keep the default.
...
at creation, so an unused prewarmed session can expire before its first message.
...
`reset()` reuses the hook instance and clears the conversation. With `prewarm: true`, it
immediately prepares the next owned session; otherwise, the next send creates it. An explicitly supplied
`session` remains externally owned and is never replaced by prewarming or reset. Use
...
For multiple chat threads, keep one saved event log and session cursor per thread. `agent`, `host`, `reducer`, `session`, `initialEvents`, `initialSession`, `auth`, `headers`, `optimistic`, `prewarm`, and `resume` are read when the hook creates its store, so remount the chat component when switching threads, for example with `key={chat.id}`.
