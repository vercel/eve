...
Pass `sessionContext: { surface: "docs" }` to `useEveAgent()` to capture application data when the session is created, including with `prewarm: true`. The backend reads the object through `ctx.session.context`. The option is captured when the hook is created; existing sessions keep their original context. See [Application context](../session-context#application-context) for an example that selects dynamic instructions without configuring a schema on the agent.
