...
Pass `context` when application context should become part of the durable conversation. Each string is appended as a user-role message before the current delivery, so later turns see it through normal session history:

```tsx
await agent.send("Summarize this project.", {
  context: ["The workspace is Acme.", "The active project is Atlas."],
});
```

Compaction can summarize durable context, and `clear()` removes it. Sending the same value again appends another copy.
