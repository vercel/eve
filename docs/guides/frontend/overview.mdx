...
| Field     | What it is                                                                                |
| --------- | ----------------------------------------------------------------------------------------- |
| `data`    | Projected UI state from the reducer. Defaults to `{ messages, tasks }`.                   |
...
Most chat UIs only need `data.messages`, `data.tasks`, and `status`. Drop down to `events` when you need the authoritative wire events directly, for example to persist an audit log or build a custom projection.
...
`data.tasks` are eve-owned `EveTask[]`. Each task carries its durable status, metadata, latest `task_update` activity, timestamps, terminal output, and terminal token usage when available. The projection rebuilds from replayed `task.updated` events after a reload. To cancel one task without cancelling the parent turn, attach a fixed [TypeScript client](../client/overview#sessions) session and call `session.cancel({ taskId })`.

When the root agent delegates, its stream also emits `subagent.called` with the child's `childSessionId`, then `subagent.completed` when the tool returns its background receipt. `data.tasks` covers the task lifecycle and explicit `task_update` messages; detailed child reasoning and tool calls remain on the child session stream. Attach the lower-level TypeScript client to that child ID only when the UI needs those execution details. See [What the parent sees](../../subagents#what-the-parent-sees) for the complete contract.
...
The default reducer projects events into `{ messages, tasks }` (`EveMessageData`). When you want `data` shaped differently, pass a `reducer` implementing `EveAgentReducer<TData>`:
