...
Earlier versions of the module wrote the legacy `experimentalServices` field into `vercel.json`. Vercel no longer routes that model — deployments build both services but every `/eve/v1/*` request returns a platform NOT_FOUND — so the module now ignores the field and warns when it sees one. Delete `experimentalServices` from `vercel.json` (or the whole file if that was all it held), and if your Vercel project's Framework Preset was set to "Services" for the legacy setup, set it back to Nuxt.js.
