...
| `pendingSubmissions`                 | `ComputedRef<readonly EveAgentPendingSubmission[]>` |
...
Call `send(text)` for text or pass content parts for attachments. The API and transport behavior match the [shared sending guide](./overview#sending-and-streaming); only reactive access differs in Vue. Read `pendingSubmissions.value` to render messages queued or steering during an active turn.
