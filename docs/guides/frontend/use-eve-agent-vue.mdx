...
| `send`, `respond`, `prewarm`, `resume`, `cancel`, `reset` | Methods                                        |
...
Pass `prewarm: true` to prepare an owned workflow on mount and after reset. It defaults
to `false`, so the first send creates the session. The binding waits for create acceptance,
retries while the inbox starts, and keeps the session stream open across turns. See [Prewarming and returned state](./overview#returned-state) for creation, failure, and
connection behavior.
...
Import `EveAgentReducer` from `eve/vue` when projecting events into application-specific state. The resulting `data` remains a `ComputedRef<TData>`; `conversation` remains a `ComputedRef<ConversationState>` regardless of your reducer. See [Custom reducer](./overview#custom-reducer) for the reducer contract and client projection events.
