...
`eve/next` lets a Next.js app serve the public routes for one or more eve agents. Wrap your config with `withEve()` to run the app and agents with one development command, then deploy them as one Vercel project. [`useEveAgent`](./overview) automatically finds the mounted routes, so you do not need CORS configuration or URL environment variables.

Use `withEve()` when your Next.js app serves browser-facing agent routes. Use `withEve({ agents })` when the app routes to several named agents. If the app deploys independently alongside agents, APIs, or workers, use a Vercel peer-service graph in [`vercel.ts`](../deployment/vercel#compose-agents-with-other-vercel-services) instead.
...
Web Chat cannot currently be installed directly in a multi-agent workspace. Instead, create or configure a root Next.js application and wrap its config with `withEve()`. eve discovers the workspace agents automatically.
...
Design how users choose an agent in your UI. For example, link to routes such as `/agents/support` and `/agents/billing`, then bind each route's chat to the matching `agent` name. Include the agent identity in durable-session URLs so the UI does not restore a session through the wrong agent route.

Configure browser authorization for every agent your application exposes. An agent that does not accept the application's credentials remains unavailable to browser traffic.
...
- **Non-Vercel hosts.** `withEve()` still provides Next.js rewrites, but it does not generate a service graph for other hosts. By default, `next start` runs each built eve service on its derived loopback port and proxies its routes. If your host supervises eve services itself, run each service separately and set `EVE_NEXT_PRODUCTION_ORIGIN` to the origin that serves its private route prefix:

  ```bash
  EVE_NEXT_PRODUCTION_ORIGIN=https://agent.example.com npm run build
  ```

  For a self-hosted multi-agent application, configure the agent services and production routing layer together. If the public routing layer mounts an agent below a path, set `EVE_PUBLIC_ROUTE_PREFIX` in that agent's build and runtime environments so callback URLs use the public path. For other frontend stacks, configure your ingress, reverse proxy, or API gateway to proxy same-origin `/eve/v1/*` requests, or pass the agent URL as `host` to the client hook.
