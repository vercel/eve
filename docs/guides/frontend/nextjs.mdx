...
Better Auth still handles browser sign-in and cookie verification. The channel's
`betterAuthSession` adapter calls `sessionViewer`, which verifies the cookie with
`auth.api.getSession()` and shares the verified identity with history and ownership
checks. The channel principal ID now uses the verified Vercel account subject
instead of Better Auth's user ID; the history owner key derives from that subject
so it stays stable across sign-ins. Existing cookies without the provider subject
require signing out and back in. Email, name, and profile picture remain principal
attributes. Deployment OIDC credentials are separate from browser identity.
