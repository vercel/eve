...
Opening a conversation scrolls to its latest message. Sending a message returns to the live end of the conversation.
...
## Session history and persistent drafts

New Web Chat apps include a responsive history sidebar, persistent per-session
drafts with undo/redo and attachments, connection recovery, and a closable subagent
inspection pane. Drafts and cached child transcripts are stored in the browser,
isolated by workspace user. Clearing browser storage removes these local copies;
eve remains the server transcript store.

Local history remembers up to 500 sessions opened or created in this browser.
It does not enumerate the machine's workflow storage or replay hidden sessions.
Clearing browser storage clears this local list; known session URLs still work.
For deployed, signed-in history, create an app with:

```bash
pnpm dlx eve init my-agent --channel-web-nextjs --web-authentication sign-in-with-vercel
```

Register a Vercel OAuth application and set `BETTER_AUTH_SECRET`,
`VERCEL_APP_CLIENT_ID`, and `VERCEL_APP_CLIENT_SECRET`. Set `WEB_CHAT_ORIGIN` to the
public HTTPS origin (for example, `https://chat.example.com`) so browser mutations
remain same-origin when Vercel routes them to the internal eve service. Configure
`SESSION_DATABASE_URL` (a Neon-compatible Postgres URL) and `SESSION_INDEX_SCOPE`
(a stable scope unique to the app/environment) in both Next.js and eve services.
Run `node scripts/migrate-session-index.mjs` once before serving production traffic.

Better Auth handles browser sign-in and cookie verification. The channel's
`betterAuthSession` adapter preserves the Better Auth user ID as `principalId`
and the email, name, and profile picture attributes. History does not modify the
authentication principal or add history-specific auth attributes.

The history store derives its owner key from the verified Vercel account subject
in the Better Auth session, keeping ownership stable across sign-ins. It records
that owner when creating a session. Activity updates and child-call bindings use
this stored ownership; event hooks do not read ownership from auth attributes.
Existing cookies without the provider subject require signing out and back in to
access indexed sessions. Deployment OIDC credentials remain separate from browser
identity.

The base Web Chat scaffold deliberately leaves its production viewer
adapter unconfigured; custom authentication must implement `lib/session-viewer.ts`
and enforce the same owner checks in the eve channel.

The index stores ownership, title, creation time, last-message time, and last-turn
time; it does not duplicate transcripts. Browser users can read/control only their
indexed sessions and inspect children linked by indexed, owner-scoped call records.
Session creation and index writes are not transactional together: a database
outage may leave an unlisted session. Activity and child-call metadata is projected
with `waitUntil`; failures are logged and leave stale metadata or
unavailable child access until the index is repaired without failing the chat turn. Existing sessions
are not automatically backfilled.

The composer clears the captured draft when its own send request is accepted,
before waiting for the agent's response. Another tab's message cannot clear it.
Custom clients can observe this boundary with `send(message, { onAccepted })`;
the callback receives the accepted `sessionId` and, when supplied by the server,
`deliveryId`. A rejected request leaves the draft available for retry.

Child panes decode their stream using eve's versioned `readMessageStream(response, { signal })`
reader. The workspace opens at most four child streams concurrently and parks a
follower when its turn finishes or its last subscriber leaves. Reopening resumes
from the retained cursor. Checkpoints are capped at 512 KiB each and 20 sessions;
larger transcripts remain available in memory and replay from the last saved
checkpoint after reload.

Browser draft storage retains up to eight distinct tab branches per conversation. Reloads reuse the same branch; if another branch cannot be saved, the composer warns and keeps that tab’s draft in memory instead of discarding existing work.
