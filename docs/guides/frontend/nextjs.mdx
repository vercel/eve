...
Use `withEve()` for a Next.js app that is the browser-facing routing layer. Use `withEve({ agents })` when that app routes to several named agents. When the app is one independently deployed service among agents, APIs, or workers, use a Vercel peer-service graph in [`vercel.ts`](../deployment/vercel#compose-agents-with-other-vercel-services) instead. See [Project structures](../../project-structures) to choose a layout.
