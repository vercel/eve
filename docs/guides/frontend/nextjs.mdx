...
## Generated chat behavior

Generated Web Chat renders links and inline code with distinct colors, removes code-block headers, and keeps the composer visible while you scroll.

Generated Web Chat includes copy buttons for tool commands, command output, parameters, results, and errors. Reasoning panels stay open or closed while responses stream, and the thinking animation pauses between sweeps.

Repeated user messages stay separate. The conversation scrolls to a newly submitted message and shows Stop only while the active chat turn is running. Input is disabled while the server is unavailable and recovers on reconnection.

Local history remembers up to 500 sessions opened or created in this browser, ordered by last message, with creation and last-turn tooltips. It does not enumerate server files or replay hidden conversations. Clearing browser storage clears this local list; known session URLs remain usable. This browser-local list is not cross-device history. The scaffold does not add a database or a production session index.
...
Inspect subagents in a closable, resizable detail pane with persistent transcripts and view state, named progress and accurate completion status.
...
Select the existing Better Auth integration when creating the Web Chat app:

```bash
pnpm dlx eve init my-agent --channel-web-nextjs --web-authentication sign-in-with-vercel
```

Register a Vercel OAuth application and configure `BETTER_AUTH_SECRET`,
`VERCEL_APP_CLIENT_ID`, and `VERCEL_APP_CLIENT_SECRET`. Better Auth verifies browser
sessions; deployment OIDC remains a separate authentication path. This option
adds sign-in, not a user-scoped session index or cross-device history. It requires
no application database or migrations. Custom session listing and resource access
policies remain application responsibilities.
