...
`eve/next` lets a Next.js app own the public routes for one or more eve agents. Wrap your config with `withEve()` to run the app and agents from one development command and deploy them as one Vercel project. [`useEveAgent`](./overview) finds the mounted routes on its own, so there's no CORS to configure and no URL env vars to keep in sync.

Use `withEve()` for a Next.js app that is the browser-facing routing layer. Use `withEve({ agents })` when that app routes to several named agents. See [Add a Web App](./add-a-web-app) to choose an application arrangement. When the app is one independently deployed service among agents, APIs, or workers, use a Vercel peer-service graph in [`vercel.ts`](../deployment/vercel#compose-agents-with-other-vercel-services) instead.
...
- **Non-Vercel hosts.** `withEve()` retains the Next.js rewrite interface, but it does not generate a host-neutral service graph. By default, `next start` runs each built eve service on its derived loopback port and proxies its routes. For a host that must supervise the eve services itself, run each service separately and set `EVE_NEXT_PRODUCTION_ORIGIN` to the origin that serves its private route prefix:

  ```bash
  EVE_NEXT_PRODUCTION_ORIGIN=https://agent.example.com npm run build
  ```

  For a self-hosted multi-agent application, configure the agent services and the production routing layer together. If the public routing layer mounts an agent below a path, set `EVE_PUBLIC_ROUTE_PREFIX` in that agent's build and runtime environments so callback URLs use the public path. See [Add a Web App](./add-a-web-app#use-another-framework).
