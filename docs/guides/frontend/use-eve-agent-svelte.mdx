...
| `events`  | `readonly MessageStreamEvent[]`               | Raw server events for this session.                                       |
| `session` | `ClientSessionState \| undefined`             | Fixed session ID and stream cursor.                                       |
...
A tool opts into approval with `approval` ([Tools](../../tools)). When one fires, the pending request rides along on a `dynamic-tool` part at `part.toolMetadata?.eve?.inputRequest`. Scan every message because an unrelated turn can add newer messages while the approval stays open, then answer through the same session with `agent.respond(inputResponses)`:
