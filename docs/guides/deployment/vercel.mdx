...
A Vercel project can use only one configuration source, so remove `vercel.json` when adopting `vercel.ts`. Keep `eve` in the project dependencies so Vercel can import `eve/vercel` while evaluating the configuration. `withEve` discovers the project containing `vercel.ts`, including when a monorepo config is compiled into that project's temporary `.vercel` directory and evaluated from the repository root. Pass `{ root: "/absolute/workspace/path" }` as the second argument only when you need to select a different eve project root.
