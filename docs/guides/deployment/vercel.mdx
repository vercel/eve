...
Run `eve dev` from the workspace root to start every agent behind one local Vercel service router. Starting the complete graph lets agents declared with `defineWorkspaceAgent` call their peers through the same public paths used by a deployment. eve asks which agent to open in the interactive TUI; the selection does not limit which agents run.

```bash
npm exec eve dev
```

The command requires Vercel CLI 59.16.0 or newer, but not a linked Vercel project: eve runs `vercel dev --local`. If the installed CLI is older, an interactive terminal offers to run the Vercel CLI's native upgrader and continue. In a non-interactive terminal, upgrade with `vercel upgrade` before retrying.

Without a root `vercel.json` or `vercel.ts`, eve writes an invocation-local service graph for the workspace and removes it when development stops. When either configuration file exists, Vercel uses that authored configuration unchanged. Each agent is available at `http://localhost:3000/<agent>/`. Pass `--agent <name>` to choose the TUI agent without a prompt, or pass `--no-ui` to run the graph without selecting an agent. Use `--host` or `--port` to select the local router address. Run `eve dev` from an agent directory to develop only that agent; default `defineWorkspaceAgent` calls require the complete graph started from the workspace root.
