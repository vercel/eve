...
Use an agent workspace when several agents form one operational unit. The root owns the shared environment and deployment, and peer services serve their agents at `/eve/agents/support/eve/v1/*` and `/eve/agents/research/eve/v1/*`. Members can expose selected peers as [workspace subagents](../../subagents#workspace-subagents) with `defineWorkspaceSubagents()` and `vercelWorkspaceTarget()`.
