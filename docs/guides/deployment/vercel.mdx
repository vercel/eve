...
Run Vercel CLI 59.16.0 or newer with `vercel dev --local` to start the complete service graph locally. A generated peer-service Web Chat project exposes this as `pnpm dev` and `pnpm dev:services`. Use `pnpm dev:web` only when you want the frontend without its agent services, or `pnpm dev:eve` when you want one agent and the eve terminal UI. Named agents use the same `/eve/<name>/v1/*` routes in development, and default `defineWorkspaceAgent` transports can call peers without deployment credentials.
