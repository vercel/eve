...
Run Vercel CLI 59.16.0 or newer with `vercel dev` to start the complete service graph locally. A generated Vercel Web Chat project exposes this as `pnpm dev:services`; the first run links the directory when needed and linked projects load their development environment and settings. Use `pnpm dev` or `pnpm dev:eve` when you want one agent and the eve terminal UI, or `pnpm dev:web` when you want the frontend without its agent services. Named agents use the same `/eve/<name>/v1/*` routes in development, and default `defineWorkspaceAgent` transports can call peers without deployment credentials.
...
When `eve deploy` creates a new Vercel project, it configures trace sampling so traces appear in Agent Runs automatically. See [Agent Runs](../../observability/agent-runs) for details. Pass `--no-trace-sampling` to skip this setting.

After an interactive deploy to an existing project without sampling rules, eve
offers to enable tracing. Declining does not block deployment or prompt again
in the same local checkout. Non-interactive deploys leave existing projects'
sampling settings unchanged.
...
Use the **Agent Runs** tab under **Observability** to browse sessions and
inspect conversation traces. See [Agent Runs](../../observability/agent-runs)
for beta access, pricing, retention, and tracing setup.
