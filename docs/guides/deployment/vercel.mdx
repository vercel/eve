...
Add credentials for direct model providers, tools, and connections to the Vercel project environment. If your app includes Web Chat, either set `EVE_ACCESS_PASSWORD` and a random `BETTER_AUTH_SECRET` of at least 32 characters for the default shared-password sign-in, or replace the app's auth and wire your provider into the channel. If your app does not include Web Chat, replace `placeholderAuth()` with a real `AuthFn` before a browser sends a production request. See [route authentication](../auth-and-route-protection) for the available policies.
