...
If one Next.js app should own same-origin browser routes to one or more agents, use [`withEve()` in `next.config.ts`](../frontend/nextjs) instead. See [Project structures](../../project-structures#vercel-peer-services) for the layouts and decision guide.
