...
Run `eve dev` from the workspace root to start every agent behind one local Vercel service router. eve asks which agent to open in the interactive TUI; its public endpoint is `http://localhost:3000/<agent>/`.

```bash
npm exec eve dev
```

The command requires the Vercel CLI, but not a linked Vercel project: eve runs `vercel dev --local`. Without a root `vercel.json` or `vercel.ts`, eve writes an invocation-local service graph for the workspace and removes it when development stops. When either configuration file exists, Vercel uses that authored configuration unchanged. Pass `--agent <name>` to choose the TUI agent without a prompt, and pass `--host` or `--port` to select the local router address. Run `eve dev` from an agent directory to develop one agent without starting the whole graph.
