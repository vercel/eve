...
An agent workspace contains several root agents that users, channels, or schedules can address independently. To create and manage this layout, see [Project Structure](../../getting-started/project-structure).

For an agent-only workspace, run `eve build` from the workspace root. eve finds direct `agents/<name>/` children without their own `package.json`, then generates an independently built service and a `/<name>/eve/v1/*` transport route for each one. A child with its own `package.json` is a separate package rather than a workspace member.

### Compose agents with other Vercel services

Use Vercel's programmatic configuration when agents, a frontend, private API, worker, or another application are peer services that Vercel builds and deploys independently. In this arrangement, Vercel's service graph handles routing instead of a Next.js app. Replace `vercel.json` with a root `vercel.ts`, then compose the service graph with `withEve`:

If a single Next.js app should serve same-origin browser routes for one or more agents, use [`withEve()` in `next.config.ts`](../frontend/nextjs) instead.
