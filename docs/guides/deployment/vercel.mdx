...
When `eve deploy` creates a new Vercel project, it configures trace sampling to 100% for all environments. Pass `--no-trace-sampling` to skip this setting. Existing linked projects are unchanged. Trace volume may incur Always-on Tracing charges; see [Agent Runs](../../observability/agent-runs) for details.
