...
The command links an existing project or creates one, then pulls its environment variables. Without a terminal — in continuous integration (CI), or from a coding agent — name the project instead of picking it:
...
Without a terminal, confirm the production deploy up front. `--project` links first, so a new project needs no separate `eve link`:
