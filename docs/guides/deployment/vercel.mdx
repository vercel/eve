...
An authored eve service routed at `/eve/v1` already uses the protocol path; callbacks remain at `/eve/v1/callback/*`. A named mount such as `/eve/agents/support` adds that mount before the protocol path, giving `/eve/agents/support/eve/v1/callback/*`.
...
```bash
curl https://your_agent.vercel.app/eve/agents/support/eve/v1/health
eve dev https://your_agent.vercel.app/eve/agents/support
```
