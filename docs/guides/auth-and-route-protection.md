...
`assertion.principal` holds the `current` and `initiator` contexts the request would install, already stamped with `eve:forwarded-by`. `initiator` equals `current` when the sender omits it. Attributes are asserted too, so check any attribute your tools trust. `assertion.principal` is absent when the predicate decides parent session lineage for a request that forwards no principal; a predicate that requires it accepts that forwarder's lineage only alongside a principal it accepts. The `ForwardedAssertion` and `TrustedForwarders` types are exported from `eve/channels/eve` for named predicates.
