...
configured to collect traces. After an interactive `eve deploy` to an existing
project with no sampling rules, eve offers to enable 100% sampling for all
environments and paths. This affects all project traffic, not only eve requests,
and may incur tracing charges. If you decline, eve remembers that choice in
the local, gitignored `.eve/trace-sampling.json` and does not ask again for that
project in this checkout. A fresh checkout or another teammate may see the
offer once. Existing rules are left unchanged.

Non-interactive deploys do not prompt or change sampling on existing projects.
Pass `--no-trace-sampling` to skip sampling setup for this deployment.
To enable tracing manually, select your team and project, then open
