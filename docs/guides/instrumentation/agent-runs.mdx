...
`eve deploy` enables 100% sampling for new projects and offers it on existing
projects without rules. Sampling is project-wide and may incur charges; declines
are remembered locally. Non-interactive deploys skip the offer, and
`--no-trace-sampling` skips setup.
...
Tracing starts as soon as you save the rule.

## Configure Agent Runs

Preview and production deployments export to Vercel Agent Runs by default.
Omit `agent/instrumentation/agent-runs.ts` to keep that default. To change it,
default-export `agentRuns(...)` from that file, or default-export
`disableInstrumentation()` to turn off Agent Runs export.
