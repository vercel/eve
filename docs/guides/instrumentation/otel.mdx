...
A deployed eve project needs at least one
[sampling rule with a rate above 0%](https://vercel.com/docs/tracing/always-on-tracing#enable-always-on-tracing)
before Vercel collects traces. We recommend setting the rate to **100%** for
eve projects so every matching request is traced.

Select your team and project, then open
...
### Use classification in an export policy

When one `defineInstrumentation()` file declares `classificationPolicy`, eve
projects its trace and event results into OpenTelemetry context. Both
`exportPolicy` callbacks receive the active classification:
  ...
  exportPolicy: {
    span: ({ classification }) =>
      classification === "restricted"
        ...
        : { emit: true },
    attribute: ({ key, span }) =>
      span.classification === "sensitive" && key === "customer.id"
...
`span` reads `classification` directly. `attribute` reads the same value
through `span.classification`. Spans mapped from a lifecycle event use that
event's classification; child model and tool spans inherit the active
classification; structural spans fall back to the trace classification.

The destination helpers do not accept `classificationPolicy` or another
`tracePolicy`. They consume the result prepared by the instrumentation
runtime, and the process-wide `otel({ tracePolicy })` remains the earlier
capture ceiling.
