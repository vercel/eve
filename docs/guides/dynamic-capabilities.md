...
The returned local or remote definition can also choose `background` for that selection. Background selections require `experimental.tasks: true` on the root. The returned remote definition can change its URL, path, headers, auth,
...
The resolved set applies to local and remote direct delegation. Blocking selections also apply to the
`Workflow` tool; background selections are not currently exposed inside Workflow programs. eve
