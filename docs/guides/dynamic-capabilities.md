...
The resolved set applies to local and remote direct delegation. When the root enables [dynamic workflows](../tools/dynamic-workflows), the `workflow` tool also receives the subagents visible for that model step. Those subagents remain available as direct model tools; dynamic workflows add an orchestration path rather than replacing direct delegation. eve
