...
To require structured output, call the remote agent from an authored workflow tool with `ctx.agent(name, { message, outputSchema })`. The schema applies to that turn, the structured value is the call's result, and the remote child remains available for follow-up messages. See [Subagents](../subagents) for continuation behavior.
...
When the parent supports human input, remote workflow tools can use `ctx.ask()` to send questions through the parent session, including after the initiating turn settles. Concurrent questions remain independently answerable: respond to each request ID with `parentSession.respond()`. Answers return through the remote session to the workflow that asked the question.
