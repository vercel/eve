...
Trust is explicit on both sides. The receiver names which transport callers may assert delegated lineage or a forwarded principal with `eveChannel({ trustedForwarders })` (see [Auth & route protection](./auth-and-route-protection#accepting-forwarded-identity-from-another-deployment)). Without that authorization, eve retries session creation without parent lineage or child trace coordinates. The remote call still runs, but as a normal root session with the root token cap and without a linked delegated trace. A forwarded principal remains fail-closed: if `forwardPrincipal: true` is set, refusing the forwarder rejects the dispatch rather than silently changing caller authority.

For compatibility, legacy callback requests that carry only `traceparent` may still continue that trace. They cannot assert eve parent lineage, change `rootSessionId`, or remove the normal root-session token cap.
