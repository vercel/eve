...
| `experimental` | `AgentExperimentalDefinition`           | unset            | Unstable opt-ins. `codeMode` enables the [`code_mode`](./tools/code-mode) tool; `workflow.world` selects the Workflow world package on the root agent; `workflow.modelCallsPerStep` batches sequential model calls into a wider replay unit.                           |
