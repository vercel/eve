...
| `experimental` | `{ codeMode?: boolean; tasks?: boolean; workflow?: { world?: string } }` | unset            | Opt-in settings that can change or disappear in any release. `codeMode: true` adds one synchronous `code_mode` orchestration tool alongside ordinary direct tools. Request-visible typed inline tools with executable input/output validation enter its capped progressive catalog automatically, including dynamically resolved tools. When `tasks: true` is also enabled, subagent tools may launch up to eight background tasks per program and return working receipts; their final results arrive later. Approval-gated, arbitrary background, provider-managed, control-plane, and foreground delegation tools remain direct only. Code mode follows ordinary tool retry semantics. `workflow.world` selects the Workflow world package backing sessions, queues, and hooks. |
