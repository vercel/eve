...
`model` accepts a gateway model id string, which routes through the [Vercel AI Gateway](https://vercel.com/docs/ai-gateway). To configure a model outside AI Gateway, pass a provider-authored `LanguageModel`.
...
| `experimental` | `{ useCodexSubscription?: boolean; workflow?: { world?: string } }` | unset            | Opt-in settings that can change or disappear in any release. Treat them as unstable. [`useCodexSubscription`](./guides/using-model-subscriptions) uses local Codex login state for OpenAI string models during development; production builds keep the model on its normal deployable route. `workflow.world` selects the Workflow world package backing session state, queues, hooks, and streams on the root agent. |
