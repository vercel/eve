...
| `inputSchema`  | Standard Schema or a JSON Schema object | none             | Custom input schema for the lowered subagent tool. Only meaningful on a declared subagent; ignored on the root agent. See [Subagents](./subagents#custom-input-schema).                                                                            |
| `formatInput`  | `SubagentInputFormatter`                | none             | Synchronous function rendering the validated subagent tool input into the delegated caller message. Only meaningful on a declared subagent; ignored on the root agent.                                                                             |
