...
| `timeout`      | `number \| false`                       | `7_200_000`      | Time limit for each call of this agent as a subagent, in milliseconds of active time; see [Subagents](./subagents#what-the-parent-sees). A call still working at the limit fails with `TIMED_OUT`. On the root agent, applies to the built-in `agent` tool. `false` removes the limit. |
