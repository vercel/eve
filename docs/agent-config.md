...
| `experimental` | `{ instrumentationProviders?: boolean; subagentPersistentSessions?: boolean; tasks?: boolean; workflow?: { world?: string } }` | unset            | Opt-in settings that can change or disappear in any release. `tasks` enables [background tasks](/docs/tools/background-tasks) on the root agent; `subagentPersistentSessions` keeps delegated children resumable; `instrumentationProviders` selects the provider-directory layout; `workflow.world` selects the Workflow world package. |
