...
The root `agent.ts` can be omitted when no runtime config is needed. eve then selects its default `agent.ts` source at the same slot, configured with `xai/glm-5.3-flash`; authoring the file replaces that source.
