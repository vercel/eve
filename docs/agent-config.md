...
| `experimental` | `{ tasks?: boolean; workflow?: { world?: string } }` | unset            | Opt-in settings that can change or disappear in any release. `tasks` enables durable background tasks and task controls; subagent-specific helpers still choose execution per definition. `workflow.world` selects the Workflow world package on the root agent. |
