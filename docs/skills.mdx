...
Supporting files are for the model. After loading a skill, the model reads its references, assets, and scripts with sandbox filesystem tools such as `read_file` and `bash`. Tools and hooks have no skill file API; when your code needs the same data, import it from a module in `lib/` so it ships with the agent.
...
Subagent lifecycle events do not re-run skill resolvers. The parent's resolved dynamic skills remain available when it resumes after delegation.
