...
The handle exposes the skill's `name` and `file(relativePath)`; file content is read lazily from the active sandbox, relative to that skill package directory. For a dynamic skill, `file("SKILL.md")` returns the resolved markdown from session state without starting a sandbox.
