...
In an interactive root session, a steering message that arrives while the turn waits on subagent calls or workflow tools moves those calls to the background, and the turn answers the message right away. Their results arrive later as task results. Subagent sessions and turns started by a schedule apply the message after the wait instead. See [Detach a waited call](/docs/tools/workflows#detach-a-waited-call).
