...
## Compose built-in channel events

Built-in channels can provide default event handlers for delivery, progress, or setup behavior. A bare handler in a channel's `events` map runs after the corresponding default and both handlers are awaited in order. Use the helpers from `eve/channels` when you need different behavior:

```ts
import { beforeDefault, replaceDefault } from "eve/channels";

const events = {
  "turn.started": beforeDefault(async (event, channel, ctx) => {
    // Run before the built-in handler.
  }),
  "message.completed": replaceDefault(async (event, channel, ctx) => {
    // Take full ownership; the built-in handler will not run.
  }),
};
```

`afterDefault(handler)` is the explicit equivalent of a bare handler. If the first composed handler throws or rejects, the second does not run.
