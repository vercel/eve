...
Subagent calls and most workflow tool calls run as detached tasks, so a steering message never stops them. A steering message from the turn's own principal ends the turn's waits and attached calls, such as `sleep`, and the model answers the message right away; the tasks' results arrive later in the same turn. A message from another person waits until the turn ends. See [Steering during a turn](/docs/concepts/tasks#steering-during-a-turn).

Set `turnPolicy: "queue"` on any built-in or custom channel when each turn must finish before the next message starts, including waiting for its subagent calls and workflow tools:
...
The eve channel is the framework's default HTTP session API, the routes the terminal UI, [`useEveAgent`](../guides/frontend/overview), and `curl` all talk to. Its selected `channels/eve.ts` source owns health, inspection, callbacks, and the session protocol as one replaceable surface. eve supplies the source when no `agent/channels/eve.ts` file exists; author that file to replace it, most often to change route auth. See [HTTP channel](./eve) for routes, auth, replacement, and disablement.
