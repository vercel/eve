...
Channels default to `turnPolicy: "steer"`. When an accepted message arrives while a turn is active, eve buffers it and applies it at the next committed workflow boundary. The current model call and tools complete safely; the message continues the same turn with the same turn ID.
