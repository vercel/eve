...
Channels default to `turnPolicy: "steer"`. When an accepted message arrives while a turn is active, eve adds it at the next safe boundary. Use `turnPolicy: "interrupt"` to stop the active turn and start a replacement with a new turn ID. Interruption emits `turn.interrupted` without an idle boundary before the replacement. Output already streamed and completed side effects are not rolled back.
