...
Messages default to `turnPolicy: "steer"`, adding input at the active turn's next safe boundary. Use `"interrupt"` to stop that turn and start a replacement. Set `turnPolicy: "queue"` when the active turn should finish first:
...
Interruption emits `turn.interrupted`, and the replacement message starts a new turn with a new turn ID without an idle boundary. Partial output and completed side effects from the interrupted turn are not rolled back. If no turn is active, the message is sent normally. You can set the policy once on `chatSdkChannel({ turnPolicy })` or override it per `send(...)`.
