...
Messages default to `turnPolicy: "steer"`: a message sent while an eve turn is active is buffered, then applied to the same turn at the next committed workflow boundary. Set `turnPolicy: "queue"` when the active turn should finish first:
...
Steering preserves the turn ID and lets the current model call and tools finish. If no turn is active, the message starts a turn normally. You can set the policy once on `chatSdkChannel({ turnPolicy })` or override it per `send(...)`.
