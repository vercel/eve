...
`send({ message, thread, ... })` starts or resumes the eve session. The `thread` determines the continuation token and persisted channel state, so replies land back on the originating thread:

```ts
bot.onNewMention(async (thread, message) => {
  await send({ message: message.text, thread, title: "Support request" });
});
```

The input accepts `{ message?, inputResponses?, context?, outputSchema?, thread, auth?, title?, mode?, callback?, adapterName?, turnPolicy? }`. `title` sets the eve session's display title without changing the model message; `auth` attaches an authenticated principal to the turn.
...
Start a session without an inbound webhook through `send(channel, { message, target, auth })` from a schedule `run` handler, or `args.receive(channel, ...)` from another channel. The target is a serialized Chat SDK thread, or `{ threadId, adapterName }` when you only have a provider-native thread id:
...
The `message` field takes plain text or an AI SDK `UserContent` array. To forward a Chat SDK message's attachments, convert it with `messageToUserContent`, which returns `message.text` when there are no attachments and a `UserContent` array (text plus one file part per attachment URL) when there are:

```ts
import { messageToUserContent } from "eve/channels/chat-sdk";

bot.onNewMention(async (thread, message) => {
  await send({ message: messageToUserContent(message), thread });
});
```
