...
Use `attachSession()` when you already have a durable session ID and must target that exact session. Attaching performs no network or runtime lookup; the first operation reports whether the session is still active:

```ts
POST("/sessions/:sessionId/clear", async (_request, { attachSession, params }) => {
  return Response.json(await attachSession(params.sessionId).clear());
});
```

The fixed session handle exposes `send`, `cancel`, `compact`, `clear`, `reset`, and `stream`. It never creates or follows another session.
...
`WS()` handlers receive the same helpers as HTTP route handlers: `send`, `cancel`, `clear`, `compact`, `reset`, `getSession`, `receive`, `params`, `waitUntil`, and `requestIp`. The returned hooks are eve-owned structural types compatible with Nitro/H3 websocket routing, including `upgrade`, `open`, `message`, `close`, and `error`.
