...
Handle `task.started` and `task.settled` to observe the subagent calls a session
delegates. [Sessions, runs and streaming](../concepts/sessions-runs-and-streaming#stream-a-session)
