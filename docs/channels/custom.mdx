...
- `reset` queues terminal retirement for the current session and returns once the
  durable inbox accepts it. It does not wait for address ownership to be released.
  After retirement, an address `send()` creates a fresh session; a fixed `Session`
  handle remains pinned to the retired ID.
