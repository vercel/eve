...
Cold-start sends wait until the winning session owns the address and publishes its
resource descriptor while holding first-turn ownership. If simultaneous sends race, eve forwards the losing
candidate's message to the winner and returns that canonical session handle.
...
Message sends use the channel's `turnPolicy`, which defaults to `"steer"` and adds input at the active turn's next safe boundary. Use `"interrupt"` to stop that turn and start a replacement, or `"queue"` when the active turn should finish first. Configure the policy on `defineChannel(...)` or override one send:
...
Attaching does no lookup. Each operation resolves the session when it dispatches.
An accepted message may still be retired if terminal settlement wins the race;
read the stream for its outcome. Call `resolveSession(address)` when you need an
...
Control operations never create a session. A missing session lookup returns a
no-active status. Controls return once their candidates are accepted and report
their outcomes on the stream. Wait for terminal session output before expecting
a reset address to start a fresh session. Authenticate and deduplicate command webhooks before
...
Re-keying adds another address for the current session. `reset` is different: it terminally retires the current session and makes its existing address available to a later `send()`. `cancel` is narrower still: it stops only the active turn and leaves the session, history, and continuation-token ownership intact.
...
At the next workflow boundary, the holder claims the new alias and retains the existing aliases. If another active session already owns the new token, re-keying fails instead of taking it over. After a successful re-key, both old and new tokens resolve to the same session until it terminates.
