...
A route's `path` is its app URL; the channel filename does not prefix it. The
route above answers at `POST /threads/:threadId/messages`, not
`/support/threads/:threadId/messages`. Use a unique method-and-path pair for
each route, and do not use the framework-owned `/eve/v1/*` namespace.
