...
- `attachSession(sessionId)` creates an I/O-free handle pinned to one durable session ID. Its `send()`, control, and stream methods never follow a replacement session.
...
- `clear({ continuationToken })` removes the model-message history while preserving the session and its durable resources. See [Clear session context](#clear-session-context).
- `compact({ continuationToken })` queues context compaction for the session that owns the token without sending model input. See [Compact a session](#compact-a-session).
...
## Fixed session handles

Use `attachSession(sessionId)` when you already have a durable session ID and must target that exact session. Attaching performs no I/O and never creates or follows a replacement session:

```ts
POST("/control/:sessionId/clear", async (_request, { attachSession, params }) => {
  const session = attachSession(params.sessionId);
  return Response.json(await session.clear());
});
```

The handle keeps every operation next to the ID it targets. The calls below show the available shapes independently:

```ts
const session = attachSession(sessionId);

await session.send({ auth: null, message: "Continue the analysis." });
...
`send()` reports `session_not_active` instead of creating a session when the ID is unknown or terminal. Control methods are benign for an inactive ID. After `reset()`, the handle remains pinned to the retired ID.
...
Call `clear` with the channel-local continuation token to remove a session's durable model-message history without replacing the session:
...
export default defineChannel({
  routes: [
    POST("/threads/:threadId/clear", async (_req, { clear, params }) => {
...
`"accepted"` means the clear was queued. The stream emits `context.cleared` followed by `session.waiting`, after which the next ordinary message resumes the same session with no prior model messages. The system prompt, configured tools and skills, durable state, limits, continuation-token ownership, and session-scoped sandbox remain. `"no_active_session"` means the token has no current owner.

## Compact a session

Call `compact` with the channel-local continuation token to summarize durable conversation history without delivering a command to the model:
...
export default defineChannel({
  routes: [
    POST("/threads/:threadId/compact", async (_req, { compact, params }) => {
...
`"accepted"` means the durable session queued the operation. A successful compaction emits `compaction.requested` followed by `compaction.completed`, then the operation returns through `session.waiting`. If summarization fails, the stream returns through `session.waiting` without `compaction.completed` and preserves the previous history. If a turn is active, eve runs the compaction after that turn settles; it does not cancel the turn. `"no_active_session"` means the token has no current owner. Both outcomes are successful, and compaction never starts a session or appends a synthetic user message.
...
`WS()` handlers receive the same helpers as HTTP route handlers: `attachSession`, `send`, `cancel`, `clear`, `compact`, `reset`, `getSession`, `receive`, `params`, `waitUntil`, and `requestIp`. The returned hooks are eve-owned structural types compatible with Nitro/H3 websocket routing, including `upgrade`, `open`, `message`, `close`, and `error`.
