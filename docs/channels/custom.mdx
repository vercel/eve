...
Handle `task.started` and `task.settled` to observe the agent calls and workflow
tool calls a session runs as tasks; `task.started.data.kind` tells them apart.
[Sessions, runs and streaming](../concepts/sessions-runs-and-streaming#stream-a-session)
