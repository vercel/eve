...
contains `sessionId` directly. A `step.failed` handler can receive a nonterminal scheduled recovery in `eventData.details.recovery`; built-in channels use it to post an automatic-retry notice. Declaring `events["step.failed"]` replaces that built-in notice for the event.
