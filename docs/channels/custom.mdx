...
- `appendHistory` adds validated history only while the session is between turns,
  including before an idle session's first send. An active session returns a typed
  `session_busy` error; it does not queue or reconcile the append.
