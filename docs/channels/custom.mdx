...
`rekey()` when this channel needs to add another address. `session.failed`
...
When another identity should address a session, add it with `channel.continuation?.rekey(rawToken)`. The runtime preserves the current channel namespace and exposes the new token as `channel.continuation.token` for later handlers.

Re-keying adds an address to the current session. Earlier addresses continue to resolve to the same session. `reset` is different: it terminally retires the current session and makes all its addresses available to later `send()` calls. `cancel` is narrower still: it stops only the active turn and leaves the session, history, and continuation-token ownership intact.
...
At the next workflow boundary, the runtime adds the new address to the session's merged inbox. If another active session already owns the new token, the re-keying session fails instead of taking it over. Every address claimed by the session remains active, including across deployment handoffs, so senders can continue using an earlier token until the session ends or resets.
