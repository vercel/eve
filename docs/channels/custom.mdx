...
Use a channel event handler, rather than an agent hook, when the reaction needs native platform context. For example, a GitHub or Slack channel can handle `"subagent.called"` and `"subagent.completed"` to correlate child work by `callId` and update the same issue, pull request, or thread when a reviewer subagent finishes.
