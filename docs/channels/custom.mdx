...
- `reset` terminally retires the current session. If the workflow cannot consume
  the reset command and release its inbox, eve directly cancels the run so the
  address can be reclaimed without replaying that session. This fallback cannot
  run the session's normal finalizers. A later address `send()` creates a fresh
  session; a fixed `Session` handle remains pinned to the retired ID.
