...
Message sends use the channel's `turnPolicy`, which defaults to `"steer"`. An accepted message arriving during an active turn is buffered and applied at the next committed workflow boundary. The active model call and tools finish safely; the message joins the same turn and keeps its turn ID. Configure `turnPolicy: "queue"` on `defineChannel(...)` when messages should wait for the active turn to finish, or override one send:
...
Use `audience(input)` to classify who can observe the conversation. The hook runs after route authentication and before the session is created:
...
The input contains the channel's `state`, authenticated `auth` principal, `channel`, `mode`, and `environment`. Return `"public"`, `"private"`, or `"unknown"`. Return `"unknown"` when the channel does not have enough evidence to classify confidently; consumers treat it as non-public. A missing, throwing, asynchronous, or malformed classifier also fails closed to `"unknown"`.
...
`metadata(state)` remains a channel-owned projection of custom observability fields. It no longer carries a reserved `audience` key. Channel extensions built before channel epoch 20 can still use that key; eve accepts it as a deprecated fallback and warns once, then authors should move classification to `audience(input)`.
...
At the next workflow boundary, the runtime adds the new address to the session's merged inbox. If another active session already owns the new token, the aliasing session fails instead of taking it over. Every address claimed by the session remains active, including across deployment handoffs, so senders can continue using an earlier token until the session ends or resets.
