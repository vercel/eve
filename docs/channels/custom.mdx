...
Bind the platform's conversation identity once, then perform every operation on
...
export default defineChannel({
  routes: [
    POST("/threads/:threadId/messages", async (request, { channelAddress, params }) => {
      ...
      const thread = channelAddress(params.threadId);

      if (body.message === "/new") {
        return Response.json(await thread.reset({ reason: "User requested /new" }));
      }

      const session = await thread.send(body.message, { auth: null });
      return Response.json({ sessionId: session.id });
    }),

    POST("/threads/:threadId/cancel", async (_request, { channelAddress, params }) =>
      Response.json(await channelAddress(params.threadId).cancel()),
    ),
    POST("/threads/:threadId/compact", async (_request, { channelAddress, params }) =>
      Response.json(await channelAddress(params.threadId).compact()),
    ),
    POST("/threads/:threadId/clear", async (_request, { channelAddress, params }) =>
      Response.json(await channelAddress(params.threadId).clear()),
    ),
  ...
  events: {
    "message.completed"(event, channel) {
      console.log(channel.session.id, event.message);
    },
  },
});
```
...
- `channelAddress(continuationToken)` binds a channel-local conversation address.
- `attachSession(sessionId)` creates an I/O-free handle pinned to one durable session ID.
- `receive(channel, options)` hands work to another authored channel.
- `params`, `waitUntil`, and `requestIp` provide request metadata and lifetime control.

Event handlers receive `(eventData, channel, ctx)`. `channel.session.id` identifies
...
## Address and session handles

`ChannelAddress` is dynamic: every call targets whichever session currently owns
the address. Only `send()` can create a session when the address is unowned.

```ts
const thread = channelAddress(threadId);

const session = await thread.send("Hello", { auth });
await thread.cancel({ turnId });
await thread.compact();
await thread.clear();
await thread.reset({ reason: "Start over" });

const currentSession = await thread.resolveSession();
```
...
await session.send({ auth, message: "Follow up" });
...
Call `resolveSession()` only when you explicitly need to snapshot an address's
current owner as a fixed handle.
...
export default defineChannel({
  routes: [
    WS("/voice/ws", async (_req, { channelAddress }) => ({
      async message(_peer, message) {
        await channelAddress("voice-demo").send(message.text(), { auth: null });
      },
    })),
  ],
});
...
`WS()` handlers receive the same `channelAddress`, `attachSession`, `receive`,
...
- The target channel's authored `receive(input, { channelAddress })` hook owns the continuation-token format and initial state. Callers supply only `{ message, target, auth }`.
...
export default defineChannel({
  ...
  routes: [
    POST("/start", async (req, { channelAddress }) => {
      const body = await req.json();
      await channelAddress(body.token).send(body.message, {
        auth: null,
...
Each `channelAddress(rawToken)` call uses a channel-local token. The framework prepends the channel name, derived from the file stem under `agent/channels/`, before handing the token to the runtime.
...
`ChannelAddress.send()` and `Session.send()` accept `string | UserContent`. To include file attachments, pass a `UserContent` array mixing text and file parts:

```ts
await channelAddress(continuationToken).send(
  [
...
defineChannel({
  ...
  routes: [
    POST("/webhook", async (req, { channelAddress }) => {
      await channelAddress(continuationToken).send(
        [
