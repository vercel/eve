...
export default defineChannel({
  routes: [
    POST("/threads/:threadId/messages", async (request, { params, reset, send }) => {
      const body = await request.json();

      if (body.message === "/new") {
        return Response.json(await reset(params.threadId, { reason: "User requested /new" }));
      }

      const session = await send(params.threadId, { auth: null, message: body.message });
...
- `send(address, input)`, `cancel(address, options?)`, `compact(address)`, `clear(address)`, `reset(address, options?)`, and `resolveSession(address)` target a channel-local continuation address.
...
```ts
const session = await send(threadId, { auth, message: "Hello" });
await cancel(threadId, { turnId });
await compact(threadId);
await clear(threadId);
await reset(threadId, { reason: "Start over" });
  ...
  routes: [
    WS("/voice/ws", async (_req, { send }) => ({
      async message(_peer, message) {
        await send("voice-demo", { auth: null, message: message.text() });
      },
    })),
  ],
});
...
`WS()` handlers receive the same channel operations, `attachSession`, `receive`,
...
`send()` accepts a `message` containing `string | UserContent`, while
