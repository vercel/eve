...
A turn cannot end while tasks it started are working. The model often says
something before it waits, such as that work started, and that text streams as
an ordinary `message.completed`, followed later by the reply to the results.
The built-in channels post both. In an interactive root session, a held turn
also emits `session.waiting` before its `turn.completed`, so treat
`turn.completed`, not `session.waiting`, as the end of a turn. See
[Held turns](../concepts/sessions-runs-and-streaming#held-turns).
