...
This mounts `POST /eve/v1/discord/gateway` in addition to the interaction route. Gateway support does not require Vercel Connect: you can instead pass `{ applicationId, botToken, publicKey }` through `credentials`, or use the matching `DISCORD_*` environment variables. The Discord application ID namespaces conversations and must match the relay's signed application identity. Omitting `gateway` leaves the route unmounted and preserves interaction-only routing.
