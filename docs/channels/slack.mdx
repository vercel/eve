...
Use `onBlockActions(interaction, ctx)` for non-HITL `block_actions` callbacks from messages, App Home, and modals. It runs once per callback and exposes the normalized actions under `interaction.actions`; each action includes the old `actionId`, `blockId`, `value`, `selectedOptionValue`, and `label` conveniences, plus `type` and the complete Slack action under `raw`.

Message-backed callbacks provide thread and eve session operations such as `cancel`, `reset`, and `send` under `ctx.message`. App Home and modal callbacks have no message context, so use the workspace-scoped `ctx.slack` handle instead.
...
export default slackChannel({
  async onBlockActions(interaction, ctx) {
    const action = interaction.actions.find(({ actionId }) => actionId === "stop");
    if (action && ctx.message) await ctx.message.cancel();
  },
});
```
...
Use `onInteraction(interaction, ctx)` as the raw fallback for signed interactive callbacks not consumed by eve's HITL handling, `onBlockActions`, or `onShortcut`. This includes custom `view_submission` callbacks, dynamic select `block_suggestion` requests, `view_closed`, shortcuts when `onShortcut` is absent, and interaction types added by Slack later. `block_actions` also fall through when `onBlockActions` is absent. Slash commands only reach `onSlashCommand`.

The handler receives the complete decoded Slack payload under `interaction.payload`, plus its `type` and normalized `user`, `teamId`, `installationTeamId`, and `enterpriseId` fields when Slack supplies them. The workspace-scoped `ctx.slack` handle resolves credentials from `installationTeamId`, which can differ from the actor's `teamId` in Slack Connect callbacks. Message-backed `block_actions` additionally provide `ctx.message` when they fall through.

The interaction handler runs inline because its response can acknowledge a modal submission or dynamic select request. Return a `Response` when Slack expects an acknowledgement body, or return nothing for an empty `200 OK`. Use `ctx.waitUntil(...)` for work that does not affect the acknowledgement:
...
export default slackChannel({
  async onInteraction(interaction, ctx) {
    if (interaction.type === "block_actions") {
      const triggerId = interaction.payload.trigger_id;
      ...
      await ctx.slack.request("views.open", {
        ...
        view: {
          ...
          title: { type: "plain_text", text: "Review hours" },
...
eve-owned HITL actions and submissions remain reserved and do not reach authored interaction hooks. `onBlockActions` takes precedence for block actions, and `onShortcut` takes precedence for shortcuts; callbacks fall through to `onInteraction` when the corresponding specific hook is absent. A `block_actions` callback containing any eve-owned action remains reserved even when it also contains custom actions.
...
Message hooks expose every current-owner operation directly on `ctx`:
...
These calls are independent examples; a handler normally chooses one. `ctx.send()` is the only method that can create a session when the thread is unowned. It derives Slack auth from the inbound user unless `auth` is supplied explicitly. Use `await ctx.resolveSession()` only when later work must stay pinned to the currently owning durable session ID. Message-backed interactions expose these operations under `ctx.message`; check that it is present before using them.
...
Reset returns `{ status: "reset", previousSessionId }` when the thread had a session, or `{ status: "no_active_session" }` when it was already free. Both outcomes are successful. Returning `null` after reset consumes the triggering message as a command; return `{ auth }` instead to deliver that message as the first turn of the fresh session. Message-backed `onBlockActions` and fallback `onInteraction` handlers receive the same operations under `ctx.message`, which is useful for a New conversation button.
