...
#### Raw interaction fallback

Use `onRawInteraction(interaction, ctx)` as the fallback for signed interactive callbacks that eve's HITL handling, `onInteraction`, or `onShortcut` does not claim. This includes App Home and modal `block_actions`, custom `view_submission` callbacks, dynamic select `block_suggestion` requests, `view_closed`, and interaction types added by Slack later. The fallback receives the complete decoded Slack payload under `interaction.payload`, plus its `type` and normalized `user`, `teamId`, `installationTeamId`, and `enterpriseId` fields when Slack supplies them. The workspace-scoped `ctx.slack` handle resolves credentials from `installationTeamId`, which can differ from the actor's `teamId` in Slack Connect callbacks.

The raw fallback runs inline because its response can acknowledge a modal submission or dynamic select request. Return a `Response` when Slack expects an acknowledgement body, or return nothing for an empty `200 OK`. Use `ctx.waitUntil(...)` for work that does not affect the acknowledgement:
...
Handler precedence matches the Events API fallback: a configured `onInteraction` claims message-backed `block_actions`, and a configured `onShortcut` claims shortcuts. Returning from either handler does not continue to `onRawInteraction`. eve-owned HITL actions and submissions remain reserved and do not reach the raw fallback.
