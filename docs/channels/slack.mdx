---
title: "Slack"
description: "Reach your agent from Slack app mentions and DMs, with thread anchoring, buttons, and managed or portable credentials."
type: integration
---

The Slack channel puts your agent inside a workspace. It answers `@mentions` and DMs, replies in threads, shows typing indicators, and turns human-in-the-loop (HITL) prompts into buttons. Use it when the conversation should happen where your team already works. [Vercel Connect](https://vercel.com/kb/guide/vercel-connect) can manage the outbound bot token and inbound webhook verification, or you can supply portable Slack credentials through environment variables. See [Channels](./overview) for the contract this builds on.
...
The command asks you to choose Vercel Connect or portable credentials, then scaffolds `agent/channels/slack.ts` and installs any required dependency.
...
Choose portable credentials when the agent does not run on Vercel or when you want to test against a public development or preview URL. The setup command adds `SLACK_BOT_TOKEN` and `SLACK_SIGNING_SECRET` to `.env.example`.

The portable flow does not create or install the Slack app. Configure it in [Slack app settings](https://api.slack.com/apps):
...
Private file downloads require the Slack bot token's `files:read` scope. Add the scope and reinstall the Slack app before sending attachments. If Slack returns an HTML sign-in page instead of file bytes, eve rejects the response with this remediation instead of staging the page as an attachment.
