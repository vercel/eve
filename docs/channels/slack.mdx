...
`bot` configures Slack's app and bot presentation. `eventSubscriptions` adds Slack Events API bot events, and `scopes` adds bot OAuth scopes. eve does not infer requirements from an open-ended `onEvent` handler, so add every event the handler accepts and the corresponding Slack scope. Scope changes may require reinstalling the Slack app.
