...
Accepted Slack messages steer by default: eve buffers the new message and applies it to the same turn at the next committed workflow boundary. Ignored mentions and messages rejected by your hooks never enter the turn. Set `turnPolicy: "queue"` when every active turn should finish:
