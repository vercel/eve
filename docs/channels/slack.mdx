...
A production build generates a Slack app manifest for each Slack channel, whether you use Vercel Connect or manage credentials yourself. `agent/channels/slack.ts` emits `channels/slack.slack-app-manifest.json` under `.output` locally or `.vercel/output` on Vercel. You can apply that file manually through Slack's app manifest tools; Connect-backed requirements reference the same file automatically.
