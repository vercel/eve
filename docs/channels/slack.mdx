...
A production build generates a Slack app manifest for each Slack channel, whether you use Vercel Connect or manage credentials yourself. `agent/channels/slack.ts` emits `channels/slack.slack-app-manifest.json` under `.output` locally or `.vercel/output` on Vercel. You can apply that file manually through Slack's app manifest tools. For Connect-backed channels, eve also embeds the Slack manifest in the Vercel Connect requirement with Connect-managed callback and trigger URLs.
...
`bot` configures Slack's app and bot presentation. `eventSubscriptions` adds Slack Events API bot events, `scopes` adds required bot OAuth scopes, and `optionalScopes` adds scopes that an installer may decline. A scope listed in both `scopes` and `optionalScopes` remains required. eve does not infer requirements from an open-ended `onEvent` handler, so add every event the handler accepts and the corresponding Slack scope. Scope changes may require reinstalling the Slack app.

By default, the generated manifest omits request URLs. Set `requestUrl` when you want a self-managed manifest that you can apply to Slack without adding the Event Subscriptions and Interactivity request URLs manually. Vercel Connect replaces these fields with its managed trigger URL during installation, so you can omit `requestUrl` for Connect-backed channels.
...
This option does not create or install the Slack app. To include your public webhook URL in its generated manifest, set `requestUrl`:
...
export default slackChannel({
  ...
  requestUrl: "https://your-agent.example/eve/v1/slack",
});
```

`requestUrl` populates both `settings.event_subscriptions.request_url` and `settings.interactivity.request_url`. Omit it when you prefer to configure those URLs in Slack or transform the manifest before applying it.

Configure the app in [Slack app settings](https://api.slack.com/apps):
