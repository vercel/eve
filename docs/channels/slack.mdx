...
To aggregate root and subagent activity without adding activity messages to the model conversation, opt into compact status:

```ts title="agent/channels/slack.ts"
import { slackChannel, slackActivityStatus } from "eve/channels/slack";
...
Compact status derives presentation from root turns, delegated work, tools, skills, and waits for approval, sign-in, or input. It prioritizes active waits, then the newest active action or delegated work, and falls back to `Working…` while root work remains active. The status clears when no represented work remains active and when the session's activity collector expires. Local and compatible remote lifecycle events use the same best-effort callback and do not start another parent turn. Action inputs and results are not included. Without `activity`, Slack keeps the default event-based typing behavior described above.
