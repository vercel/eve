...
Use `onInteraction(interaction, ctx)` for signed interactive callbacks not consumed by eve's HITL handling or the more specific `onShortcut` hook. This includes message, App Home, and modal `block_actions`; custom `view_submission` callbacks; dynamic select `block_suggestion` requests; `view_closed`; shortcuts when `onShortcut` is absent; and interaction types added by Slack later. Slash commands only reach `onSlashCommand`.

The handler receives the complete decoded Slack payload under `interaction.payload`, plus its `type` and normalized `user`, `teamId`, `installationTeamId`, and `enterpriseId` fields when Slack supplies them. The workspace-scoped `ctx.slack` handle resolves credentials from `installationTeamId`, which can differ from the actor's `teamId` in Slack Connect callbacks. For message-backed callbacks, `ctx.message` also provides thread operations and eve session operations such as `cancel`, `reset`, and `send`; it is absent for view-backed callbacks.
...
eve-owned HITL actions and submissions remain reserved and do not reach `onInteraction`. `onShortcut` takes precedence for shortcuts when configured; otherwise shortcuts fall through to `onInteraction`. A `block_actions` callback containing any eve-owned action remains reserved even when it also contains custom actions.
...
Reset returns `{ status: "reset", previousSessionId }` when the thread had a session, or `{ status: "no_active_session" }` when it was already free. Both outcomes are successful. Returning `null` after reset consumes the triggering message as a command; return `{ auth }` instead to deliver that message as the first turn of the fresh session. Message-backed `onInteraction` handlers receive the same operations under `ctx.message`, which is useful for a New conversation button.
