...
Set `privateToolApprovals` when tool inputs must be visible only to one reviewer. `"ephemeral"` keeps the preview and controls in the original thread but visible only to the reviewer. `"direct-message"` opens an IM with the bot and routes the response back to the original session; add the `im:write` bot scope. By default eve targets the Slack user who triggered the turn, or provide `reviewer` to resolve another user:

```ts title="agent/channels/slack.ts"
import { slackChannel } from "eve/channels/slack";

export default slackChannel({
  privateToolApprovals: {
    delivery: "direct-message",
    when: (request) => request.action.toolName === "review_answer",
    reviewer: (_request, ctx) =>
      String(ctx.session.auth.current?.attributes.reviewerUserId ?? "") || null,
  },
});
```

By default this setting applies to every tool approval on the channel; use `when` to select specific tools. Nonmatching approvals, questions, and session-limit prompts remain in the session thread. When `reviewer` is omitted, eve uses the Slack user who triggered the turn. Returning `null` from an authored resolver, or failing to resolve the triggering user, logs the undelivered request and posts no fallback, so private approval fails closed. The existing `onInputResponse` or tool approval response policy still decides whether the person who clicks may approve.
