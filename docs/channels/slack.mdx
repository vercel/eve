...
Activity renderers update after activity events by default. Set `activity.periodicRefreshIntervalMs` to re-render active activity on a timer in addition to those event-driven renders. Periodic refreshes stop when the snapshot has no running work, running actions, or blocked input:

```ts title="agent/channels/slack.ts"
import { experimental_slackActivityStatus, slackChannel } from "eve/channels/slack";
...
Use periodic refreshes when a custom renderer displays elapsed time or another time-based value that must advance while work emits no events. The interval must be between 1,000 milliseconds and 24 hours. Provider writes and rate limits still apply, so choose the slowest interval that meets the presentation need.
