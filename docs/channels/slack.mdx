...
1. Add the `app_mentions:read` and `chat:write` bot scopes. Add `im:history` for DMs, `files:read` for inbound attachments, and `files:write` for explicit uploads and automatic long-response attachments.
...
| A long response cannot be attached                                    | Check whether the installed Slack app has the `files:write` bot scope.                                                                                                                 | Add `files:write`, then reinstall the Slack app so the new scope takes effect.                                                                                       |
...
The built-in `message.completed` handler renders responses up to 12,000 characters inline. Longer responses are attached unchanged as `eve-response.md` with a short in-thread note, which requires the `files:write` bot scope. An authored `events["message.completed"]` handler replaces this behavior and owns its own delivery limits.
...
The example below overrides `onAppMention` to gate on an authored message. The built-in completion handler still posts the reply and attaches long responses. Event handlers receive `(eventData, channel, ctx)`, with Slack platform handles on `channel.thread` and `channel.slack`:
