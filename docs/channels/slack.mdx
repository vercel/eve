...
- `onMessage(ctx, message)` handles Slack `message` events. eve drops messages authored by the installed app before this hook runs, preventing self-reply loops. With `excludeOutsiders` disabled, messages from other bots remain visible; check `message.author?.isBot` when those should also be ignored. `ctx.isBotMentioned()` and `ctx.isSubscribed()` support mention and active-thread policies.
...
#### Exclude guests and external Slack Connect members

Set `excludeOutsiders: true` to accept inbound requests only from full members of the bot's workspace. Outsiders include both single-channel and multi-channel guest accounts, and external Slack Connect members.
...
`excludeOutsiders` defaults to `false`. Leaving it disabled adds no API calls or required scopes and preserves the existing inbound behavior. To enable it, add the `users:read` bot scope to your Slack app or Vercel Connect connector and reinstall the app so the scope takes effect.

After signature verification, the guard checks signed inbound Events API callbacks and all interactivity before any authored handler or model work runs. This includes eve-owned HITL buttons, opening and submitting HITL modals, custom actions, slash commands, and message and global shortcuts. Authored handlers cannot bypass the guard; accepted requests still pass through your handlers and their authorization policies.

For each request, eve selects the bot token using the app installation workspace (`installationTeamId`, passed as `teamId` to a function-form token provider). It calls `auth.test` to establish the bot's workspace from `team_id`, then `users.info` with that token to check the actor. The actor must meet both conditions:

- `user.team_id` matches the bot's workspace, or `user.enterprise_user.teams` contains that workspace. Membership elsewhere in the same enterprise is not enough.
- `user.is_restricted` and `user.is_ultra_restricted` are both explicitly `false`, excluding both guest account types.

The guard rejects actors marked external or stranger, deleted users, and bots. It fails closed when the actor or required workspace or user metadata is missing, or either API call fails, including a missing `users:read` scope. Events API callbacks must carry a string `event.user`; callbacks without one do not reach `onEvent` while the guard is enabled. These checks are not cached. The guard does not change the default Slack auth attributes.

This is an inbound invocation guard, not a content or visibility filter. It does not remove outsider messages from fetched thread history, prevent outsiders from reading replies in shared channels, or guard proactive sends. Use conversations limited to the intended audience for sensitive replies, and keep authorization checks inside sensitive tools.

#### Restrict who can invoke the agent

A valid Slack signature proves that Slack sent the event. It does not decide which channel or person your agent should trust. Use `excludeOutsiders` for the workspace-membership policy above. For a specific user or conversation allowlist, fail closed in the inbound handler before returning auth, especially when the app is installed in a shared Slack Connect channel:
