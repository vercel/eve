...
A Connect-backed build generates a Slack app manifest for each Slack channel. The manifest always includes the `app_mentions:read` and `chat:write` bot scopes, the `app_mention` event, interactivity, and an App Home Messages tab. Configure the bot's presentation and add requirements for your authored handlers on `slackChannel()`; eve deduplicates events and scopes against the baseline:
...
export default slackChannel({
  ...
  botScopes: ["channels:history", "groups:history"],
});
```

`botName`, `botDescription`, `botLongDescription`, `botBackgroundColor`, and `botAlwaysOnline` configure Slack's app and bot presentation. `botEvents` adds Slack Events API subscriptions, and `botScopes` adds bot OAuth scopes. eve does not infer requirements from an open-ended `onEvent` handler, so add every event the handler accepts and the corresponding Slack scope. Scope changes may require reinstalling the Slack app.
...
`isDMOrPrivateChannel()` returns `true` for DMs, group DMs, and private channels. Slack message events identify public channels as `channel` and private channels as `group`, so those events do not require an API lookup. Events such as `app_mention` omit the conversation type; for those, the helper calls Slack's `conversations.info` API and checks `is_private`. Missing scopes, API errors, and ambiguous responses return `true` so privacy-sensitive behavior fails closed. `isBotMentioned()` identifies an explicit mention. `isSubscribed()` checks whether the message belongs to a thread with an active eve session. Add `message.channels` through `botEvents` and `channels:history` through `botScopes` for public channels. Private channels additionally need `message.groups` and `groups:history`.
