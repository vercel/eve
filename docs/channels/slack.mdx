...
`isBotMentioned()` identifies an explicit mention. `isSubscribed()` checks whether the message belongs to an active eve session that has not been unsubscribed.

For example, the following policy treats mentioning another person without mentioning the bot as a handoff and unsubscribes the bot from the thread:
...
`ctx.unsubscribe()` marks the thread as unsubscribed while preserving its session and history, so later calls to `ctx.isSubscribed()` return `false`. Call `ctx.subscribe()` to mark it as subscribed again; explicitly mentioning the bot does this automatically and resumes the same session. Authored hook logic decides how subscription status affects message routing.
