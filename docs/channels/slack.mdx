...
Accepted Slack messages use `turnPolicy: "steer"` by default: eve adds input at the active turn's next safe boundary. Set `turnPolicy: "interrupt"` to stop that turn and start a replacement, or `turnPolicy: "queue"` when every active turn should finish. Ignored messages never alter the active turn:
