...
`post(...)` resolves to `{ id, fileIds, raw }`. `id` is the Slack message `ts`, so a later `chat.update` can target the same message; `fileIds` lists the Slack file ids the post created, in upload order, and is empty when the post carried no `files`.

One variant has no `id` to give. A `{ text, files }` post is delivered as a single Slack message by `files.completeUploadExternal`, and that method returns file ids without a message `ts`, so `id` is empty. Post `{ markdown | blocks | card }` with `files` when the message has to be targetable later; the message lands first through `chat.postMessage` and the files follow in the same thread.
...
`api.fetch` supplies the fetch implementation used for those calls, for a test double or an instrumented client:
