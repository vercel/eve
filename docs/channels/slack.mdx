...
`private`. When Slack does not provide enough visibility information, eve uses
`unknown`. The audience controls observability content capture, not access to
the channel. See [Audience](./overview#audience) for the shared channel model.
