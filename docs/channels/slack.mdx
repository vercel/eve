...
When a shortcut opens a custom modal, handle its submission with `onViewSubmission(submission, ctx)`. The submission contains the callback ID, private metadata, submitting user, workspace, and flattened Block Kit values. Text inputs use `value`; single-select inputs use `selectedOptionValue`. eve-owned HITL modals continue through `onInputResponse` instead.
