...
| The webhook reaches the deployment but the agent does not reply       | Open the project's Vercel runtime logs and locate the request for the inbound Slack event.                                                                                             | Use the first missing or failed stage to narrow the issue to channel dispatch, model or tool execution, or outbound Slack delivery.                                  |
...
```ts
import { callSlackApi, resolveSlackTransportOptions } from "eve/channels/slack";
import { connectSlackCredentials } from "@vercel/connect/eve";

const { botToken } = connectSlackCredentials("slack/my-agent");
const api = resolveSlackTransportOptions({ apiBaseUrl: process.env.SLACK_API_BASE_URL });
...
JSON). It has no channel to inherit
[`api`](#point-the-channel-at-a-different-slack-api-host) from, so pass the same one the
channel was given, through `resolveSlackTransportOptions` — that is where the bases are
checked and resolved. An `apiBaseUrl` of `undefined` keeps Slack's own host.
