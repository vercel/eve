...
Inbound Events API deliveries use Slack's verified `event_id` as a durable session idempotency
key. Retries can reach any application replica, but a session starts at most one turn while that
key remains in its bounded recent-delivery window.
