...
slackChannel({
  api: {
    ...
    fetch: (input, init) => {
      const proxyKey = process.env.SLACK_PROXY_KEY;
      if (proxyKey === undefined) throw new Error("SLACK_PROXY_KEY is required.");
...
`api.fetch` says how the traffic travels. It replaces the global `fetch` for every Slack Web API call and every attachment download, Slack's own file hosts included — use it to attach the per-call header a host behind an authenticating proxy requires.
