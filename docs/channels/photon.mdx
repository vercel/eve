...
## Durable state and retry deduplication

The channel uses in-memory Chat SDK state by default. That default is convenient
for local development, but its subscriptions and inbound-message deduplication do
not survive a process restart.

For production or multi-instance hosts, install a durable Chat SDK state adapter
and pass it through `state`. `dedupeTtlMs` controls how long Chat SDK remembers an
inbound Photon `message.id`; Photon keeps that identifier stable across retries.
...
const PHOTON_DEDUPE_TTL_MS = 48 * 60 * 60 * 1_000;

export default photonIMessageChannel({
  credentials: connectPhotonCredentials("photon/my-agent"),
  dedupeTtlMs: PHOTON_DEDUPE_TTL_MS,
  state: createRedisState({ keyPrefix: "my-agent:photon" }),
});
```

`createRedisState` reads `REDIS_URL` when no explicit URL is supplied. Use a
dedicated key prefix when multiple agents share one Redis database.
