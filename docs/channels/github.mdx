...
GitHub owns default event handlers for native side effects: `turn.started` reacts and checks out the repository, `message.completed` posts the final reply, and `turn.failed`/`session.failed` post error comments. A bare handler in `events` runs after the matching default, so status customization does not accidentally disable checkout or replies:
...
export default githubChannel({
  events: {
    async "turn.started"(_event, channel) {
      await channel.thread.post("Starting deeper triage.");
    },
  },
});
```

Use `beforeDefault(handler)` to run first, or `replaceDefault(handler)` when you intentionally want to take over the built-in side effect:

```ts title="agent/channels/github.ts"
import { replaceDefault } from "eve/channels";
import { githubChannel } from "eve/channels/github";

export default githubChannel({
  events: {
    "message.completed": replaceDefault(async (event, channel) => {
      if (event.message) await channel.thread.post(`Custom reply:\n\n${event.message}`);
    }),
  },
});
```
