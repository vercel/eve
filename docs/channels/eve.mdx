...
- `POST /eve/v1/sessions` (start a session and send its first message)
- `POST /eve/v1/sessions/:sessionId/messages` (send a follow-up)
- `POST /eve/v1/sessions/:sessionId/cancel` (cancel the in-flight turn)
- `POST /eve/v1/sessions/:sessionId/clear` (clear the session's model history)
- `POST /eve/v1/sessions/:sessionId/compact` (compact the session's context)
- `POST /eve/v1/sessions/:sessionId/reset` (retire the session)
- `GET /eve/v1/sessions/:sessionId/stream` (stream events as NDJSON)
...
```bash
curl -N https://<deployment>/eve/v1/sessions/wrun_A/stream
# {"type":"turn.started",...}
...
```bash
curl -X POST https://<deployment>/eve/v1/sessions/wrun_A/cancel
# {"ok":true,"sessionId":"wrun_A","status":"accepted"}
```

Cancellation is asynchronous: `"accepted"` means the request was durably queued. Confirm the outcome on the stream as `turn.cancelled` followed by `session.waiting`—never as a failure. Active local and remote subagents are cancelled recursively before the parent settles. Content emitted before cancellation stays on the event stream, while durable model history keeps only content that had already settled. The session accepts the next message normally.

When there is no cancellable turn—including an unknown session, a settled turn, or a duplicate cancel—the route returns `"no_active_turn"`. A `turnId` naming any other turn is accepted and consumed as a no-op, so a guarded cancel racing a turn boundary cannot stop a turn the caller never saw.
...
```bash
curl -X POST https://<deployment>/eve/v1/sessions/wrun_A/compact
# {"ok":true,"sessionId":"wrun_A","status":"accepted"}
```
...
```bash
curl -X POST https://<deployment>/eve/v1/sessions/wrun_A/reset \
  -H "Content-Type: application/json" \
  -d '{"reason":"Start over"}'
# {"ok":true,"previousSessionId":"wrun_A","status":"reset"}
```

After reset, the old ID cannot accept another message. Start a replacement explicitly with `POST /eve/v1/sessions`.
