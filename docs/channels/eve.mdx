...
The application exposes a health route plus eve channel routes that inspect the agent, create or resolve sessions, send follow-ups, cancel turns, and stream events:

- `GET /eve/v1/health`
- `GET /eve/v1/info`
- `GET /eve/v1/session?continuationToken=...` (resolve the session that owns a continuation token)
- `POST /eve/v1/session` (start or resume a session)
...
Start a session with a minimal body. The response returns `sessionId` and the `continuationToken` you reuse for follow-ups. To bind the session to an application-owned identity, include that value as `continuationToken` in the create body; eve resumes its current owner when one exists and otherwise starts a session with that token.
...
Resolve an application-owned continuation token without delivering input:

```bash
curl "https://<deployment>/eve/v1/session?continuationToken=authoring-session-123"
# {"ok":true,"sessionId":"ses_01h..."}
```

An unknown token returns `404`. Stream a resolved session's events as newline-delimited JSON (`application/x-ndjson; charset=utf-8`), one event object per line:
...
The stream stays live by default. For a finite recovery replay, pass a nonnegative `startIndex` and `throughCurrentTail=true`. The route snapshots the current tail, emits through the first current-turn boundary at or beyond that snapshot, then closes; if `startIndex` is already at the snapshot, it closes empty.
