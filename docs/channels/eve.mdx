...
Post to `/eve/v1/session/:sessionId/reset` to queue terminal retirement for that session. The
response does not wait for the session inbox to be released. Reset never replaces the ID
automatically; create a new session explicitly for a fresh conversation. See
[Compact, clear, and reset](../concepts/sessions-runs-and-streaming#compact-clear-and-reset).
