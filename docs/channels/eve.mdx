...
### Cancel a turn or background task

Post to `/eve/v1/session/:sessionId/cancel` to request cancellation of the active turn. Include the observed `turnId` to keep a late request from cancelling a newer turn, or include `taskId` to cancel one background task owned by the session. Cancellation is asynchronous. Turn cancellation settles as `turn.cancelled` followed by `session.waiting`; task cancellation settles through a `task.updated` event whose view has status `cancelled`.
