...
Cancellation is asynchronous: `"cancelling"` means the cancel was delivered, and the turn then settles on the stream as `turn.cancelled` followed by `session.waiting` — never as a failure. Content emitted before the cancel stays in the session's history, and the session accepts the next message normally. When there is nothing to cancel (no turn in flight, the turn already settled, or a duplicate cancel), the route responds with `"no_active_turn"` — also a success, so a stop button can fire and forget. A `turnId` naming any other turn is delivered and consumed as a no-op, so a cancel racing a turn boundary can never stop a turn the caller never saw.
