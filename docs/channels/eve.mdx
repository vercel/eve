...
Compaction is asynchronous. An accepted request emits `compaction.requested` and `compaction.completed`; when a turn is active, the operation waits for it to settle. `"no_active_session"` is the successful no-op response when the token has no owner.
