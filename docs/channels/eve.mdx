...
The Next.js Web Chat starter includes a shared-password sign-in powered by Better Auth. To keep the default, set `EVE_ACCESS_PASSWORD` and `BETTER_AUTH_SECRET`; local development skips sign-in unless `EVE_REQUIRE_LOCAL_AUTH=1`. The starter includes a local `fromBetterAuth(auth)` adapter, so you can configure your own Better Auth provider in the app without adding Better Auth support to eve core. If you use another auth library, replace that channel entry with the provider's `AuthFn`.
