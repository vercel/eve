...
The workflow establishes its inbox and parks before session initialization or `turn_0`.
No `session.started` or `session.waiting` event is emitted until a message arrives. The first
message invokes `onMessage`, then runs session-scoped hooks, dynamic definitions, and sandbox
setup with that message's auth and context. Its `title` sets the prewarmed workflow's title;
if omitted, eve derives the title from the first message. Later messages do not rename it.
...
can return `409 session_not_ready`; retry that code with bounded backoff. The event-stream client retries the
