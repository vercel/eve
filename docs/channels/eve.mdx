...
A follow-up can carry its own `operationId`, a string of up to 256 characters. The session admits one message per `operationId`, so a retried request is not delivered twice.

Accepted create and follow-up responses report `taskProtocol`, the task protocol version this deployment uses when another eve deployment calls it as a [remote agent](../guides/remote-agents#upgrading-remote-agents). A request that carries a `callback` or a `taskProtocol` must send this deployment's version, or the route answers `409` with `"code": "TASK_PROTOCOL_MISMATCH"`. A calling eve deployment reads `GET /eve/v1/session/:sessionId/reports/:callId`, with the channel's auth, once at a call's deadline to recover a result whose callback never arrived.
