...
Stream a session as newline-delimited JSON from `GET /eve/v1/session/:sessionId/stream`. Live responses close after emitting `session.waiting`, `session.completed`, or `session.failed`, which flushes the turn boundary through intermediaries. Reconnect from the next event index to follow later turns. The [session protocol](../concepts/sessions-runs-and-streaming#stream-a-session) defines the event set, envelopes, cursors, and reconnection behavior.
