...
The eve channel serves the routes that check reachability, inspect the agent, create sessions, send follow-ups, control sessions, and stream events:

- `GET /eve/v1/health` (check whether the application is reachable; always public)
- `GET /eve/v1/info` (inspect the agent; behind the channel's `auth`)
...
The channel also mounts the framework's callback routes on the same surface: connection authorization callbacks, the session callback, and the task-input route eve uses internally.

Authoring `agent/channels/eve.ts` — typically another `eveChannel({ ... })` call — replaces this entire surface, including the health and info routes. The `eveChannel()` factory always serves `GET`/`HEAD /eve/v1/health` publicly, so load balancers and uptime monitors keep working with any `auth` policy.

The home page (`GET /`) is a separate framework channel at the `agent/channels/home.ts` slot. Author that file with your own `defineChannel(...)` to replace it.
