...
The `202` response means the holding workflow accepted the first message. Resources
and initial state initialize in that workflow and its first turn. Immediate follow-ups
can address the returned session; their background dispatch waits for initialization.

Authenticated callers can pass an `operationId` for create-once semantics. The same
operation under the same authenticated principal resolves to its canonical session,
including when creation attempts arrive concurrently. Anonymous callers cannot use
`operationId`. Operation ownership expires when the session is no longer resumable.
...
Follow-up messages use `turnPolicy: "steer"` by default, adding input at the active turn's next safe boundary. Set `turnPolicy: "queue"` when the active turn should finish first, or `turnPolicy: "interrupt"` to replace it. An interruption emits `turn.interrupted` without an idle boundary before the replacement. `inputResponses` never interrupt.

A follow-up `202` acknowledges the validated request. Session resolution and turn
dispatch run through the host's `waitUntil` after acknowledgement; the response does
not confirm durable delivery or execution. Delivery failures are logged by the host.
Read the session stream for execution outcomes. The route never creates or follows
a replacement session, and does not preflight whether the session is still active.

Task-input, authorization, and subagent callbacks also deliver in background work
after validating their request. A callback acknowledgement does not confirm that
the receiving owner applied it.
...
Post to `/eve/v1/session/:sessionId/cancel` to request cancellation of the active turn. You can include the observed `turnId` to keep a late request from cancelling a newer turn. The route returns `202` and dispatches cancellation through `waitUntil`. An actual cancellation appears on the stream as `turn.cancelled` followed by `session.waiting`.

Include `tasks: true` to also cancel background tasks owned by the session, including while it is parked.
...
Post to `/eve/v1/session/:sessionId/reset` to request terminal retirement. The route returns `202` with `status: "reset"` and `previousSessionId` while dispatch continues through `waitUntil`; the response does not mean retirement has completed. Reset never replaces the ID automatically; create a new session explicitly for a fresh conversation. See [Compact, clear, and reset](../concepts/sessions-runs-and-streaming#compact-clear-and-reset).
...
`onMessage` must return an auth result. Return `title` alongside `auth` to set the title when the dispatch starts a run. An accepted HTTP follow-up schedules delivery to its addressed session.
