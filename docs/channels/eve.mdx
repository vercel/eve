...
The TypeScript client and raw HTTP routes accept `context` as an array of strings. The eve channel merges those entries after any context returned by `onMessage`, then appends all of them as durable user-role messages immediately before the delivery. Later turns see that context through normal session history.
