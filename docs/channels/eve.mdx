...
### Continuation-addressed routes

The application exposes a health route plus eve channel routes that inspect the agent, create sessions, send follow-ups, cancel turns, compact or reset sessions, and stream events:
...
- `POST /eve/v1/session/clear` (clear the continuation owner's model history)
- `POST /eve/v1/session/compact` (compact the continuation owner's context)
- `POST /eve/v1/session/reset` (retire the continuation owner)
...
Clear the model-message history of the session that owns a continuation token:

```bash
curl -X POST https://<deployment>/eve/v1/session/clear \
  -H "Content-Type: application/json" \
  -d '{"continuationToken":"eve:7f3c..."}'
# {"ok":true,"sessionId":"ses_01h...","status":"accepted"}
```

An accepted clear emits `context.cleared` followed by `session.waiting`. The next message resumes the same session without prior model messages; the system prompt, tools and skills, durable state, limits, and session-scoped sandbox remain.

Compact the session that owns a continuation token without sending a message:

```bash
curl -X POST https://<deployment>/eve/v1/session/compact \
  -H "Content-Type: application/json" \
  -d '{"continuationToken":"eve:7f3c..."}'
# {"ok":true,"sessionId":"ses_01h...","status":"accepted"}
```

Compaction is asynchronous. A successful operation emits `compaction.requested` and `compaction.completed`, then returns through `session.waiting`; when a turn is active, the operation waits for it to settle. If summarization fails, the session still returns to `session.waiting` with its previous history. `"no_active_session"` is the successful no-op response when the token has no owner.
