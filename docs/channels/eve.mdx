...
- `POST /eve/v1/session/:sessionId/cancel` (cancel the in-flight turn or a background task)
...
Post to `/eve/v1/session/:sessionId/cancel` to request cancellation of the active turn. You can include the observed `turnId` to keep a late request from cancelling a newer turn. Pass `{ "taskId": "..." }` instead to stop one background task and leave the turn running, or `{ "tasks": true }` to cancel the active turn and every working task. Cancellation is asynchronous; confirm the turn boundary on the stream as `turn.cancelled` followed by `session.waiting`, and a stopped task as `task.settled` with `status: "cancelled"`.
