...
at creation; they do not change when the first message arrives. Turn-scoped fields such as
`clientContext` and `outputSchema` require a message on the create request.
...
You can also pass a function that receives `caller`, channel, and deployment
environment. Classification is fixed at session creation, so a later
...
`onMessage` must return an auth result. Return `title` alongside `auth` to set the title when the dispatch creates a session or sends the first message to a prewarmed session. For a create request from a trusted remote subagent, `ctx.eve.invocation.operationId` exposes the replay-stable operation identity eve already uses to recover that child session; applications can use it to idempotently admit related state before execution. It is absent for ordinary create requests and continuations. A successful canonical eve HTTP message always dispatches and therefore always produces or continues a session.
