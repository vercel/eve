...
The `202` response means the session resources and initial state are ready, and the
first turn is accepted. Immediate follow-ups can safely address the returned session.
...
A follow-up `202` means its turn candidate was durably accepted; the session may retire that input if cancellation or terminal settlement wins the race. Read the stream for execution outcomes.

Sending a message when the session lookup fails returns `409` with
...
Post to `/eve/v1/session/:sessionId/cancel` to request cancellation of the active turn. You can include the observed `turnId` to keep a late request from cancelling a newer turn. The response waits until the cancellation candidate and the turn it targeted have settled. An actual cancellation appears on the stream as `turn.cancelled` followed by `session.waiting`.
