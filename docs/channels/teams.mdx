...
The guided flow creates a managed Microsoft Teams bot through Vercel Connect,
opens the administrator sign-in and consent step, attaches verified Activities
to `/eve/v1/teams`, and writes:
...
It requires an Azure subscription ID. Optionally provide an existing Azure
resource group; otherwise Vercel Connect creates one. After deployment, add the
managed app from Vercel Connect to the Teams team or chat where it will run.
