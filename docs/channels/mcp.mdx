...
## Expose tools and skills directly

`mcpChannel` delegates whole tasks. When another agent should call your agent's tools one at a time instead, add `mcpCapabilitiesChannel`. It publishes the agent's own tools and skills as a stateless MCP server (protocol `2026-07-28`, JSON responses) at `/eve/v1/mcp-capabilities`:
...
export default mcpCapabilitiesChannel({
  auth: vercelOidc({
    subjects: [vercelSubject({ teamSlug: "acme", projectName: "orchestrator", environment: "*" })],
  }),
});
```

Authentication works as in `mcpChannel`, including `oauthResource(...)` metadata. Set `route` to publish somewhere else.

- `tools/list` returns authored and extension tools. Each tool carries `_meta["eve.dev/owner"]` (the agent name) and `_meta["eve.dev/approval"]`. Framework tools such as `load_skill` and `bash`, workflow tools, and `execution: "background"` tools are not listed.
- `tools/call` runs the tool's `execute` on your deployment, in the same context it gets in the model loop: `ctx.getSandbox()`, `ctx.getSkill()`, `ctx.getToken()`, and `ctx.session.auth` (the authenticated caller) all work. The result's `content` is the tool's `toModelOutput`, or its output as text or JSON. Object outputs also appear in `structuredContent`. A thrown error becomes an `isError` result.
- `resources/list` returns one `skill://<agent>/<skill>` resource per skill, with supporting file paths in `_meta["eve.dev/files"]`. `resources/read` returns the skill instructions, or a supporting file at `skill://<agent>/<skill>/<path>`.
- `tools/list` also returns one tool per declared local subagent, named after the subagent, with `_meta["eve.dev/kind"]: "subagent"` and a single required `message` input. See [Subagents](#subagents).

### Sessions and sandboxes

Send `eve-capability-session: <key>` (or `params._meta["eve.dev/session"]`) on every request of one caller conversation. eve derives the tool session ID as `sha256(principal + "\n" + key)`, so two callers cannot share a session by choosing the same key. Every call with the same key sees the same `ctx.session.id` and the same sandbox, including across function instances, because providers name the sandbox after the session ID. `server/discover` starts provisioning that sandbox in the background and reports `_meta["eve.dev/sandbox"]` as `"none"`, `"warming"`, or `"ready"`.

Requests without a key get a one-off session, and eve deletes its sandbox after the request.

On an instance that has not seen the session yet, eve runs your `defineSandbox()` function again against the existing sandbox, so keep sandbox setup idempotent.

### Forwarded principals

A calling agent can run tools on behalf of its user, the same way remote agents forward principals to `eveChannel`. Set `trustedForwarders` to decide which route-authenticated callers may do this:

```ts title="agent/channels/mcp-capabilities.ts"
import { vercelOidc, vercelSubject } from "eve/channels/auth";
import { mcpCapabilitiesChannel } from "eve/channels/mcp";

const orchestrator = vercelSubject({ teamSlug: "acme", projectName: "orchestrator" });

export default mcpCapabilitiesChannel({
  auth: vercelOidc({ subjects: [orchestrator] }),
  trustedForwarders: (forwarder) => forwarder.subject === orchestrator,
});
```

The caller sends `eve-forwarded-principal: <value>`, where the value is base64url-encoded JSON `{ current, initiator? }` of session auth contexts. `encodeForwardedPrincipalHeader()` from `eve/channels/mcp` builds it. When the forwarder is trusted, the tool sees the forwarded contexts as `ctx.session.auth.current` and `ctx.session.auth.initiator` (initiator defaults to current), each stamped with the forwarder's `principalId` in the `eve:forwarded-by` attribute. An untrusted forwarder, or a channel without `trustedForwarders`, gets `403`; a malformed header gets `400`. Without the header, tools run as the route-authenticated caller.

A forwarded call's session ID includes both identities, `sha256(forwarder + "\n" + forwarded current + "\n" + key)`, so users behind the same forwarder never share a session or sandbox.

### Approvals and sign-in

A tool that needs approval, or a `ctx.getToken()` call that needs the user to sign in, returns an MCP input-required result instead of running. Approval is a form elicitation with a boolean `approved` field. Sign-in is a URL elicitation. Retry `tools/call` with the same arguments, the `inputResponses`, and the returned `requestState`. eve rejects a `requestState` from a different session, tool, or argument set. Clients must declare `elicitation` (`form` and `url`) in their client capabilities to receive these requests.

Sign-in callbacks arrive at `<route>/authorize/:name/:attemptId`. That route is public, so the provider's browser redirect can reach it.

### Subagents

Calling a subagent tool runs that subagent to completion on your deployment and returns its final answer as text, or as `structuredContent` when the answer is JSON. The subagent receives `message` the same way it receives a delegation from your agent, runs as the calling principal (including a forwarded principal), and uses the capability session's sandbox when it declares `defineParentSandbox()`. Remote agents are not listed.

When the subagent asks a question, the call returns an input-required result with one form elicitation per question: pick an option with `optionId`, or answer with `text` when the question allows free-form answers. When it needs a sign-in, the call returns a URL elicitation. Retry with the same arguments, the `inputResponses`, and the returned `requestState` to continue the same subagent session. Declining a question cancels the subagent. When the subagent starts background work of its own, such as its own subagents, a reply it gives while that work runs is interim: the call waits for the answer the subagent gives once all of it settles. A subagent still working after 150 seconds is cancelled together with its background work, and the call returns an error saying so.
