...
| Tool           | Input                         | Purpose                                                     |
| -------------- | ----------------------------- | ----------------------------------------------------------- |
| `agent_start`  | `{ message }`                 | Start durable work and immediately return an invocation ID. |
...
`agent_start` creates one eve session and returns after durable acceptance without
...
- `completed`: the agent's turn settled, after any [tasks](/docs/tools/workflows#run-calls-as-tasks-task) it started finished. Consume the optional `result`: the final assistant message.
...
| --------------- | ---------------------------------------------------------------------------- | ----------- |
| `invalid_input` | An argument was rejected, for example an oversized `message`.                | `false`     |
...
Requests are bounded: the whole MCP request body is limited to 1 MiB, `message` to 64 KiB, each input-response `text` to 16 KiB (both measured as UTF-8 bytes, not characters), and one `agent_update` to 64 responses. Oversized bodies receive a JSON-RPC `413`; oversized fields fail input validation before any work starts.
