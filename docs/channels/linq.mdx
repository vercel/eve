...
The default webhook route is `/eve/v1/linq`. The channel verifies forwarded webhooks with same-project Vercel OIDC by default, derives user auth from each message author, marks accepted messages as read, and continues the same eve session for every message in a Linq conversation. A new accepted message adds input at the active turn's next safe boundary. Set `turnPolicy: "interrupt"` to stop the active turn and start a replacement.
