...
`connectSendblueCredentials(...)` to select one. The channel derives user auth
from each message author. When no direct webhook secret is configured, the
channel verifies Connect-forwarded webhooks with Vercel OIDC.
