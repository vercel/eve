...
## What a fire carries over

Nothing, by default. A markdown-form fire starts a fresh session every time, so it opens with no history from the previous run. A handler-form fire is also a new session unless the target it sends to resolves to a continuation address an earlier session still owns — then it lands as another turn on that conversation, with its history and [state](./concepts/state) intact. A target that resolves to a new address each time, such as a proactive Slack send with a `channelId` and no `threadTs`, starts a new session on every fire.

So a schedule that has to remember something from one run to the next keeps it in a store you own. [State](./concepts/state) covers where session-scoped memory ends, and [dynamic scheduling](./patterns/dynamic-scheduling) shows the pattern for schedule data that outlives any one run.
