...
Markdown schedules start a new session on every fire. Handler schedules do too,
unless they send to an existing conversation, such as a Slack thread. Store data
that must survive across fires outside session state.
