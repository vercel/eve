...
This is the minimal schedule. eve runs the agent on the prompt and throws away the output, though the agent can still call tools, write to backends, and log along the way. The session has no channel, so it cannot ask a person for input, and nothing can send it a follow-up message. After its turn settles, the session parks like any other session until its [session timeout](./agent-config#runtime-limits) ends it.
