...
Each markdown schedule fire starts a new session, so it does not inherit an
earlier run's history or [state](./concepts/state). Handler schedules also start
new sessions unless they send to a channel address that an existing conversation
owns, such as a Slack thread. Store data that must survive across fires outside
session state; [dynamic scheduling](./patterns/dynamic-scheduling) shows one
such pattern.
