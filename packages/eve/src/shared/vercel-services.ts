...
function assertRootServicesIncludeEve(
  ...
  if (findEveService(services) === undefined) {
    throw new Error(
      `${VERCEL_JSON_FILE_NAME} already defines services, so the eve ${frameworkName} integration cannot add a generated eve service. Add an eve service (framework "eve") and a rewrite from ${EVE_ROUTE_PREFIX}/(.*) to it in ${VERCEL_JSON_FILE_NAME}, or remove services from ${VERCEL_JSON_FILE_NAME}.`,
    );
  }
}
...
function createGeneratedServiceBuild(input: {
  ...
  const rootDirectory = join(input.hostRoot, EVE_VERCEL_SERVICES_DIRECTORY, EVE_SERVICE_NAME);
