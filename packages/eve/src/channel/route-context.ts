...
const routeAgentNameKey = "__eveRouteAgentName";
...
export function attachRouteAgentName<TArgs extends RouteHandlerArgs>(
  args: TArgs,
  agentName: string,
): TArgs {
  const routeArgs: InternalRouteArgs = args;
  routeArgs[routeAgentNameKey] = agentName;
  return args;
}

export function readRouteAgentName(args: RouteHandlerArgs): string | undefined {
  const routeArgs: InternalRouteArgs = args;
  return routeArgs[routeAgentNameKey];
}
