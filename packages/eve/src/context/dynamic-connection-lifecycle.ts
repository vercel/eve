...
async function resolveConnections(input: {
  ...
  const value = await handler(input.event, buildResolveContext(input.ctx, input.messages));
...
export async function dispatchDynamicConnectionEvent(input: {
  ...
  for (let index = 0; index < outcomes.length; index += 1) {
    ...
    if (outcome.status === "rejected") {
      log.error(`Dynamic connection resolver (${input.event.type}) failed — omitting its result.`, {
        error: toErrorMessage(outcome.reason),
        logicalPath: resolver.logicalPath,
      });
      updates.set(resolver.slug, []);
      continue;
    }
