import {
  ...
  isObjectRecord,
  readInstallationTeamId,
  type ParsedBlockActionsPayload,
} from "#public/channels/slack/interaction-payload.js";
export { parseBlockActionsPayload } from "#public/channels/slack/interaction-payload.js";
import { resolveSlackSessionAnchor } from "#public/channels/slack/session-anchor.js";
/**
  ...
  if (hitlActions.length > 0) {
    ...
    ctx.waitUntil(
      dispatchBlockInputResponses({
        appId: readOptionalString(payload.raw.api_app_id),
        ctx,
        deps,
        interaction,
        submission: {
...
async function dispatchBlockInputResponses(input: {
  ...
  try {
    const address = await resolveInteractionAddress({
      appId: input.appId,
      channelId: input.interaction.channelId,
      threadTs: input.interaction.threadTs,
      installationTeamId: input.interaction.installationTeamId,
      deps: input.deps,
      resolveSession: input.ctx.resolveSession,
    });
    await input.ctx.from(address).respond(input.submission.inputResponses, {
      auth: result.auth,
...
async function dispatchViewInputResponse(input: {
  readonly appId: string | undefined;
  readonly ctx: { from: ChannelFrom<SlackChannelState>; resolveSession: ChannelResolveSession };
  readonly deps: InteractionHandlerDeps;
  readonly installationTeamId: string | undefined;
...
async function resolveInteractionAddress(input: {
  readonly appId: string | undefined;
  ...
  const address = slackContinuationToken(input.channelId, input.threadTs);
  if ((await input.resolveSession(address)) !== undefined) return address;
  const anchor = await resolveSlackSessionAnchor({
    ...input,
  ...
  return anchor?.continuationToken ?? address;
}
