...
export interface SlackAppConfig {
  ...
  /** Display name used for the Slack app. */
  readonly name?: string;
}

export interface SlackBotConfig {
  /** Slack bot display name. Invalid characters are normalized to hyphens. */
...
export interface SlackChannelConfig {
  ...
  readonly app?: SlackAppConfig;
  /** Slack bot presentation. */
...
export function slackChannel(config: SlackChannelConfig = {}): SlackChannel {
  ...
  const manifest = defineSlackAppManifest({
    appName: config.app?.name,
    backgroundColor: config.app?.backgroundColor,
    botDisplayName: config.bot?.displayName,
