...
export interface SlackChannelConfig {
  /**
   * Slack API transport overrides. `apiBaseUrl` redirects every outbound
   * Slack Web API call, defaulting to `https://slack.com/api/`;
   * `fileBaseUrl` names an extra base authenticated attachment
   * downloads may come from, falling back to `apiBaseUrl`; and `fetch`
   * replaces the global `fetch` for requests to those two bases only —
   * Slack's own hosts keep the global `fetch`. Inbound webhook
...
export function slackChannel(config: SlackChannelConfig = {}): SlackChannel {
  ...
  // Built once, so `api` is validated at channel construction and every
  // outbound call shares one resolved endpoint, token binding and fetch.
  const transport = createSlackTransport({
    api: config.api,
    botToken: config.credentials?.botToken,
  });
  const slackFetchFile = createSlackFetchFile({ transport });
  ...
  >({
    ...
    context(state, session) {
      return rebuildSlackContext(state, session, transport);
    },
      ...
      POST<SlackChannelState>(config.route ?? SLACK_CHANNEL_DEFAULT_ROUTE, async (req, args) => {
        ...
        if (contentType.includes("application/x-www-form-urlencoded")) {
          return handleInteractionPost(
            ...
            { config, onInputResponse, transport },
          );
        }
    ...
    receive(input, { from }) {
      return receiveOnSlack(input, { from, transport });
    },
