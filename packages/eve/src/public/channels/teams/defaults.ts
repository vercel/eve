...
/** Default auth projection for Teams message and invoke actors. */
export function defaultTeamsAuth(
  ...
  if (activity.from.name !== undefined) attributes.user_name = activity.from.name;
  if (activity.from.aadObjectId !== undefined) {
    attributes.aad_object_id = activity.from.aadObjectId;
  }
  if (tenantId !== undefined) attributes.tenant_id = tenantId;
  if (activity.teamId !== undefined) attributes.team_id = activity.teamId;
  if (activity.teamsChannelId !== undefined) attributes.channel_id = activity.teamsChannelId;

  const principalId = tenantId
    ? `teams:${tenantId}:${activity.from.id}`
  ...
  return {
    ...
    principalId,
    principalType: activity.from.role === "bot" ? "service" : "user",
    subject: activity.from.aadObjectId,
  };
}
...
/** Builds the Teams event handlers for typing, replies, HITL, auth cards, and terminal errors. */
export function createDefaultTeamsEvents(input: {
  readonly adaptiveCardVersion: string;
  readonly resolveHitlSecret: () => string | Promise<string>;
}): TeamsChannelEvents {
  return {
    ...
    async "input.requested"(event, channel, _ctx) {
      const secret = await input.resolveHitlSecret();
      const tenantId = channel.teams.tenantId;
      if (!tenantId) {
        throw new Error("teamsChannel: a Teams tenant id is required for HITL cards.");
      }
      for (const request of event.requests) {
        const continuationToken = teamsContinuationToken({
          conversationId: channel.teams.conversationId,
          replyToActivityId: channel.teams.replyToActivityId,
          tenantId,
        });
        await channel.thread.post(
          renderInputRequestMessage(request, {
            adaptiveCardVersion: input.adaptiveCardVersion,
            continuationToken,
            conversationId: channel.teams.conversationId,
            secret,
            tenantId,
    ...
    async "authorization.required"(event, channel, _ctx) {
      const displayName =
        event.authorization?.displayName ?? formatConnectionDisplayName(event.name);
        ...
        attachments: [
          {
            content: parseJsonObject({
              ...
              type: "AdaptiveCard",
              version: input.adaptiveCardVersion,
            }),
