...
import type { ChannelAddressFn } from "#public/definitions/channel.js";
...
export async function dispatchIssueComment(input: {
  ...
  readonly channelAddress: ChannelAddressFn<GitHubChannelState>;
}): Promise<void> {
  if (
...
async function sendGitHubTurn(input: {
  ...
  try {
    await input.channelAddress(continuationTokenFromState(input.state)).send(
      {
        message: turnMessage,
