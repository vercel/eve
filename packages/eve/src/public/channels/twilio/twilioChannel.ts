...
export function twilioChannel(config: TwilioChannelConfig): TwilioChannel {
  ...
  >({
    ...
    async receive(input, { send }) {
      ...
      return send(twilioContinuationToken(phoneNumber, from ?? undefined), {
        auth: input.auth,
...
async function dispatchText(input: {
  ...
  readonly onText: NonNullable<TwilioChannelConfig["onText"]>;
  readonly send: ChannelOperations<TwilioChannelState>["send"];
}): Promise<void> {
  const { message } = input;
  ...
  try {
    await input.send(twilioContinuationToken(message.from, message.to), {
      auth: result.auth,
...
async function dispatchVoiceTranscription(input: {
  ...
  try {
    await input.send(twilioContinuationToken(transcription.from, transcription.to), {
      auth: result.auth,
      context: [contextBlock],
