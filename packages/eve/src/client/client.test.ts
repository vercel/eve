...
describe("Client request policy", () => {
  it("does not follow redirects by default when client auth is configured", async () => {
    const fetchMock = vi
  ...
  it("forwards AbortSignal to info and health", async () => {
    const fetchMock = vi
      .spyOn(globalThis, "fetch")
    ...
    await client.info({ signal: infoAbort.signal });
    await client.health({ signal: healthAbort.signal });

    expect(fetchMock.mock.calls[0]?.[1]?.signal).toBe(infoAbort.signal);
    expect(fetchMock.mock.calls[1]?.[1]?.signal).toBe(healthAbort.signal);
  });
