...
const MIXED_AGENT_MESSAGING_INSTRUCTION =
  "Agent messaging\nEach subagent tool description says whether the call waits for a child result or starts a durable background task and returns a receipt. Background tasks may notify you when they complete, fail, need input, or send an update. Agents you have already delegated to remain visible in the framework-authored `<agents>` conversation note. `availability=busy` means the listed child is still working; wait for its result or notification before continuing it. `availability=available` means you may pass agentId to the original subagent tool to continue it. Calling a subagent without agentId always starts a new agent session.";
