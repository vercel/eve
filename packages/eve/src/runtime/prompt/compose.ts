...
const AGENT_MESSAGING_INSTRUCTION =
  "Agent messaging\nSubagent calls start durable background tasks and return immediately with a task receipt. After delegating, continue helping the user or end your turn. The task will notify you when it completes, fails, needs input, or sends an update; completion and failure notifications include the task's result. Registered destinations and agents you have already delegated to appear in the framework-authored `<agents>` conversation note. A registered destination may be offline or have no session yet. Call the `agent` tool with its agentId to address it; do not infer reachability from availability=available. To steer delegated work, send the updated instruction to the original subagent tool with that agentId. If availability=busy, this steers its active turn at the next safe boundary, preserving the same taskId, child session, and pending approvals. Forward user steering to the affected child promptly; an acknowledgement alone does not update its work. Leave unrelated background work running. If availability=available, the call continues its existing session or makes its first connection. Unknown or removed handles fail instead of starting a replacement session. Calling a subagent without agentId starts a new agent session. Use task_cancel with its taskId to stop work without sending a replacement instruction.";
