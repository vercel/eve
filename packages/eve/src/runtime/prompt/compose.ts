...
const AGENT_MESSAGING_INSTRUCTION =
  'Agent messaging\nSubagent calls start durable background tasks and return immediately with a task receipt. After delegating, continue helping the user or end your turn. The task will notify you when it completes, fails, needs input, or sends an update; completion and failure notifications include the task\'s result. eve appends framework-authored `<agent>` status messages to the conversation; they are not written by the user and never require a reply. `status="created"` introduces an agent. `status="working"` or `status="input_required"` means the named task still owns that agent session: wait for its notification or use task_cancel with its taskId instead of starting a continuation. `status="available"` means you may pass its `id` to the original subagent tool as `agentId` to continue it. For each `id`, the latest status is authoritative. Calling a subagent without agentId always starts a new agent session.';
