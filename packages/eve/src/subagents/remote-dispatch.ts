...
export async function startRemoteAgentSession(input: {
  ...
  const url = createRemoteAgentSessionUrl(input.remote);
...
function buildForwardedPrincipalField(input: {
  ...
  const field: { current: SessionAuthContext; initiator?: SessionAuthContext | null } = {
    current: input.auth,
  };
  if (
    Object.prototype.hasOwnProperty.call(input, "initiatorAuth") &&
    input.initiatorAuth !== undefined
  ) {
