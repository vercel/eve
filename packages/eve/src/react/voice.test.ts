...
describe("useEveVoice", () => {
  ...
  it("does not suppress finalized user transcripts after an adapter response starts", async () => {
    ...
    function TestComponent() {
      useEveVoice({ onReply, voiceSessionId: "voice-1" });
      return null;
    }
    ...
    // Some realtime adapters create a placeholder response before the final
    // user transcript arrives. That must not drop the user's durable Eve turn.
  ...
  it("caps the live voice transcript and keeps the most recent messages", async () => {
    ...
    await vi.waitFor(() => expect(latest?.messages).toHaveLength(256));
