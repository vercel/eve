...
import {
  ...
  type RegisteredConnectionDefinition,
...
const logger = createLogger("execution.tools.connection-search");
...
async function completePendingAuthorizations(
  registry: ConnectionRegistry,
  connections: readonly RegisteredConnectionDefinition[],
): Promise<Set<string>> {
...
/** Resolves connection search and previously discovered tools for one step. */
export async function resolveConnectionSearchTools(): Promise<Record<string, object> | null> {
  const registry = loadContext().get(ConnectionRegistryKey);
