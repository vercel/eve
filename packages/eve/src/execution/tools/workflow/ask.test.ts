...
describe("ask", () => {
  it("resolves as unavailable without waiting when the session cannot request input", async () => {
    ...
    attachWorkflowToolRunContext(ctx, { canRequestInput: false, from, owner: { inbox: "inbox" } });
  ...
  it("throws after a resumable body replied, as ctx.agent does", () => {
    const ctx = {} as ToolContext;
    attachWorkflowToolRunContext(ctx, { from, owner: { inbox: "inbox" }, replied: true });

    expect(() => ask(ctx, { prompt: "Which region?" })).toThrow(
      "ctx.ask() was called after ctx.reply()",
    );
    expect(createHook).not.toHaveBeenCalled();
  });
});
