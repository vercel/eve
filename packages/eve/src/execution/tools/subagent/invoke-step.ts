...
export async function dispatchAgentInvocation(input: {
  ...
  const failed = async (result: RuntimeSubagentResult): Promise<AgentInvocationDispatchResult> => ({
    kind: "failed",
    result,
  ...
  if (entry.kind === "resume") {
    ...
    outcome = await dispatchToClaimedAgentAddress({
      ...
      handle: claimed,
      reply: { kind: "reply", parentToken: input.replyTo },
    });
...
/**
 * Dispatches one agent request. The callback origin is parent-owned, so it is
 * derived from this workflow's own metadata rather than accepted from the
 * inbound request.
 */
export async function dispatchAgentInvocationStep(
...
export async function settleAgentInvocationStep(input: {
  readonly ownerId: string;
  ...
  if (input.result.outcome.kind === "terminal") {
    if ("continuationToken" in handle.address) {
