...
/** The one complete schema for persisted session-inbox wire version 1. */
...
/** Builds and validates the complete current wire value before persistence. */
export function encodeSessionCommand(
  ...
  const parsed = sessionInboxV1Schema.safeParse(wire);
  if (!parsed.success) {
