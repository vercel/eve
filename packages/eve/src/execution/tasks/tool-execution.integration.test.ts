...
import { startTaskRun } from "#execution/tasks/runtime.js";
import {
  backgroundToolExecutionProvider,
  readRetainedBackgroundToolResult,
} from "#execution/tasks/background.js";
...
vi.mock("#execution/tasks/control.js", () => ({ cancelOwnedTask: vi.fn() }));
...
describe("background subagent steering", () => {
  beforeEach(() => {
    ...
    vi.mocked(startTaskRun).mockResolvedValue({ runId: "steering-task-run" });
  ...
  it.each([
    ...
    async ({ metadata }) => {
      ...
      expect(getHookByToken).not.toHaveBeenCalled();
      expect(startTaskRun).toHaveBeenCalledWith(
        expect.objectContaining({
          workflow: expect.objectContaining({ workflowId: expect.any(String) }),
        }),
      );
    },
  );
