...
vi.mock("#execution/session/ingress.js", () => ({ dispatchSessionCommandByToken: vi.fn() }));
vi.mock("#execution/workflow-tool/answer.js", () => ({
...
describe("deliverTaskInputResponsesStep", () => {
  ...
  it("delivers a local child answer through the child session inbox", async () => {
    ...
    expect(dispatchSessionCommandByToken).toHaveBeenCalledWith(answer.childContinuationToken, {
      auth: undefined,
      kind: "send",
  ...
  it("posts a remote child answer to its narrowed task-input route", async () => {
    ...
    expect(dispatchSessionCommandByToken).not.toHaveBeenCalled();
  });
  ...
  it("replies to a workflow tool through its correlated owner inbox", async () => {
...
describe("wakeTaskAgentRequestParentStep", () => {
  it("forwards an agent invocation through the typed task envelope", async () => {
    ...
    expect(dispatchSessionCommandByToken).toHaveBeenCalledWith("parent-token", {
      kind: "send",
      payload: {
        task: {
          agentRequests: [
