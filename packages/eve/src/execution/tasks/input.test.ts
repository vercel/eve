...
const request = {
  replyTo: {
    kind: "inbox" as const,
    address: { ownerRunId: "run-1", token: "tool-owner" },
    requestId: "req-1",
  },
  request: {
...
describe("recordTaskInputRequest", () => {
  ...
  it("records a generic workflow answer route after matching the task view", async () => {
    ...
    const result = await recordTaskInputRequest({ request, sessionState });
    ...
    expect(
      ...
      childContinuationToken: request.replyTo.address.token,
      inboxResponse: request.replyTo,
  ...
  it("rejects a request that does not match the task's outstanding batch", async () => {
    ...
    await expect(recordTaskInputRequest({ request, sessionState })).resolves.toEqual({
      accepted: false,
      sessionState,
    });
  });
  ...
  it("records a narrowed remote response route for a claimed remote child", async () => {
    ...
    const remoteRequest = {
      ...request,
      replyTo: { kind: "session" as const, token: remoteReplyTo },
    ...
    const result = await recordTaskInputRequest({ request: remoteRequest, sessionState });
...
describe("recordTerminalTaskViews", () => {
  it("caches an owned terminal view and releases the task's agent lease", async () => {
    ...
    const result = await recordTerminalTaskViews({ sessionState, views: [view] });
