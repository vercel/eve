...
export async function appendTaskViewStep(input: { readonly view: TaskView }): Promise<void> {
  ...
  try {
    ...
    if (isTerminalTaskStatus(input.view.status)) await writer.close();
  } finally {
    writer.releaseLock();
  }
}
...
export async function wakeTaskParentStep(input: {
  ...
  try {
    await dispatchSessionCommandByToken(input.token, command);
  } catch (error) {
...
export async function wakeWorkflowTaskInputRequestParentStep(input: {
  ...
  const command: SessionCommand = {
    ...
    taskDeliveryId: `${input.taskId}:input:${(input.request.requests ?? [input.request.request]).map((request) => (typeof request === "object" && request !== null ? Reflect.get(request, "requestId") : "")).join(":")}`,
  };
  try {
    await dispatchSessionCommandByToken(input.token, command);
  } catch (error) {
...
export async function deliverTaskInputResponsesStep(input: {
  ...
  try {
    ...
    } else if (input.inboxResponse !== undefined) {
      await resumeWorkflowToolRunAnswers(input.inboxResponse, command.payload.inputResponses);
    } else {
      await dispatchSessionCommandByToken(input.answer.childContinuationToken, command);
    }
    return "delivered";
  } catch (error) {
    if (isTaskWorkflowTargetGone(error)) {
