...
import { recordSessionTask, findSessionTaskEntry } from "#tasks/session-index.js";
...
class BackgroundToolExecutionScope implements BackgroundToolExecutor {
  ...
  private async start(input: {
    ...
    const settled = isAsyncIterable(output)
      ? await executeBackgroundIterable({ callId: input.options.toolCallId, output, task })
      : await output;
...
async function executeBackgroundIterable(input: {
  ...
  while (!next.done) {
    ...
    const delivered = await sendTaskInboundPayload({
      taskInboxToken: input.task.taskInboxToken,
      payload,
    });
    if (delivered !== "delivered")
      throw new Error(`Task "${input.task.taskId}" did not accept progress.`);
