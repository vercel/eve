...
import type { WorkflowToolRunTaskInputRequest } from "#execution/tasks/workflow.js";
...
export function workflowToolRunRequestToInputRequestPayload(
  ...
  return {
    callId: from.callId,
    childContinuationToken: replyTargetToken(replyTo),
        ...
        request.kind === "input-batch"
          ...
          : [
              normalizeInputRequest(
                request,
                from,
                replyTo.kind === "inbox" ? replyTo.requestId : replyTo.token,
              ),
            ],
