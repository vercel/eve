...
import { completeWorkflowStepAuthorization } from "#execution/workflow-tool/authorization-completion.js";
import {
...
export function withWorkflowStepAuthorization(execute: (...args: never[]) => unknown) {
  const wrapped = async function (
    ...
    context.setVirtualContext(
      AuthorizationHookKey,
      input.replyTo === undefined ? input.token : createCallbackCapability(input.replyTo),
    );
