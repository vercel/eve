...
import type {
  ...
  WorkflowStepResult,
} from "#execution/workflow-tool/step-context.js";
...
async function executeAuthorizedStep(
  ...
  for (;;) {
    const requestId = `${run.from.runId}:authorization:${run.answerSeq++}`;
    const callback = {
      async *[Symbol.asyncIterator]() {
        while (true) yield (await run.inbox.response(requestId, ctx.abortSignal)).payload;
      },
    };
    {
...
async function invokeAuthorizedStep(input: {
  ...
  const context: WorkflowStepContext = {
    ...
    token: callbackToken,
    replyTo: { kind: "inbox", address: run.inbox.address, requestId: callbackToken },
    authorizationResults,
  };
...
async function reportAuthorization(
  ...
  await withAbort(
    sendInboxStep(run.owner, {
      eventId: `${run.from.runId}:authorization:${challenge.attemptId}:${outcome ?? "required"}`,
      kind: "tool.request",
      payload: {
        from: run.from,
        replyTo: { kind: "inbox", address: run.inbox.address, requestId: challenge.attemptId },
        request: {
