...
describe("compileHookEntry", () => {
  ...
  it("records sorted event names", () => {
    const definition = compileHookEntry(
      {
        logicalPath: "hooks/audit.ts",
        sourceId: "hooks/audit.ts",
        sourceKind: "module",
      },
      { events: { "turn.started": () => undefined, "session.started": () => undefined } },
    );

    expect(definition.eventNames).toEqual(["session.started", "turn.started"]);
  });
});
