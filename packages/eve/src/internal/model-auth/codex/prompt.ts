...
import type { CodexAppServerInput, CodexDynamicTool } from "#internal/model-auth/codex/types.js";
...
export function mapCodexTools(
  ...
  for (const tool of tools ?? []) {
    if (tool.type !== "function") {
      warnings.push({
        details: "Codex models only bridge AI SDK function tools through eve.",
...
function formatFileReference(mediaType: string): string {
  if (!mediaType.startsWith("image/")) {
    throw new Error(`Codex models support image files, not "${mediaType}".`);
  }
...
function toImageDataUrl(data: Uint8Array | URL | string, mediaType: string): string {
  ...
  if (data instanceof URL || /^[a-z][a-z\d+.-]*:/iu.test(data)) {
    ...
    throw new Error("Codex models require inline image data, not a remote file URL.");
  }
