...
function createDevelopmentRolldownWatchOptions(appRoot: string): { exclude: string[] } | undefined {
  if (appRoot.length === 0) {
    return undefined;
  }

  return {
    exclude: [appRoot, join(appRoot, "**")],
  };
}
...
export async function createApplicationNitro(
  ...
  const nitroRolldownConfig = {
    ...
    // Dev-runtime snapshots are immutable. Eve's authored-source watcher owns
    // rebuilds and triggers Nitro reloads when runtime wiring changes, so
    // Rolldown should not keep a native watcher rooted inside the snapshot.
    watch: dev ? createDevelopmentRolldownWatchOptions(preparedHost.appRoot) : undefined,
  };
  ...
  const nitro = await createNitro(
    ...
    dev
      ? {
          // Eve's authored-source watcher owns dev rebuilds and explicitly
          // triggers Nitro reloads, so Nitro's config watcher is redundant.
          watch: false,
        }
      : undefined,
  );
