 ...
 * dev-server restart.
 *
 * `artifactsConfig` is baked in once by `configure-nitro-routes.ts` at
 * server-build time — inside the unbundled host CLI process — and passed
 * through as-is rather than re-derived here. Recomputing it at request time
 * would call `resolvePackageSourceFilePath` from code running inside the
 * bundled Nitro dev output, where its "find an ancestor directory literally
 * named `dist`" heuristic fails and silently resolves to a broken path (see
 * vercel/eve#311).
 */
