...
describe("react-server module maps", () => {
  ...
  // NODE_OPTIONS conditions resolve through the same `authoredModuleConditions`
  // call; its unit test owns reading them from the environment.
  it("builds and loads server-only channels with conditions from the Node command line", async () => {
    ...
    await execFileAsync(
      process.execPath,
      ["--conditions=react-server", join(app.appRoot, "node_modules/eve/bin/eve.js"), "build"],
      {
        cwd: app.appRoot,
        timeout: 90_000,
