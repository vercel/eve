...
export async function createCompiledArtifactsBootstrapSource(input: {
  ...
  return [
    ...
    `import { installEveWorkflowQueueNamespace } from ${stringifyEsmImportSpecifier(resolvePackageSourceFilePath("src/internal/workflow/queue-namespace.ts"))};`,
    `import { resolveBundledCompilerArtifactsRoot } from ${stringifyEsmImportSpecifier(resolvePackageSourceFilePath("src/runtime/bundled-compiler-artifacts-root.ts"))};`,
    "",
    `installEveWorkflowQueueNamespace(${JSON.stringify(agentName)});`,
    "",
    moduleMapSource,
    ...
    "    compilerArtifactsRoot: resolveBundledCompilerArtifactsRoot(import.meta.url),",
    "    manifest,",
    "    metadata,",
    "    moduleMap,",
    "  });",
