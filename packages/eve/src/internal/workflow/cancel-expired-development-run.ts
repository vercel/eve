...
import { isInactiveDevelopmentRunError } from "#internal/workflow/is-inactive-development-run-error.js";
...
export async function cancelExpiredDevelopmentRun(world: World, runId: string): Promise<void> {
  try {
    ...
    try {
      ...
      if (isInactiveDevelopmentRunError(error)) return;
    ...
    if (!isInactiveDevelopmentRunError(error)) throw error;
  }
}
