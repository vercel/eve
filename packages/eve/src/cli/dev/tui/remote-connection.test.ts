...
describe("createRemoteConnectionController", () => {
  it.each([
    ...
    },
    {
      ...
      expected: { state: "unavailable", failure: { cause: "http", status: 401 } },
    },
    {
    ...
    },
    {
      ...
      expected: { state: "unavailable", failure: { cause: "http", status: 503 } },
    },
    {
      name: "a network failure",
      error: new Error("offline"),
      expected: { state: "unavailable", failure: { cause: "network", message: "offline" } },
    },
    {
      ...
      expected: {
        state: "unavailable",
        failure: { cause: "invalid-response", message: "bad JSON" },
      },
    },
  ...
  it("waits for deployment authority before probing or resolving ambient credentials", async () => {
    ...
    const harness = createHarness({ resolveDeployment, resolveOidcToken });
  ...
  it("uses the authenticated token resolver for every request", async () => {
    ...
    await expect(
      harness.controller.authenticate("startup", async () => ({
        kind: "prepared",
        target: VERIFIED_TARGET,
        resolveToken,
        completedMutations: [],
  ...
  it("reports a rejected token and its completed mutations without retrying", async () => {
    ...
    await harness.controller.check();
    await expect(harness.controller.authenticate("startup", attempt)).resolves.toEqual({
      kind: "failed",
      failure: {
    ...
    expect(harness.credentials.current()).toEqual({ kind: "anonymous" });
  });
  ...
  it("restores prior connection authority when verification is cancelled", async () => {
    ...
    expect(harness.credentials.current()).toEqual({ kind: "vercel", target: VERIFIED_TARGET });
    await expect(harness.credentials.resolveHeaders()).resolves.toMatchObject({
