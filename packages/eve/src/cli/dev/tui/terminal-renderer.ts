...
export class TerminalRenderer implements AgentTUIRenderer {
  ...
  #backgroundCallIds = new Set<string>();
  ...
  renderConversation(view: AgentTUIConversationView): void {
    ...
    this.#view = view;
    const previousBackground = this.#backgroundCallIds;
    this.#backgroundCallIds = new Set(
      Object.values(view.conversation.children)
        .filter((call) => call.background && call.observation.status !== "ended")
    ...
    for (const callId of this.#backgroundCallIds) {
      if (!previousBackground.has(callId)) this.#moveSubagentCohortToBackgroundTail(callId);
    }
  ...
  #pushBlock(block: Block) {
    ...
    const isBackgroundChild =
      block.subagentCallId !== undefined && this.#backgroundCallIds.has(block.subagentCallId);
    const backgroundIndex = isBackgroundChild
      ? -1
      : this.#blocks.findIndex(
          (candidate) =>
            candidate.subagentCallId !== undefined &&
            this.#backgroundCallIds.has(candidate.subagentCallId),
        );
  ...
  #pushStatusLine(rows: string[], width: number): void {
    ...
    if (agentModel?.routing.kind === "dynamic") {
      ...
      input.model = modelId === undefined ? "dynamic model" : `dynamic model · ${modelId}`;
