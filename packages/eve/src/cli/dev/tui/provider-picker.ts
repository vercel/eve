...
function keyPhase(
  ...
  return selectValueAtCursor(options, select.cursor) === "ai-gateway-key"
...
export function transitionProviderPicker(
  ...
  if (state.phase.kind === "validating") {
    ...
    return {
      kind: "settle",
      result: { kind: "ai-gateway-key", key: state.phase.key, validation: event.validation },
    };
  }
  ...
  switch (event.type) {
    ...
    case "submit": {
      ...
      if (value === "ai-gateway-project" || value === "chatgpt" || value === "external") {
        return { kind: "settle", result: { kind: value } };
      }
      if (value !== "ai-gateway-key" || state.phase.kind === "inactive") return ignore(state);
