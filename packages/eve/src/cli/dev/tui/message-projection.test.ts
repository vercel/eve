...
describe("terminal message projection", () => {
  ...
  it("preserves the second run after removing the first run with a reused step index", () => {
    const reducer = defaultMessageReducer();
    const terminal = new TerminalMessageProjection();
    let data = reducer.initial();
    const events = [
      ...
      {
        type: "step.completed",
        data: { turnId: "t0", sequence: 0, stepIndex: 0, finishReason: "tool-calls" },
      },
      {
        type: "message.appended",
        data: { turnId: "t0", sequence: 0, stepIndex: 0, messageDelta: "answer" },
      },
      {
        type: "message.completed",
        data: { turnId: "t0", sequence: 0, stepIndex: 0, message: "answer", finishReason: "stop" },
      },
    ] as EveAgentReducerEvent[];
    const updates = events.flatMap((event) => {
      data = reducer.reduce(data, event);
    ...
    expect(updates).toEqual([
      ...
      { type: "assistant-delta", id: "text:t0:0#1", delta: "answer" },
      { type: "assistant-complete", id: "text:t0:0#1" },
    ]);
  });

  it("does not replay completed blocks when tool parts change between text runs", () => {
    const reducer = defaultMessageReducer();
    const terminal = new TerminalMessageProjection();
    let data = reducer.initial();
    const events = [
      {
        type: "message.completed",
        data: { turnId: "t0", sequence: 0, stepIndex: 0, message: "first", finishReason: "stop" },
      },
      {
        type: "actions.requested",
        data: {
          ...
          sequence: 0,
          stepIndex: 0,
          actions: [{ kind: "tool-call", callId: "call", toolName: "tool", input: {} }],
        },
      },
      {
        type: "message.completed",
        data: { turnId: "t0", sequence: 0, stepIndex: 0, message: "second", finishReason: "stop" },
      },
    ] as EveAgentReducerEvent[];
    const updates = events.flatMap((event) => {
      data = reducer.reduce(data, event);
    ...
    expect(updates).toEqual([
      { type: "assistant-complete", id: "text:t0:0", text: "first" },
      { type: "assistant-complete", id: "text:t0:0#1", text: "second" },
    ]);
  });
});
