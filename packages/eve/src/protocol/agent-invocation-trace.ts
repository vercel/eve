...
export type AgentInvocationRole =
  (typeof AGENT_INVOCATION_ROLES)[keyof typeof AGENT_INVOCATION_ROLES];
export const AGENT_SESSION_KINDS = {
  delegated: "delegated",
  root: "root",
} as const;
export type AgentSessionKind = (typeof AGENT_SESSION_KINDS)[keyof typeof AGENT_SESSION_KINDS];
...
/** Builds the strict remote-create trace extension without internal SpanContext fields. */
export function buildAgentInvocationTrace(input: {
  ...
  if (input.parent !== undefined) trace.parent = traceCoordinates(input.parent);
  return trace;
}

export type AgentInvocationBindingError = "call-id-mismatch" | "trace-context-mismatch";

/** Cross-validates fields whose relationships cannot be expressed by their individual schemas. */
