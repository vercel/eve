...
export async function coordinateApprovalDelivery(input: {
  ...
  if (discardedDuplicate && !hasMeaningfulInput(deduplicatedInput)) {
    return deliveryResult(session, deduplicatedInput, "park");
  }
  if (batch === undefined) return deliveryResult(session, deduplicatedInput);

  const stepInput = normalizePendingInputResponses({
    state: session.state,
    stepInput: deduplicatedInput,
  });
