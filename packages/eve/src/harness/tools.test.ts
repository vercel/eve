...
describe("buildToolSet", () => {
  ...
  ] as const)("rejects content file parts with %s", async (_label, data, expected) => {
    const tools: HarnessToolMap = new Map<string, HarnessToolDefinition>([
      [
        "screenshot",
        {
          ...
          toModelOutput: () => ({
            type: "content" as const,
            value: [{ type: "file" as const, data: data as never, mediaType: "image/png" }],
          }),
        },
      ],
    ]);
  ...
  it("rejects empty content part arrays with the toModelOutput error identity", async () => {
    const tools: HarnessToolMap = new Map<string, HarnessToolDefinition>([
      [
        "screenshot",
        {
          ...
          inputSchema: jsonSchema({}),
          name: "screenshot",
          toModelOutput: () => ({ type: "content" as const, value: [] }),
        },
      ],
    ]);
    ...
    await expect(
      ...
      'Tool "screenshot" call "call_screenshot" returned a non-JSON-serializable model output. ' +
        'Expected content model output "value" to be a non-empty array.',
    );
  });

  it("rejects unknown content part types", async () => {
    const tools: HarnessToolMap = new Map<string, HarnessToolDefinition>([
      [
        "screenshot",
        {
          ...
          toModelOutput: () => ({
            type: "content" as const,
            value: [{ type: "media", data: "aGVsbG8=", mediaType: "image/png" } as never],
          }),
        },
      ],
    ]);
    ...
    await expect(
      projectSdkToolOutput({ output: { ok: true }, tool: result.screenshot }),
    ).rejects.toThrow('Expected content part type to be "text" or "file".');
  });
