...
export const WORKFLOW_RULES: readonly SemanticErrorRule[] = [
  {
    ...
    message:
      "The local workflow store (`.eve/.workflow-data`) was created by an incompatible version. Remove that directory to reset local sessions, then retry.",
  },
  {
    ...
    message:
      "The local workflow store (`.eve/.workflow-data`) could not be created or accessed. Check filesystem permissions and free disk space, then retry.",
  },
  {
    ...
    message:
      "The durable workflow replay diverged from the recorded run — usually caused by code changing mid-session in a way the runtime cannot reconcile. Start a fresh session (`/new` in `eve dev`).",
  },
  {
    ...
    message:
      "The durable event log for this run is corrupted. Remove `.eve/.workflow-data` to reset local sessions, then retry.",
  },
  {
    ...
    message:
      "This session's durable run no longer exists (the local store was reset or pruned). Start a new session.",
  },
];
