 ...
 * Source: packages/sandbox-vercel/src/vercel-network-policy-manager.ts
 * in https://github.com/vercel/ai.
 */
...
/**
 * Other than vendored imports and formatting, this is a direct copy of
 * AI SDK's own `VercelNetworkPolicyManager` class from the
 * `@ai-sdk/sandbox-vercel` package.
 *
 * It handles scoped iterative updates over Vercel Sandbox's
 * `setNetworkPolicy()`.
 */
...
export class VercelNetworkPolicyManager {
  ...
  setNetworkPolicy(policy: HarnessV1NetworkPolicy): Promise<void> {
    return this.#enqueueMutation(async () => {
      ...
      if (this.#state == null && inspection.requestTransformationHosts.length > 0) {
  ...
  addRequestTransformations(
    ...
    return this.#enqueueMutation(async () => {
      const inspection = this.#inspectPolicy();
      const incomingTransformations = transformations.map(cloneRequestTransformation);
  ...
  #inspectPolicy(): CurrentPolicyInspection {
    ...
    return inspectCurrentPolicy(this.#sandbox.currentSession().networkPolicy ?? "allow-all");
  }
  ...
  async #applyState({
    ...
    this.#state = {
      ...
      requestTransformations: requestTransformations.map(cloneRequestTransformation),
...
function toNetworkAccessPolicy(policy: HarnessV1NetworkPolicy): NetworkAccessPolicy {
  switch (policy.mode) {
    case "allow-all":
    ...
    case "custom": {
      ...
      if (allowedHosts.length === 0 && allowedCIDRs.length === 0 && deniedCIDRs.length === 0) {
        throw createPolicyConflictError(
...
function composeNetworkPolicy({
  ...
  const allowedHosts = accessPolicy.mode === "allow-all" ? ["*"] : accessPolicy.allowedHosts;
  ...
  const hasActiveRules = [...rulesByHost.values()].some((rules) => rules.length > 0);
  if (!hasActiveRules) {
...
function toVercelSubnets(
  ...
  if (accessPolicy.allowedCIDRs.length === 0 && accessPolicy.deniedCIDRs.length === 0) {
    return {};
  }
  return {
...
function minimizeHostPatterns(patterns: ReadonlyArray<string>): ReadonlyArray<string> {
  const uniquePatterns = [...new Set(patterns)];
  return uniquePatterns.filter(
    (pattern) =>
      !uniquePatterns.some(
        (other) =>
          other !== pattern && isHostPatternSubset({ candidate: pattern, container: other }),
      ),
  );
}
...
function hasRequestTransformation(rule: NetworkPolicyRule): boolean {
  return (
    rule.transform?.some(
      (transformation) =>
        transformation.headers != null && Object.keys(transformation.headers).length > 0,
    ) ?? false
  );
}

function getRequestTransformationHosts(policy: NetworkPolicy): ReadonlyArray<string> {
  if (
    policy === "allow-all" ||
    policy === "deny-all" ||
    policy.allow == null ||
...
function getRequestTransformationIdentity(transformation: HarnessV1RequestTransformation): string {
...
function createPolicyConflictError(message: string): HarnessCapabilityUnsupportedError {
