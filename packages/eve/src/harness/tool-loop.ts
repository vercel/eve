...
import { resolveDeliveryPolicy } from "#harness/delivery-policy.js";
...
export function createToolLoopHarness(config: ToolLoopHarnessConfig): StepFn {
  ...
  async function executeStepBody(
    ...
    // Task results are delivered at a step boundary inside their turn, never
    ...
    // turn's focus. On a no-input settle resume it trails the tool results,
    // keeping the request user-final for providers that reject
    // assistant-final histories. Appending instead of rewriting keeps the
    ...
    const prepareModelTools = async (opts: ModelCallOptions) => {
      ...
      return { effectiveTools, advertisedHarnessTools, modelTools };
    };

    const runSingleModelCall = async (
      opts: ModelCallOptions & { readonly attemptIndex: number },
    ): Promise<HarnessStepResult> => {
      let { effectiveTools, advertisedHarnessTools, modelTools } = await prepareModelTools(opts);
      ...
      if (compaction.compacted) {
        ...
        currentMessages = createRequestMessages();
        ({ effectiveTools, advertisedHarnessTools, modelTools } = await prepareModelTools(opts));
        requestEnvelopeTokens = await estimateRequestEnvelope({
          history: projectedMessages,
          instructions: prepareModelInstructions(opts.extraSystemNote),
          messages: withTrailingUserNote(currentMessages.nonSystemMessages, opts.trailingUserNote),
...
async function handleStepResult(input: {
  ...
  if (pendingCoordination.length > 0) {
    const tasks = pendingCoordination;
  ...
  // Only working tasks refuse it, with an error result (`holdTurnOnTasks`).
  ...
  const held = await holdTurnOnTasks({
    emissionState,
    emit,
    finalOutputCallId: calledFinalOutput
      ? (result.toolCalls ?? []).find((call) => call.toolName === FINAL_OUTPUT_TOOL_NAME)
          ?.toolCallId
      : undefined,
    runStep,
    session: nextSession,
