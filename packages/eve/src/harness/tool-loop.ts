...
export function createToolLoopHarness(config: ToolLoopHarnessConfig): StepFn {
  ...
  async function executeStepBody(
    ...
    const runSingleModelCall = async (
      ...
      const modelCallHarnessTools = new Map(advertisedHarnessTools);
      ...
      if (ctx !== undefined) {
        ...
        for (const [name, toolDefinition] of Object.entries(dynamicToolSet)) {
          if (name === "code_mode") {
            throw new Error('Dynamic tool "code_mode" uses a reserved framework tool name.');
          }
        ...
        // Snapshot request-visible dynamic definitions with the same precedence
        // as the ToolSet so code mode can apply admission to this provider call.
        for (const definition of dynamicTools.toReversed()) {
          modelCallHarnessTools.set(definition.name, definition);
        }
      }
