...
describe("packed package consumption", () => {
  ...
  it("builds a fresh app using only installed tarball contents", async () => {
    ...
    await access(join(packageRoot, "scaffold/setup.js"));
    await access(join(evePackageRoot, "dist/src/index.js"));
    await expect(
      readFile(join(evePackageRoot, "dist/src/setup/scaffold/channels-catalog.js"), "utf8"),
    ).resolves.not.toContain('from "@eve/catalog"');
