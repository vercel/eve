...
Edit [`lib/agent.ts`](../lib/agent.ts):
...
Also update site metadata in [`apps/web/app/app.config.ts`](../apps/web/app/app.config.ts) (`site.name`, `site.title`, `site.description`, `site.tagline`).

Replace branding assets in [`apps/web/public/`](../apps/web/public/):
...
Use your own design files when ready — keep them in `apps/web/public/` and update `site.ogImage` in [`apps/web/app/app.config.ts`](../apps/web/app/app.config.ts) if the path changes.
...
Categories are defined in [`lib/types/memory.ts`](../lib/types/memory.ts):
...
- [`lib/memory/export-prompt.ts`](../lib/memory/export-prompt.ts) — ChatGPT export prompt
...
Skills are markdown files in [`agent/skills/`](../agent/skills/). See [`daily-summary.md`](../agent/skills/daily-summary.md) for an example. Reference skills from home quick actions in [`apps/web/app/pages/index.vue`](../apps/web/app/pages/index.vue).

## 6. Integrations

### GitHub

Uses Vercel Connect OAuth and [@github-tools/sdk/eve](https://github-tools.com/frameworks/eve). Connector UID: [`lib/connect.ts`](../lib/connect.ts) (`GITHUB_CONNECTOR`), registry: [`server/connectors.ts`](../apps/web/server/connectors.ts), tools: [`agent/tools/github.ts`](../agent/tools/github.ts).
...
2. Update `GITHUB_CONNECTOR` in [`lib/connect.ts`](../lib/connect.ts) if it differs from `vercel connect list`
...
- Global styles: [`apps/web/app/assets/css/main.css`](../apps/web/app/assets/css/main.css)
- Nuxt UI config: [`apps/web/app/app.config.ts`](../apps/web/app/app.config.ts)
- Layout and navigation: [`apps/web/app/layouts/default.vue`](../apps/web/app/layouts/default.vue), [`apps/web/app/components/Navbar.vue`](../apps/web/app/components/Navbar.vue)

## 8. Deploy your fork

See [Deploy on Vercel](../README.md#deploy-on-vercel) in the README. Remember:

- Dual services: `web` + `eve` ([`vercel.ts`](../vercel.ts))
