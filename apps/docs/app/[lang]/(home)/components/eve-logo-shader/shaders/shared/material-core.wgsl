import { sample_cubemap_array } from "./cube-sample.wgsl";
...
export fn env_reflection_from_dir(envCube: texture_2d_array<f32>, envSampler: sampler, reflected: vec3f) -> vec3f {
  return sample_cubemap_array(envCube, envSampler, reflected);
}

export fn env_reflection(envCube: texture_2d_array<f32>, envSampler: sampler, n: vec3f, v: vec3f) -> vec3f {
  return env_reflection_from_dir(envCube, envSampler, env_reflect_dir(n, v));
}
