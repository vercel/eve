...
import { DEFAULT_IMPRINT_GLYPH_SCALE, DEFAULT_IMPRINT_GRID_SCALE_MULTIPLIER, MATERIAL_KIND } from "./constants";
...
export function writeParams(args: { target: { buffer: Buffer }; controls: RenderControls; logicalWidth: number; logicalHeight: number; passKind: number; projectionPaddingRadius: number; imprint?: ImprintRenderOptions; meshBounds: Bounds; orbitTarget: Vec3; thicknessScale: number; isLight: boolean }) {
  const { target, controls, logicalWidth, logicalHeight, passKind, projectionPaddingRadius, imprint = {}, meshBounds, orbitTarget, thicknessScale, isLight } = args;
