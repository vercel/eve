...
| `sandbox.ts`                 | `defineSandbox(() => environment.create())` | every local node | exports `DefaultSandbox.environment()`; stable semantic revision      |
...
Register a public `defineSandbox(() => environment.create())` value and exported
