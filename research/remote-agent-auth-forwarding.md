...
Slack user U ── router deployment ──────────────► site-ops deployment
               auth.current = U      POST /eve/v1/session
                                     ...
                                     acceptForwardedPrincipalFrom(caller):   router app may forward
                                                    │
...
### Receiver: `acceptForwardedPrincipalFrom` on `eveChannel`
...
- `acceptForwardedPrincipalFrom?: (caller: SessionAuthContext) => boolean | Promise<boolean>`. The
...
channel has no `acceptForwardedPrincipalFrom`, so a receiving deployment must author its own
...
- Scenario: two in-process eve servers over real HTTP — router with `forwardPrincipal: true` dispatches
  to a receiver whose `acceptForwardedPrincipalFrom` predicate accepts it, asserting the child session
