...
The hook owns only response suppression:
...
Returning `"skip"` suppresses the pending terminal event. Returning `undefined` releases it. The
serialized history control remains independent: an application may queue
`session.restoreHistory({ to })` through a session handle and separately return `"skip"` from this
hook. This keeps response release useful without history restoration and keeps restoration useful
without response gating.
...
2. **Add the minimal pre-release capability needed by this use case.** Run
   `beforeResponseRelease` after terminal synthesis while `message.completed` is held. Returning
   `"skip"` suppresses the pending response; returning `undefined` releases it normally.
3. **Leave policy, restoration, and coarse rollback scope to the application.** The hook decides
   whether to suppress release. Application code separately chooses a history index and queues the
   serialized restoration control; restoring an earlier prefix may discard unrelated intervening
   input.
