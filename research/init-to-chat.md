...
Activation compares the intended project settings before writing. Only model source changes acquire a runtime suspension lease and wait for activation. Account, team, and key changes are atomic metadata updates; they do not recompile the agent or replace its worker.

The development host owns credential resolution. Workers request current credentials through the authenticated loopback control plane, reusing the host’s OAuth access token and ChatGPT broker across worker replacements. Rejected-token fingerprints allow refresh without placing tokens in URLs. Credential selection happens at request time; deployments continue using their explicit server credentials.

Successful login returns nonsecret model and connection metadata directly to the TUI. `/info` and setup diagnostics refresh in the background, with revision checks discarding stale results. Neither a failed inspection nor a pending footer refresh delays the first message. Source activation still completes before a changed model is used.

Status text describes the current wait: browser sign-in, team loading, Gateway access, model availability, or loading the selected model. Local settings reads and credential validation have distinct labels. Loading indicators wait 150 ms to avoid flashing for quick work. Fixed-name debug events measure source inspection, catalog loading, authentication, and activation without recording credentials or team details.
