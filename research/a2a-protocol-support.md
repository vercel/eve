...
The first release implements the authoring surfaces and JSON-RPC operations below. Public setup and limits are documented in `docs/subagents/a2a.md` and `docs/channels/a2a.md`. Remote task-level authorization returns a resumable result to the parent; continuing the handle resumes polling. SSE publishes durable snapshots and completed artifacts rather than model-token deltas. General task listing scans at most 1,000 canonical workflow metadata records and authorizes before reading task content.
...
| `SendStreamingMessage`    | Return the initial task, then ordered `statusUpdate` and `artifactUpdate` SSE events. |
...
length, and artifact inclusion. Ownership is checked before task content is read; returned counts include only accessible tasks.
...
| Working                | `TASK_STATE_WORKING`        | Current durable execution state.                      |
...
| `subagents/a2a-dispatch.ts`           | Lower A2A agents to background subagent execution.                                                            |
...
| Invocation kernel                     | Add bounded history and owner-scoped listing; the channel owns SSE polling.                                   |
...
- **Owner-scoped listing:** scan at most 1,000 canonical workflow metadata records and filter ownership before reading task content. This works across workflow worlds without depending on optional, eventually consistent analytics. Beyond that bound, callers supply `contextId`.
- **Agent version:** `card.version` is optional and defaults to `1.0.0`.
- **Multiple agents per deployment:** one public card route per origin. A future card-routing surface can support multiple independently advertised agents.
- **Remote input and authorization:** return a resumable subagent result with the remote message. Input continuation sends a message on the same task; authorization continuation polls it. Shared credential-provider authorization uses eve's existing challenge/callback flow.
- **Verification:** serving and consuming ship together. The official SDK scenario covers discovery, task lifecycle, SSE, delegation, authenticated requests, and dynamic definitions. A fixture-owned eval covers the inbound lifecycle and owner isolation across workflow worlds. Outbound interoperability runs in the scenario tier so it can use a public local card without deployment-protection credentials.
