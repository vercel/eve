...
Most fixture agents default to `anthropic/claude-sonnet-5`, overridable in one
place by setting `EVE_EVAL_MODEL` for the run (e.g.
`EVE_EVAL_MODEL=openai/gpt-5.5 pnpm test:e2e`); judges run against
`openai/gpt-5.5`. Both are real models, so the environment must provide the
corresponding model-provider credentials. `agent-workflow-stress` uses eve's
