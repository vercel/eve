...
`agent-channels/evals/custom-channels/session-inbox-redeploy.eval.ts`
holds a turn active, redeploys the fixture with the same inbox wire format,
and queues a replacement message through the existing session's metadata-free
hooks. It then cancels the blocked turn and verifies that the session runs the
buffered follow-up.
