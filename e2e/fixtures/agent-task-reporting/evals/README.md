...
Each trial launches exactly three independent warehouse tasks in one root turn.
The `probe` tool requires a separate approval for each lookup instead of delaying
execution with a timer. The eval waits for all three approval requests, approves
the first, and reads that child's completed result from its own session stream.
The other two requests remain unapproved during the partial-state checks.
