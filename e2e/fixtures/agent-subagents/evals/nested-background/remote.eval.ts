...
export default defineEval({
  ...
  timeoutMs: 240_000,
  description:
    "A remote subagent yields with nested work pending and delivers the final result once.",
  async test(t) {
    const key = crypto.randomUUID();
    const sessions: string[] = [];
    try {
      // The caller delegates to remote-loopback and yields.
      const caller = await t.session();
      sessions.push(caller.sessionId);
      const callerTurn = (
        await caller.send(
          signOffRequest(
            "Hand this off with the remote-loopback tool, using the note below as its message.",
            key,
          ),
        )
      ).expectOk();
      callerTurn.requireToolCall("remote-loopback", { output: { status: "working" } });
      ...
      assert.equal(detectorCall.data.name, "remote-loopback");
      assert.ok(detectorCall.data.remote, "detector must use the remote HTTP transport");
      ...
      callerResult.event("subagent.completed", {
        data: { subagentName: "remote-loopback", output: (output) => output.includes(receipt) },
        count: 1,
      });
