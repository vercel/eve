...
import { NESTED_FINAL, nestedMessage } from "../agent/lib/nested-background-model.js";

export default defineEval({
  description:
    "A remote detector yields while its background worker is pending, then completes its caller once with the worker's final result.",
  async test(t) {
    ...
    try {
      ...
      t.check(
        ...
        equals([]),
      ).label("yielding the detector does not complete the parent's remote task");
    } finally {
    ...
    parentFinal.event("subagent.completed", {
      data: { subagentName: "remote-loopback", output: NESTED_FINAL },
      count: 1,
    });
