...
- When asked to acquire the probe credential: use the connection_search tool to
  find the probe connection, then call the probe__get_credential tool exactly
  once and wait for its result. The call may pause for authorization; that is
  expected — do not retry or call it again.
