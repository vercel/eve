 ...
 *   real Python runtime executes template-authored code.
 * - The sandbox definition runs once per live session. It writes a per-session marker
 ...
 * the preparation below assumes that real-binary environment and is not meant
 * to run against the dependency-free `just-bash` fallback.
 *
 * `EVE_TEST_AUTHOR_SNAPSHOT_ID`, when set, overrides the provider with
 * `VercelSandbox.template({ source: { type: "snapshot", snapshotId } })` so the
 * sandbox-author-snapshot smoke test can verify that an author-supplied
 * snapshot is honored as the template base layer while preparation still
 * runs on top.
 */
export const SANDBOX_MARKER_PATH = "/workspace/smoke-marker.txt";
export const SANDBOX_MARKER_TOKEN = "sandbox-template-prepare-ok-J3Q";

/**
 * Custom CLI installed during template preparation. `/usr/local/bin` is on the default
 ...
 * whether preparation runs as root (Docker) or as `vercel-sandbox` (Vercel).
 */
export const SANDBOX_CLI_PATH = "/usr/local/bin/eve-greet";
export const SANDBOX_CLI_TOKEN = "eve-greet-cli-ok-R7M";

/** Per-session marker written by the definition (live session, not the template). */
export const SANDBOX_SESSION_MARKER_PATH = "/workspace/session-marker.txt";
export const SANDBOX_SESSION_MARKER_TOKEN = "sandbox-session-setup-ok-X5T";
...
export const template =
  authorSnapshotId !== undefined
    ...
    : DefaultSandbox.template({ prepare: prepareSandbox });

async function prepareSandbox(sandbox: Awaited<ReturnType<typeof DefaultSandbox.create>>) {
  await sandbox.writeTextFile({
    path: SANDBOX_MARKER_PATH,
    content: SANDBOX_MARKER_TOKEN,
  });
  // Install a custom CLI onto the PATH and make it executable. Later
  // sessions inherit it from the template without re-running preparation.
  ...
  if (chmod.exitCode !== 0) {
    throw new Error(`prepare: chmod of ${SANDBOX_CLI_PATH} failed: ${chmod.stderr}`);
  }
}
