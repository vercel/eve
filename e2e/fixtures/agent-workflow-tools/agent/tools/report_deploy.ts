...
/**
 * Background workflow tool: the model gets a receipt, each `yield` is a
 * durable progress note, and the agent is woken with the return value.
 */
export default defineTool({
  ...
  async *execute({ service }) {
    ...
    yield `WORKFLOW-REPORT-PROGRESS ${plan}`;
