...
export default defineEval({
  ...
  async test(t) {
    const result = await t.send(
      "Run the bash command `cat /usr/local/share/eve-image-marker /workspace/workspace-marker.txt $HOME/.agents/skills/mounted-skill/SKILL.md` and reply with the command output verbatim.",
    );
    t.succeeded();
