...
export default defineEval({
  ...
  async test(t) {
    const result = await t.send(
      "Run the bash command `cat /tmp/eve-image-marker /workspace/workspace-marker.txt $HOME/.agents/skills/mounted-skill/SKILL.md /workspace/.eve/provider` and reply with the command output verbatim.",
    );
    t.succeeded();
