...
export default defineChannel({
  routes: [
    ...
    POST("/threads/:threadId/stop", async (_request, { cancel, params }) => {
      const result = await cancel(params.threadId ?? "");
      return Response.json(result);
    }),
    POST("/threads/:threadId/new", async (_request, { params, reset }) => {
      const result = await reset(params.threadId ?? "", {
    ...
    POST("/threads/:threadId/compact", async (_request, { compact, params }) => {
      const result = await compact(params.threadId ?? "");
      return Response.json(result);
    }),
    POST("/threads/:threadId/clear", async (_request, { clear, params }) => {
      const result = await clear(params.threadId ?? "");
      return Response.json(result);
    }),
