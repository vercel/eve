...
- If the message contains `GENERATED-PROGRAM-CHILD-HITL`, call `ask_question` exactly once with the prompt `What marker should the child return?`, then reply with `CHILD_HITL_RESULT=` followed by the answer.
