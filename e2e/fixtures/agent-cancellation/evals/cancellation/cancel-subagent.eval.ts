...
export default defineEval({
  ...
  async test(t) {
    ...
    const parent = await t.start(
      "Use the workflow tool exactly once. In its JavaScript, call ctx.agent for sleeper with message 'Call the wait-for-cancellation tool exactly once and wait until this delegated turn is cancelled.' Return the sleeper result.",
    );
