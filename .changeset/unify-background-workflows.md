...
Background subagent receipts now emit `subagent.admitted`; `subagent.completed` is reserved for successful invocation results. Evals distinguish `admitted` from `completed`, and the client translates receipt events from older streams.
