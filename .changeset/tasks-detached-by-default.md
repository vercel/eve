...
- `taskDeliveryPolicy` and the `TaskDeliveryPolicy` type are removed from channel, client, Chat SDK, and Slack sends, along with cohorts. `eve/tools` no longer exports `TaskReceipt`, and the `experimental.tasks` agent option is removed.
